  ...
  switch (config.provider) {
    case 'assemblyai':
      ...
      if (!cachedProvider.isAvailable()) {
        throw new Error('AssemblyAI provider selected but API key not configured');
      }
      break;
      
    case 'stub':
      if (!config.enableStressTestMode) {
        console.warn('[Transcription Provider] WARNING: Stub provider requires ENABLE_STRESS_TEST_MODE=true');
      }
      cachedProvider = new StubTranscriptionProvider();
      break;
      
    default:
      console.warn(`[Transcription Provider] Unknown provider: ${config.provider}, falling back to AssemblyAI`);
      cachedProvider = new AssemblyAIProvider();
  }
  
  console.log(`[Transcription Provider] Using: ${cachedProvider.name} (type: ${cachedProvider.type})`);
...
export function isTranscriptionProviderAvailable(): boolean {
  try {
    ...
    console.error('[Transcription Provider] Error checking availability:', error);
    return false;
  }
}
