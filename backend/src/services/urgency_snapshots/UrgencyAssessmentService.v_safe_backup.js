...
class UrgencyAssessmentService {
  ...
  assessUrgency(transcript, context = {}) {
    ...
    if (this.engine) {
      try {
        ...
        // Apply STABLE THRESHOLDS
        let boostedUrgencyLevel;
        if (boostedScore >= 0.8) boostedUrgencyLevel = "CRITICAL";
        else if (boostedScore >= 0.6) boostedUrgencyLevel = "HIGH";
        else if (boostedScore >= 0.3) boostedUrgencyLevel = "MEDIUM";
  ...
  fallbackAssessment(transcript, context = {}) {
    ...
    // Convert score to level using STABLE THRESHOLDS
    let urgencyLevel;
    if (score >= 0.8) urgencyLevel = "CRITICAL";
    else if (score >= 0.6) urgencyLevel = "HIGH";
    else if (score >= 0.3) urgencyLevel = "MEDIUM";
