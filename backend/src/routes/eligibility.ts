...
router.post(
  ...
  async (req, res) => {
    try {
      const { questionnaire, context, userId, sessionId, includeAIAnalysis } =
        req.body;
      ...
      if (includeAIAnalysis) {
        try {
          ...
          logger.warn(
            "AI analysis failed, proceeding with rules engine result only:",
            aiError,
          );
        }
      }
      ...
      const response = {
        ...
        metadata: {
          ...
          disclaimer:
            "This assessment provides informational guidance only. Actual eligibility must be determined by program administrators.",
        },
      };
      ...
      res.status(500).json({
        ...
        fallback: {
          message:
            "Please contact your local social services office for assistance",
          emergencyContacts: [
...
router.get("/programs", async (req, res) => {
  try {
    ...
    const programs = await prisma.aidProgram.findMany({
      where: whereClause,
      orderBy: [{ category: "asc" }, { jurisdiction: "asc" }, { name: "asc" }],
    });
...
router.get("/programs/:id", async (req, res) => {
  try {
    ...
    const eligibilityStats = {
      ...
      likelyEligible: recentAssessments.filter(
        (a) => a.overallResult === "LIKELY_ELIGIBLE",
      ).length,
      possiblyEligible: recentAssessments.filter(
        (a) => a.overallResult === "POSSIBLY_ELIGIBLE",
      ).length,
      averageConfidence:
        recentAssessments.length > 0
          ? Math.round(
              recentAssessments.reduce((sum, a) => sum + a.confidenceScore, 0) /
...
router.get("/quick-guidance", async (req, res) => {
  try {
    ...
    res.json({
      ...
      disclaimer:
        "This is general guidance only. Please verify information with official program sources.",
    });
  } catch (error) {
      ...
      res.json({
        ...
        statistics: {
          ...
          averageConfidence: Math.round(
            avgConfidence._avg.confidenceScore || 0,
          ),
          activePrograms: await prisma.aidProgram.count({
            where: { isActive: true },
          }),
        },
      });
