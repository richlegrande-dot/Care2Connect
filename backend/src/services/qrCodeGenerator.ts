...
export async function createPaymentQR(
  options: CreatePaymentQROptions,
): Promise<CreatePaymentQRResult> {
  try {
    console.log(
      `[QR Generator] Creating payment QR for ticket ${options.ticketId}`,
    );
    ...
    const successUrl =
      options.successUrl ||
      `${baseUrl}/payment-success?ticket=${options.ticketId}`;
    const cancelUrl =
      options.cancelUrl ||
      `${baseUrl}/payment-cancel?ticket=${options.ticketId}`;
    ...
    if (existingQR) {
      ...
      if (existingQR.amountCents && existingQR.amountCents !== amountCents) {
        ...
        console.log(
          `[QR Generator] Amount changed from $${existingQR.amountCents / 100} to $${amountCents / 100}, regenerating QR`,
        );
      } else {
        // Amount same, reuse existing QR
        console.log(
          `[QR Generator] Reusing existing QR code for ticket ${options.ticketId}`,
        );
        return {
          success: true,
    ...
    const checkoutResult = await createCheckoutSession({
      ...
      description:
        options.description ||
        `Donation for ${ticket.displayName || "Campaign"}`,
      successUrl,
      cancelUrl,
    });
    ...
    if (process.env.SAVE_QR_IMAGES === "true") {
      ...
      qrImagePath = path.join(
        qrDir,
        `qr-${options.ticketId}-v${existingQR ? existingQR.version + 1 : 1}.png`,
      );
    ...
    if (existingQR && shouldRegenerateQR) {
      ...
      console.log(
        `[QR Generator] QR code updated to version ${qrCode.version}`,
      );
    } else {
      // Create new QR code
