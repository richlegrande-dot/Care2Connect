...
import fs from 'fs';
import { processTicket, getTicketStatus } from '../services/donationPipeline/orchestrator';
import { generateDocument, getTicketDocuments } from '../services/documentGenerator';
import { createPaymentQR, getTicketQRCode } from '../services/qrCodeGenerator';
import { jobQueue, PipelineJob } from '../services/donationPipeline/jobOrchestrator';
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (include && typeof include === 'string') {
      ...
      if (parts.includes('attributions')) includeOptions.stripeAttributions = true;
      if (parts.includes('qr')) includeOptions.qrCodeLink = true;
      if (parts.includes('transcriptions')) includeOptions.transcriptionSessions = true;
 ...
 */
router.post('/:id/upload-audio', upload.single('audio'), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
router.patch('/:id/draft', async (req: Request, res: Response) => {
  try {
    ...
    if (beneficiaryLocation !== undefined) updateData.beneficiaryLocation = beneficiaryLocation;
    ...
    if (Object.keys(updateData).length === 0) {
      return res.status(400).json({
        error: 'No update fields provided',
        allowed: ['title', 'story', 'goalAmount', 'beneficiaryName', 'beneficiaryLocation', 'currency', 'breakdown'],
      });
    }
...
router.post('/:id/process', async (req: Request, res: Response) => {
  try {
    ...
    processTicket(id, { audioFilePath })
      .then(result => {
        console.log(`[Tickets API] Pipeline completed for ticket ${id}:`, result.finalStatus);
      })
      .catch(error => {
  ...
  try {
    ...
    if (!qrCode) {
      return res.status(404).json({
        ...
        message: 'Create a payment first using POST /api/tickets/:id/create-payment',
      });
    }
...
router.get('/:id/status', async (req: Request, res: Response) => {
  try {
    ...
    if (!job) {
      ...
      const mappedStatus = statusMap[ticket.status] || { status: 'READY', stage: 'CREATED', progress: 0 };
