...
async function checkTLS(
  url: string,
): Promise<{ success: boolean; error?: string; certificateInfo?: any }> {
...
router.get("/status", async (req: Request, res: Response) => {
  try {
    ...
    // Tunnel status check
    let tunnelStatus: ConnectivityStatus["tunnelStatus"] = {
      lastKnownUrl: tunnelUrl,
    };
    if (tunnelUrl) {
      try {
        const tunnelResult = await makeHttpRequest(
          `${tunnelUrl}/health/live`,
          "GET",
        );
        tunnelStatus = {
          lastKnownUrl: tunnelUrl,
    ...
    const systemCheck = dnsChecks.find(
      (c) => c.resolver !== "1.1.1.1" && c.resolver !== "8.8.8.8",
    );
...
router.post("/test", async (req: Request, res: Response) => {
  try {
    ...
    if (testDns) {
      ...
      testResults.dns = {
        cloudflare: dnsChecks.find((c) => c.resolver === "1.1.1.1") || {
          success: false,
        },
        google: dnsChecks.find((c) => c.resolver === "8.8.8.8") || {
          success: false,
        },
    ...
    if (testLocal) {
      ...
      if (!localTest.success) {
        recommendations.push(
          "Local backend server is not responding - check if server is running on the correct port",
        );
      }
    } else {
    ...
    if (testPublic) {
      ...
      if (publicUrl) {
        ...
        if (!publicTest.success) {
          recommendations.push(
            "Public URL is not accessible - check tunnel configuration and DNS propagation",
          );
        }
      } else {
        publicTest.url = null;
        recommendations.push(
          "Set PUBLIC_URL environment variable for public access testing",
        );
      }
    ...
    if (testWebhook) {
      ...
      if (publicUrl) {
        ...
        if (!webhookTest.success) {
          recommendations.push(
            "Webhook endpoint is not accessible - verify routing configuration",
          );
        }
      } else {
    ...
    const successfulTests = allTests.filter(
      (t: any) => t.success || (t.cloudflare && t.cloudflare.success),
    );
    const overallStatus =
      successfulTests.length === allTests.length
        ? "pass"
