...
import { TelemetryCollector, calculateQualityScore, withTelemetry } from '../../../src/services/telemetry';
...
describe('Phase 5: Observability and Metrics Tests', () => {
  ...
  describe('Comprehensive Extraction with Telemetry', () => {
    test('should extract all fields and record telemetry', () => {
      const transcript = 'My name is John Smith and I need $5000 for medical bills. This is urgent!';
    ...
    test('should track fallback usage correctly', () => {
      ...
      if (result.results.name.confidence <= 0.1 || result.results.amount.confidence <= 0.1) {
      ...
      for (let i = 0; i < 1000; i++) {
        telemetry.recordParsingMetrics(`session${i}`, {
          ...
          extractedFields: {
            ...
            amount: { extracted: Math.random() > 0.4, confidence: Math.random() },
            relationship: { extracted: true, value: 'myself' },
  ...
  describe('PII Protection Validation', () => {
    ...
    test('should not store transcript content in metrics', () => {
      const transcript = 'This is sensitive personal information that should not appear in telemetry';
      const result = extractAllWithTelemetry(transcript);
