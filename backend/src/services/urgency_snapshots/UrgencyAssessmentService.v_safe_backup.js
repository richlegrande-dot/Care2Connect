...
class UrgencyAssessmentService {
  ...
  assessUrgency(transcript, context = {}) {
    ...
    if (this.engine) {
      try {
        ...
        if (boostedScore >= 0.80) boostedUrgencyLevel = 'CRITICAL';
        else if (boostedScore >= 0.60) boostedUrgencyLevel = 'HIGH';
        else if (boostedScore >= 0.30) boostedUrgencyLevel = 'MEDIUM';
  ...
  fallbackAssessment(transcript, context = {}) {
    ...
    else if (score >= 0.60) urgencyLevel = 'HIGH';
    else if (score >= 0.30) urgencyLevel = 'MEDIUM';
