...
interface ParsingMetrics {
  ...
  extractedFields: {
    ...
    relationship: {
      extracted: boolean;
      value: "myself" | "family_member" | "other";
    };
    urgency: {
      extracted: boolean;
...
export class TelemetryCollector {
   ...
   */
  recordParsingMetrics(
    sessionId: string,
    metrics: Omit<ParsingMetrics, "sessionId" | "timestamp">,
  ): void {
    try {
      ...
      console.error(
        "[TELEMETRY_ERROR] Failed to record parsing metrics:",
  ...
  recordDocumentMetrics(
    sessionId: string,
    metrics: Omit<DocumentMetrics, "sessionId" | "timestamp">,
  ): void {
    try {
      ...
      console.error(
        "[TELEMETRY_ERROR] Failed to record document metrics:",
        (error as Error).message,
      );
    }
  }
  ...
  recordSystemMetrics(): void {
    try {
      ...
      console.error(
        "[TELEMETRY_ERROR] Failed to record system metrics:",
        (error as Error).message,
      );
    }
  }
  ...
  getDashboardMetrics(): {
    ...
    const recentParsing = this.metricsBuffer.filter(
      (m) => m.timestamp > last24Hours,
    );
    const recentDocuments = this.documentMetrics.filter(
      (m) => m.timestamp > last24Hours,
    );
    ...
    const averageConfidence =
      totalExtractions > 0
        ? recentParsing.reduce((sum, m) => sum + m.qualityScore, 0) /
          totalExtractions
        : 0;

    const fallbackCount = recentParsing.filter(
      (m) => m.fallbacksUsed.length > 0,
    ).length;
    const fallbackRate =
      totalExtractions > 0 ? (fallbackCount / totalExtractions) * 100 : 0;

    const qualityDistribution = {
      excellent: recentParsing.filter((m) => m.qualityScore >= 80).length,
      good: recentParsing.filter(
        (m) => m.qualityScore >= 60 && m.qualityScore < 80,
      ).length,
    ...
    const averageSize =
      totalGenerated > 0
        ? recentDocuments.reduce((sum, m) => sum + m.documentSize, 0) /
          totalGenerated
        : 0;

    const docFallbackCount = recentDocuments.filter(
      (m) => m.fallbackUsed,
    ).length;
    const docFallbackRate =
      totalGenerated > 0 ? (docFallbackCount / totalGenerated) * 100 : 0;

    const averageGenerationTime =
      totalGenerated > 0
        ? recentDocuments.reduce((sum, m) => sum + m.generationDuration, 0) /
          totalGenerated
        : 0;
    ...
    const averageMemoryUsage =
      recentSystem.length > 0
        ? recentSystem.reduce((sum, m) => sum + m.memoryUsage.heapUsed, 0) /
          recentSystem.length
        : 0;

    const cacheEffectiveness =
      recentSystem.length > 0
        ? recentSystem.reduce((sum, m) => sum + m.cacheHitRate, 0) /
          recentSystem.length
        : 0;
...
cache_hit_rate_percent ${dashboard.system.cacheEffectiveness}
  ...
  private flushMetrics(): void {
    try {
      ...
      this.metricsBuffer = this.metricsBuffer.filter(
        (m) => now - m.timestamp < retentionPeriod,
      );
      this.documentMetrics = this.documentMetrics.filter(
        (m) => now - m.timestamp < retentionPeriod,
      );
      ...
      console.log(
        `[TELEMETRY] Flushed old metrics. Current buffer sizes: parsing=${this.metricsBuffer.length}, documents=${this.documentMetrics.length}`,
      );
    } catch (error) {
      console.error(
        "[TELEMETRY_ERROR] Failed to flush metrics:",
        (error as Error).message,
      );
    }
  }
  ...
  private calculateAverageProcessingTime(): number {
    ...
    return (
      recent.reduce((sum, m) => sum + m.extractionDuration, 0) / recent.length
    );
  }
  ...
  getHealthStatus(): {
    ...
    const memoryStatus =
      dashboard.system.averageMemoryUsage > 1024
        ? "critical"
        : dashboard.system.averageMemoryUsage > 512
          ? "warning"
          : "healthy";
    ...
    const errorRate =
      this.metricsBuffer.length > 0
        ? (recentErrors / Math.min(100, this.metricsBuffer.length)) * 100
        : 0;
    const errorStatus =
    ...
    const overallStatus = [memoryStatus, errorStatus, latencyStatus].includes(
      "critical",
    )
      ? "critical"
      : [memoryStatus, errorStatus, latencyStatus].includes("warning")
        ? "warning"
        : "healthy";
    ...
    return {
      ...
      checks: {
        memoryUsage: {
          status: memoryStatus,
          value: dashboard.system.averageMemoryUsage,
          threshold: 512,
        },
        errorRate: {
          status: errorStatus,
...
export function calculateQualityScore(
  extractedFields: ParsingMetrics["extractedFields"],
): number {
  let score = 0;
  let totalFields = 0;
