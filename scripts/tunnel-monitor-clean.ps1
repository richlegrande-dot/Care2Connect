...
function Send-Alert {
    ...
    Write-Log "ðŸš¨ ALERT [$Severity]: $AlertMessage" "SECURITY"
...
function Test-TunnelProcess {
    try {
        ...
        if ($process) {
            ...
            if ($cpuUsage -gt 80) {
                Write-Log "âš ï¸ High CPU usage detected: $([math]::Round($cpuUsage, 2))%" "WARN"
            }
            
            Write-Log "âœ… Tunnel process running (PID: $($process.Id), CPU: $([math]::Round($cpuUsage, 2))%)"
            return $true
        }
        Write-Log "âš ï¸ Tunnel process not found" "WARN"
        return $false
    } catch {
        Write-Log "âŒ Error checking tunnel process: $($_.Exception.Message)" "ERROR"
        return $false
    }
}
...
function Test-TunnelConnectivity {
    try {
        ...
        if ($response.StatusCode -eq 200) {
            Write-Log "âœ… Tunnel connectivity OK (Status: $($response.StatusCode), Time: $($responseTime)ms)"
            
            # Warn on slow responses
            if ($responseTime -gt 5000) {
                Write-Log "âš ï¸ Slow response time detected: $($responseTime)ms" "WARN"
            }
            ...
            if ($response.Content) {
                try {
                    ...
                    if ($json.status -ne "OK" -and $json.status -ne "alive") {
                        Write-Log "âš ï¸ Health endpoint returned non-OK status" "WARN"
                    }
                } catch {
        ...
        Write-Log "âš ï¸ Tunnel connectivity issue (Status: $($response.StatusCode))" "WARN"
        return $false
    } catch {
        Write-Log "âŒ Tunnel connectivity failed: $($_.Exception.Message)" "ERROR"
        return $false
    }
}
...
function Start-Tunnel {
    try {
        Write-Log "ðŸ"„ Starting Cloudflare tunnel..." "INFO"
        
        # Find cloudflared executable
        $cloudflaredPath = "C:\Program Files (x86)\cloudflared\cloudflared.exe"
        
        if (-not (Test-Path $cloudflaredPath)) {
            Write-Log "âŒ cloudflared.exe not found at $cloudflaredPath" "ERROR"
            return $false
        }
        ...
        if (Test-TunnelProcess) {
            Write-Log "âœ… Tunnel started successfully" "SUCCESS"
            return $true
        } else {
            Write-Log "âŒ Tunnel failed to start" "ERROR"
            return $false
        }
    } catch {
        Write-Log "âŒ Error starting tunnel: $($_.Exception.Message)" "ERROR"
        return $false
    }
}

function Restart-Tunnel {
    Write-Log "ðŸ"„ Restarting tunnel..." "WARN"
...
Write-Log "========================================" "INFO"
Write-Log "ðŸ›¡ï¸ Hardened Tunnel Monitor Started" "SUCCESS"
...
while ($true) {
    ...
    if ((Get-Date) -gt $restartWindowStart.AddHours(1)) {
        ...
        Write-Log "ðŸ"Š Hourly Summary - Uptime: $uptimeFormatted, Success Rate: $successRate%, Restarts: $($global:TunnelMetrics.RestartAttempts)"
    }
    ...
    if ($processOk -and $connectivityOk) {
        ...
        Write-Log "ðŸš¨ Tunnel health check FAILED (Consecutive: $($global:TunnelMetrics.ConsecutiveFailures))" "ERROR"
        ...
        if ($restartCount -ge $MaxRestarts) {
            ...
            Write-Log "â›" Max restarts reached. Waiting 5 minutes before next check..."
        ...
        if (Restart-Tunnel) {
            ...
            Write-Log "âœ… Tunnel restarted successfully" "SUCCESS"
        } else {
            $global:TunnelMetrics.FailedRestarts++
            Write-Log "âŒ Tunnel restart failed" "ERROR"
