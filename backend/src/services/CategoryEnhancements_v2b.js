...
class CategoryEnhancements_v2b {
  ...
  enhanceCategory(transcript, baseResult) {
    try {
      ...
      const originalCategory =
        baseResult.category || baseResult.extractedCategory || "UNKNOWN";
      ...
      const employmentTransportResult =
        this.disambiguateEmploymentTransportation(text, originalCategory);
      ...
      const healthcareSafetyResult = this.analyzeHealthcareSafetyContext(
        text,
        enhancedCategory,
      );
      ...
      const housingSafetyResult = this.resolveHousingSafetyPriority(
        text,
        enhancedCategory,
      );
      ...
      const educationResult = this.recognizeEducationPatterns(
        text,
        enhancedCategory,
      );
      ...
      const utilitiesResult = this.detectUtilitiesEnhanced(
        text,
        enhancedCategory,
      );
      ...
      const priorityMatrixResult = this.applyPriorityMatrix(
        text,
        enhancedCategory,
      );
      ...
      if (this.debugMode && enhancedCategory !== originalCategory) {
        console.log(
          `[V2b Category Enhancement] ${originalCategory} → ${enhancedCategory} | Reasons: ${reasons.join(", ")}`,
        );
      }
      ...
      console.error(
        "[CategoryEnhancements_v2b] Error in enhanceCategory:",
        error,
      );
      return {
  ...
  disambiguateEmploymentTransportation(text, currentCategory) {
    ...
    const employmentMatches = employmentIndicators.filter((pattern) =>
      pattern.test(text),
    ).length;
    const transportationMatches = transportationIndicators.filter((pattern) =>
      pattern.test(text),
    ).length;

    // Context analysis: root cause vs immediate need
    const rootCauseEmployment =
      /lost.*job.*now.*need|fired.*need.*car|unemployed.*car.*broke/i.test(
        text,
      );
    const rootCauseTransportation =
      /car.*broke.*can't.*work|no.*car.*no.*job|need.*car.*to.*work/i.test(
        text,
      );
    ...
    if (rootCauseEmployment && employmentMatches >= transportationMatches) {
      ...
      rootCauseTransportation &&
      transportationMatches > employmentMatches
    ) {
      return {
  ...
  analyzeHealthcareSafetyContext(text, currentCategory) {
    ...
    const healthcareMatches = healthcareContext.filter((pattern) =>
      pattern.test(text),
    ).length;
    const safetyMatches = safetyContext.filter((pattern) =>
      pattern.test(text),
    ).length;

    // Medical emergency can be both, prioritize based on immediate vs ongoing
    const immediateEmergency =
      /emergency.*room|911|ambulance|life.*threatening/i.test(text);
    const ongoingTreatment =
      /ongoing|continuing|routine|follow.*up|regular/i.test(text);
    ...
    if (immediateEmergency && healthcareMatches >= safetyMatches) {
      ...
      safetyMatches > healthcareMatches &&
      /immediate|right.*now|urgent/i.test(text)
    ) {
      return {
        category: "SAFETY",
  ...
  resolveHousingSafetyPriority(text, currentCategory) {
    ...
    const housingMatches = housingNeeds.filter((pattern) =>
      pattern.test(text),
    ).length;
    const safetyMatches = immediateSafety.filter((pattern) =>
      pattern.test(text),
    ).length;

    // Distinguish between "eviction threat" and "violence threat"
    const evictionThreat = /threatening.*eviction|landlord.*threatening/i.test(
      text,
    );
    const violenceThreat = /threatening.*violence|threatening.*to.*hurt/i.test(
      text,
    );

    if (
      violenceThreat ||
      (safetyMatches > housingMatches &&
        /immediate|urgent|right.*now/i.test(text))
    ) {
      return {
        category: "SAFETY",
  ...
  recognizeEducationPatterns(text, currentCategory) {
    ...
    const educationMatches = educationIndicators.filter((pattern) =>
      pattern.test(text),
    ).length;
  ...
  detectUtilitiesEnhanced(text, currentCategory) {
    ...
    const utilitiesMatches = utilitiesBillPatterns.filter((pattern) =>
      pattern.test(text),
    ).length;

    if (
      utilitiesMatches >= 1 &&
      (currentCategory === "OTHER" || currentCategory === "HOUSING")
    ) {
      return {
        category: "UTILITIES",
  ...
  applyPriorityMatrix(text, currentCategory) {
    ...
    if (detectedCategories.length > 1) {
      for (const priority of priorityOrder) {
        if (
          detectedCategories.includes(priority) &&
