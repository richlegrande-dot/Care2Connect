...
export class StoryExtractionService {
   ...
   */
  async extractGoFundMeData(transcript: string, formData?: any): Promise<ExtractionResult> {
    try {
      const aiProvider = getAIProvider();
      console.log('[StoryExtraction] Generating GoFundMe draft using:', aiProvider.name);
      ...
      console.log('[StoryExtraction] Enhanced signals extracted:', {
        ...
        category: signals.needsCategories.length > 0 ? signals.needsCategories[0].category : 'NONE'
      });
      ...
      let extractedData: any = {
        ...
        timeline: draftResult.timeline || this.estimateTimelineFromUrgency(signals.urgencyLevel),
      ...
      if (!dataValidation.isComplete) {
        console.log('[StoryExtraction] Applying suggestions for incomplete data');
        for (const [field, suggestion] of Object.entries(dataValidation.suggestions)) {
          if (!extractedData[field] || extractedData[field] === null) {
            extractedData[field] = suggestion;
            console.log(`[StoryExtraction] Applied suggestion for ${field}:`, suggestion);
          }
        }
      }
      ...
      if (extractedData && typeof extractedData.extractedAt === 'string') {
        try { extractedData.extractedAt = new Date(extractedData.extractedAt); } catch {}
      }
      if (extractedData && typeof extractedData.lastUpdated === 'string') {
        try { extractedData.lastUpdated = new Date(extractedData.lastUpdated); } catch {}
      }
      ...
      const enhancedMissingFields = signals.missingFields.filter(field => 
        !['name', 'goalAmount', 'story'].includes(field) || !draft[field as keyof GoFundMeDraft]
      );
      
      draft.missingFields = [...new Set([...missingFields, ...enhancedMissingFields])];
      ...
      const confidence = this.calculateEnhancedConfidence(draft, signals, dataValidation);
      ...
      return {
        ...
        confidence,
        errors: [],
        warnings: draft.missingFields.length > 0 ? ['Some fields enhanced with intelligent suggestions'] : []
      };

    } catch (error) {
      console.error('Story extraction error:', error);
      return this.createFallbackResult(transcript, [error instanceof Error ? error.message : 'Unknown error']);
    }
  }
  ...
  async mergeFollowUpAnswers(
    ...
    for (const answer of answers) {
      ...
      if (updatedDraft[fieldName] && typeof updatedDraft[fieldName] === 'object') {
    ...
    updatedDraft.followUpQuestions = this.generateFollowUpQuestions(updatedDraft.missingFields).filter((q): q is NonNullable<typeof q> => q !== null);
...
Return the data as valid JSON following the exact structure specified in the system prompt.`;
  ...
  private generateFallbackTitle(signals: any): string {
    ...
    const category = signals.needsCategories.length > 0 ? signals.needsCategories[0].category.toLowerCase() : 'support';
  ...
  private generateFallbackTags(signals: any): string[] {
    ...
    if (signals.urgencyLevel === 'CRITICAL' || signals.urgencyLevel === 'HIGH') {
    ...
    for (const field of missingFields) {
      switch (field) {
        ...
        case 'title':
          ...
          if (suggestedTitle) {
            questions.push(`We suggested the title "${suggestedTitle}". Would you like to use this or provide a different title?`);
          } else {
            questions.push('What would you like the title of your fundraiser to be?');
          }
          break;
          
        case 'category':
          if (signals.needsCategories.length === 0) {
            questions.push('What category best describes your fundraising need? (Medical, Housing, Emergency, Education, etc.)');
          }
          break;
   ...
   */
  private calculateEnhancedConfidence(draft: any, signals: any, validation: any): number {
  ...
  private calculateOverallConfidence(draft: GoFundMeDraft): number {
    const fields = [
      draft.name, draft.location, draft.category, draft.goalAmount,
      draft.title, draft.storyBody, draft.shortSummary
    ];
   ...
   */
  private createFallbackResult(transcript: string, errors: any[] = []): ExtractionResult {
    const fallbackDraft: GoFundMeDraft = {
      name: { value: undefined, confidence: 0, source: 'extracted' as const },
      dateOfBirth: { value: undefined, confidence: 0, source: 'extracted' as const },
      location: { value: { country: 'United States' }, confidence: 0, source: 'extracted' as const },
      beneficiary: { value: 'myself', confidence: 0.5, source: 'extracted' as const },
      category: { value: undefined, confidence: 0, source: 'extracted' as const },
      goalAmount: { value: undefined, confidence: 0, source: 'extracted' as const },
      title: { value: undefined, confidence: 0, source: 'extracted' as const },
      storyBody: { value: transcript || undefined, confidence: transcript ? 1.0 : 0, source: 'extracted' as const },
      shortSummary: { value: undefined, confidence: 0, source: 'extracted' as const },
      contact: { value: {}, confidence: 0, source: 'extracted' as const },
      consentToPublish: false,
      transcript,
      missingFields: [
        'name', 'dateOfBirth', 'location', 'category', 
        'goalAmount', 'title', 'shortSummary'
      ],
      followUpQuestions: this.generateFollowUpQuestions([
        'name', 'dateOfBirth', 'location.zip', 'category', 'goalAmount', 'title'
      ]).filter((q): q is NonNullable<typeof q> => q !== null),
      extractedAt: new Date(),
      lastUpdated: new Date()
    };
