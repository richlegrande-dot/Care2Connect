...
export class ShelterAvailabilitySync {
  ...
  async syncAllSources(): Promise<{
    ...
    if (this.syncInProgress) {
      logger.warn(
        "SYNC_IN_PROGRESS",
        "Shelter availability sync already in progress",
      );
    ...
    logger.info(
      "SYNC_START_ALL",
      "Starting shelter availability sync for all sources",
    );
    ...
    try {
      for (const source of AVAILABILITY_DATA_SOURCES) {
        if (!source.isActive) {
          logger.debug(
            "SKIP_INACTIVE",
            `Skipping inactive source: ${source.name}`,
          );
          continue;
        }
        ...
        try {
          ...
          logger.info(
            "SYNC_SUCCESS",
            `Successfully synced ${source.name}: ${updated} shelters updated`,
          );
          ...
          failed++;
          logger.error("SYNC_FAILED", `Failed to sync ${source.name}:`, {
      ...
      logger.info(
        "SYNC_COMPLETE",
        `Shelter availability sync complete: ${successful} successful, ${failed} failed, ${totalUpdated} total updates`,
      );
    } finally {
  ...
  async syncDataSource(source: AvailabilityDataSource): Promise<number> {
    logger.info(
      "SYNC_START",
      `Syncing shelter availability from ${source.name}`,
    );
    ...
    try {
      ...
      for (const update of shelterUpdates) {
        try {
          ...
          logger.error(
            "UPDATE_FAILED",
            `Failed to update shelter ${update.shelterId}:`,
            {
              error:
      ...
      logger.error("SYNC_ERROR", `Error syncing from ${source.name}:`, {
        error: error instanceof Error ? error.message : "Unknown error",
      });
      throw error;
    }
  ...
  ): Promise<ShelterAvailabilityUpdate[]> {
    if (!source.url) {
  ...
  private async syncFromHTML(
    ...
    $(".shelter-item, .facility-row, .availability-item").each(
      (index: number, element: any) => {
        const shelterName = $(element)
          .find(".name, .facility-name, h3")
          .first()
          .text()
          .trim();
        const availableBeds = $(element)
          .find(".available-beds, .beds-available")
          .first()
          .text()
          .trim();
    ...
    for (const record of records) {
      try {
        ...
        if (mapping.capacityFields?.total) {
          ...
          if (
            capacity &&
            !update.availableTotal &&
            update.availableTotal !== 0
          ) {
        ...
        logger.warn(
          ...
          `Failed to process record from ${source.name}:`,
          {
            error:
  ...
  private async applyShelterUpdate(
    update: ShelterAvailabilityUpdate,
  ): Promise<void> {
    let shelter;
    ...
    if (!shelter) {
      logger.warn(
        "SHELTER_NOT_FOUND",
        `Shelter not found for update: ${update.shelterId}`,
      );
      return;
    }
    ...
    if (update.availableMen !== undefined)
      updateData.currentAvailableMen = update.availableMen;
    if (update.availableWomen !== undefined)
      updateData.currentAvailableWomen = update.availableWomen;
    if (update.availableFamilies !== undefined)
      updateData.currentAvailableFamilies = update.availableFamilies;
    if (update.availableYouth !== undefined)
      updateData.currentAvailableYouth = update.availableYouth;
    if (update.availableTotal !== undefined)
      updateData.currentAvailableTotal = update.availableTotal;
    ...
    logger.debug(
      "SHELTER_AVAILABILITY_UPDATED",
      `Updated shelter availability: ${shelter.name}`,
      {
        shelterId: shelter.id,
        availableTotal: update.availableTotal,
      },
  ...
  async updateShelterAvailability(
    ...
    logger.info(
      "SHELTER_AVAILABILITY_MANUAL_UPDATE",
      `Manual shelter availability update applied`,
      {
        shelterId,
        updatedBy,
        availableTotal: update.availableTotal,
  ...
  async bulkUpdateFromFile(
    ...
    try {
      ...
      for (const record of records) {
        ...
        try {
          ...
          const update: ShelterAvailabilityUpdate = {
            ...
            availableMen: this.parseNumber(
              record.availableMen || record.available_men,
            ),
            availableWomen: this.parseNumber(
              record.availableWomen || record.available_women,
            ),
            availableFamilies: this.parseNumber(
              record.availableFamilies || record.available_families,
            ),
            availableYouth: this.parseNumber(
              record.availableYouth || record.available_youth,
            ),
            availableTotal: this.parseNumber(
              record.availableTotal || record.available_total,
            ),
            source: "import",
            updatedBy,
          ...
          if (update.shelterId) {
            ...
            results.errors.push(
              `Missing shelter ID for record: ${JSON.stringify(record)}`,
            );
          }
        } catch (recordError) {
          results.errors.push(
            `Failed to process record: ${recordError instanceof Error ? recordError.message : "Unknown error"}`,
          );
        }
      }

      logger.info(
        "SHELTER_AVAILABILITY_BULK_UPDATE_COMPLETE",
        `Bulk shelter availability update complete`,
        {
      ...
      logger.error("BULK_UPDATE_FAILED", "Bulk update failed:", {
        error: error instanceof Error ? error.message : "Unknown error",
      });
      results.errors.push(
  ...
  async getSyncStatus(): Promise<any> {
    const lastSyncResults = Array.from(this.lastSyncResults.entries()).map(
      ([sourceId, result]) => ({
        sourceId,
        ...result,
      }),
    ...
    return {
      ...
      recentActivity: {
        ...
        coveragePercentage:
          totalShelters > 0
            ? ((sheltersWithRecentUpdates / totalShelters) * 100).toFixed(1)
            : "0",
      },
  ...
  async estimateAvailabilityForUnsynced(): Promise<void> {
    logger.info(
      ...
      "Estimating availability for shelters without recent updates",
    );
      ...
      try {
        ...
        if (estimatedTotal > 0) {
          const update: ShelterAvailabilityUpdate = {
            ...
            source: "estimated",
            notes:
              "Estimated availability based on capacity and historical patterns",
          };
        ...
        logger.warn(
          "ESTIMATE_FAILED",
          `Failed to estimate availability for shelter ${shelter.id}:`,
          { error: error instanceof Error ? error.message : "Unknown error" },
        );
      }
    }
    ...
    logger.info(
      "ESTIMATE_COMPLETE",
      `Estimated availability for ${estimatedCount} shelters`,
    );
  }
