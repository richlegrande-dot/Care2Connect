...
const setTaskCompletion = (sessionId: string, taskId: string, completed: boolean): void => {
...
const sortTasksByPriority = (tasks: Task[]): Task[] => {
  return [...tasks].sort((a, b) => {
    const aPriority = PRIORITY_ORDER[a.priority as keyof typeof PRIORITY_ORDER] || 5;
    const bPriority = PRIORITY_ORDER[b.priority as keyof typeof PRIORITY_ORDER] || 5;
...
function TaskItem({ task, taskId, sessionId, isAdvancement = false }: {
  task: Task;
  taskId: string;
  sessionId: string;
  ...
  return (
    <div className={`p-3 rounded-lg border transition-opacity ${
      ...
      isAdvancement ? 'bg-indigo-50 border-indigo-200' : 'bg-white border-gray-200'
    }`}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className={`text-xs ${completed ? 'text-gray-400' : 'text-gray-600'}`}>
            {task.description}
          </p>
        </div>
      </div>
...
export default function EnhancedRoadmapTasks({ 
  ...
  const allTasks = [
    ...actionPlan.immediateTasks.map(t => ({ ...t, horizon: 'immediate' as const })),
    ...actionPlan.shortTermTasks.map(t => ({ ...t, horizon: 'short_term' as const })),
    ...actionPlan.mediumTermTasks.map(t => ({ ...t, horizon: 'medium_term' as const }))
  ];
    ...
    <div className="space-y-6">
        ...
        <div>
          ...
          <div className="mt-3 p-3 bg-indigo-50 rounded-lg border border-indigo-100">
            <p className="text-xs text-indigo-700">
              These tasks are identified as most relevant for moving from Level {currentLevel} to Level {nextLevel} based on priority and category.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="mt-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
        <p className="text-xs text-gray-600">
          <strong>Privacy Note:</strong> Task completion status is stored locally in your browser only. 
          No personal information is sent to our servers when you check tasks as complete.
        </p>
      </div>
    </div>
  );
