...
router.get('/cloudflare/preflight', async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      ...
      instructions: {
        windows: {
          method1: 'Download from https://github.com/cloudflare/cloudflared/releases/latest',
          method2: 'winget install --id Cloudflare.cloudflared',
          method3: 'scoop install cloudflared',
        },
        macos: {
          method1: 'brew install cloudflared',
          method2: 'Download from https://github.com/cloudflare/cloudflared/releases/latest',
        },
        linux: {
          method1: 'curl -L --output cloudflared.deb https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-amd64.deb && sudo dpkg -i cloudflared.deb',
          method2: 'Download from https://github.com/cloudflare/cloudflared/releases/latest',
        },
      },
    });
 ...
 */
router.post('/cloudflare/validate-webhook', async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.json({
      ...
      message: response.status === 400 ? 'Endpoint reachable (400 expected for test request)' : 'Endpoint responding',
    });
  } catch (error: any) {
    res.json({
      reachable: false,
      error: error.message,
      suggestion: 'Check if cloudflared tunnel is running and public URL is correct',
    });
  }
});
