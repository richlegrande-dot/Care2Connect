...
import {
  extractSignals,
  TranscriptInput,
} from "../../src/services/speechIntelligence/transcriptSignalExtractor";
import {
  loadTranscriptFixture,
  loadAllTranscriptFixtures,
  TestFactory,
...
describe("Speech Analysis - Signal Extraction", () => {
  describe("Name Extraction", () => {
    it('should extract name from "My name is" pattern', async () => {
      ...
      expect(result.nameCandidate).toBe(
        fixture.expectedExtraction.nameCandidate,
      );
    });
  ...
  describe("Contact Information Extraction", () => {
    it("should extract email addresses", async () => {
      ...
      expect(result.contactCandidates.emails).toEqual(
        fixture.expectedExtraction.contactCandidates.emails,
      );
    });
    ...
    it("should extract multiple contact methods", async () => {
      ...
      const totalContacts =
        result.contactCandidates.emails.length +
        result.contactCandidates.phones.length;
  ...
  describe("Location Detection", () => {
    it("should extract city and state from transcript", async () => {
      ...
      expectedLocations.forEach((location: any) => {
        expect(
          result.locationCandidates.some((loc) => loc.includes(location)),
        ).toBe(true);
      });
    });
  ...
  describe("Needs Categorization", () => {
    it("should identify housing needs", async () => {
      ...
      const housingNeeds = result.needsCategories.filter(
        (n) => n.category === "HOUSING",
      );
    ...
    it("should identify food needs", async () => {
      ...
      const foodNeeds = result.needsCategories.filter(
        (n) => n.category === "FOOD",
      );
    ...
    it("should identify healthcare needs", async () => {
      ...
      const healthcareNeeds = result.needsCategories.filter(
        (n) => n.category === "HEALTHCARE",
      );
    ...
    it("should identify employment needs", async () => {
      ...
      const employmentNeeds = result.needsCategories.filter(
        (n) => n.category === "EMPLOYMENT",
      );
    ...
    it("should identify safety/crisis needs", async () => {
      ...
      const safetyNeeds = result.needsCategories.filter(
        (n) => n.category === "SAFETY",
      );
  ...
  describe("Urgency Scoring", () => {
    ...
    it("should be normalized between 0.0 and 1.0", async () => {
      ...
      for (const fixture of allFixtures) {
        if (!fixture?.transcript) continue; // Skip fixtures with missing transcript
        const result = await extractSignals({ text: fixture.transcript });
  ...
  describe("Key Points Extraction", () => {
    ...
    it("should handle short transcripts gracefully", async () => {
      ...
      expect(result.keyPoints.length).toBeGreaterThan(0);
      expect(result.keyPoints.length).toBeLessThanOrEqual(
  ...
  describe("Language Detection", () => {
    it("should detect English for English transcripts", async () => {
      ...
      const englishWords = fixture.transcript
        .split(/\s+/)
        .filter(
          (word: string) => /^[a-zA-Z]+$/.test(word) && word.length > 2,
        ).length;
  ...
  describe("Missing Fields Detection", () => {
    ...
    it("should identify missing contact information", async () => {
      ...
      if (
        result.contactCandidates.emails.length === 0 &&
        result.contactCandidates.phones.length === 0
      ) {
  ...
  describe("Edge Cases", () => {
    it("should handle noisy transcripts with filler words", async () => {
      ...
      if (!fixture?.transcript) return; // Skip if fixture unavailable
      const result = await extractSignals({ text: fixture.transcript });

      // Should still extract some information despite noise
      // Name may not be extractable from very noisy input
...
describe("Speech Analysis - Database Persistence", () => {
  ...
  beforeEach(async () => {
    ...
    const session = await TestFactory.createTranscriptionSession(
      prisma,
      testTicketId,
    );
  ...
  it("should create SpeechAnalysisResult record", async () => {
    ...
    const result = await TestFactory.createSpeechAnalysis(
      prisma,
      testSessionId,
      analysisData,
    );
  ...
  it("should handle analysis failure gracefully", async () => {
    ...
    try {
      ...
      if (!ticket) {
        console.warn(
          `Test ticket ${testTicketId} not found - this may indicate a test setup issue`,
        );
