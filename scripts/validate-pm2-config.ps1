...
function Test-PM2ProcessHealth {
    ...
    try {
        ...
        # Check if any processes exist by looking for process IDs in the table
        if ($pm2Output -match 'id.*name.*mode.*status') {
            # Count online vs total by parsing the table
            $onlineMatches = ([regex]::Matches($pm2Output, 'online')).Count
            $idMatches = ([regex]::Matches($pm2Output, '^\s*\d+\s+', [System.Text.RegularExpressions.RegexOptions]::Multiline)).Count
            
            $onlineCount = $onlineMatches
            $totalCount = $idMatches
            
            Write-Status "PM2 processes: $onlineCount online / $totalCount total" "INFO"
            return @{
                healthy = ($onlineCount -eq $totalCount -and $totalCount -gt 0)
                online = $onlineCount
                total = $totalCount
            }
        } else {
            Write-Status "PM2 has no processes running" "WARNING"
            return @{
                healthy = $false
                online = 0
                total = 0
...
function Repair-EcosystemConfig {
    ...
    if ($changesMade.Count -gt 0) {
        # Create optimal Windows-compatible config - use single-quoted here-string
        $ecosystemJs = @'
module.exports = {
  apps: [
    {
...
'@
        ...
        # Write fixed config
        Set-Content -Path $ecosystemConfig -Value $ecosystemJs -Encoding UTF8
        Write-Status "Applied optimal Windows-compatible PM2 configuration" "SUCCESS"
        
        $changesMade | ForEach-Object { Write-Status "  checkmark $_" "SUCCESS" }
        return $true
    }
...
Write-Host "PM2 CONFIGURATION VALIDATOR AND AUTO-FIXER" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan
