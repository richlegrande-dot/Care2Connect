...
export async function extractSignals(
  ...
  if (useEnhancedCategory) {
    ...
    categoryConfidence =
      needsCategories.length > 0 ? needsCategories[0].confidence : 0.5;
  ...
  if (useV2Urgency) {
    ...
    try {
      ...
      console.warn(
        "V2 urgency service failed, falling back to enhanced:",
        error.message,
      );
    ...
    const urgencyAssessment = urgencyEngine.assessUrgency(cleanedText, {
      category: primaryCategory,
      segments: input.sentences?.map((s) => ({
        text: s,
        startMs: 0,
        endMs: 0,
      })),
  ...
  const beneficiaryRelationship = extractBeneficiaryRelationship(
    originalText,
    primaryCategory,
  );
...
function resolveCategoryPriority(
  ...
  const hasJobLoss =
    /(?:lost my job|laid off|fired|unemployed|job loss|no income|lost employment)/i.test(
      lowerTranscript,
    );
  const hasFinancialStress =
    /(?:bills?|rent|mortgage|payments?|behind on|can't pay|struggling)/i.test(
      lowerTranscript,
    );
 ...
 */
function extractBeneficiaryRelationship(
  text: string,
  category: string,
): string | undefined {
  ...
  const relationshipPatterns = [
    {
      pattern:
        /\b(?:my|our)\s+(?:son|daughter|child|kid|baby|toddler|teenager)\b/i,
      relationship: "CHILD",
    },
    {
      pattern: /\b(?:my|our)\s+(?:mom|mother|dad|father|parent)\b/i,
      relationship: "PARENT",
    },
    {
      pattern: /\b(?:my|our)\s+(?:wife|husband|spouse|partner)\b/i,
      relationship: "SPOUSE",
    },
    {
      pattern: /\b(?:my|our)\s+(?:brother|sister|sibling)\b/i,
      relationship: "SIBLING",
    },
    {
      pattern: /\b(?:my|our)\s+(?:friend|neighbor|colleague)\b/i,
      relationship: "OTHER",
    },
...
function extractName(originalText: string, sentences: string[]): string | null {
  ...
  const rejectWords =
    /^(really|not|very|so|quite|practicing|nervous|desperate|panicking|scared|worried|anymore|doing|going|trying|wanting|needing|behind|on|my|the|working|stopped|illness)$/i;

  // **v4.0 CRITICAL**: Reject sentence fragments that are clearly not names
  const rejectPhrases =
    /(?:behind on|stopped working|not practicing|had to stop|due to|because of)/i;
  ...
  for (const sentence of sentences) {
    ...
    for (const pattern of patterns) {
      ...
      if (match && match[1]) {
        ...
        name = name.replace(
          /\s+(calling|going|trying|working|looking|seeking|hoping|stopped|practicing)$/i,
          "",
        );

        // **v4.0 CRITICAL**: Remove leading articles/words before name
        name = name.replace(
          /^(?:Dr\.?|Mr\.?|Mrs\.?|Ms\.?|Miss)\s+(not|stopped|behind|really)\s+/i,
          "",
        );
        ...
        if (
          /\s(?:on|behind|stopped|working|practicing|doing|going|because|illness)$/i.test(
            name,
          )
        ) {
          continue;
...
function extractLocations(text: string): string[] {
  ...
  const inPattern =
    /\sin[\s,]+(?:uh,?\s+)?([A-Z][a-z]+)(?:,\s*([A-Z][a-z]+))?/g;
  let match;
...
function categorizeNeeds(text: string): NeedCategory[] {
  ...
  for (const [category, keywords] of Object.entries(NEEDS_KEYWORDS)) {
    ...
    if (
      lowerText.includes(`not ${categoryLower}`) ||
      lowerText.includes(`not ${categoryLower} related`)
    ) {
...
function calculateNeedsConfidence(categories: NeedCategory[]): number {
  ...
  const avgConfidence =
    topCategories.reduce((sum, cat) => sum + cat.confidence, 0) /
...
function extractKeyPoints(
  ...
  const scoredSentences = sentences.map((sentence, index) => {
    ...
    // Boost for urgency
    if (
      Object.values(URGENCY_KEYWORDS)
        .flat()
        .some((keyword) => sentence.toLowerCase().includes(keyword))
    ) {
      score += 1;
...
function detectMissingFieldsEnhanced(draft: {
  ...
  if (
    ...
    !draft.story.toLowerCase().includes("medical")
  ) {
  ...
  if (
    ...
    !draft.story.toLowerCase().match(/rent|evict|housing|shelter|homeless/)
  ) {
