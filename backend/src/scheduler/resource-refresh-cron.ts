...
export class ResourceRefreshScheduler {
  ...
  private scheduleJobs(jobs: RefreshJobConfig[], frequency: string): void {
    jobs.forEach((jobConfig) => {
      ...
      logger.info(
        `Scheduled ${frequency} job: ${jobConfig.jobName} (${jobConfig.schedule})`,
      );
    });
  }
  ...
  async executeJob(config: RefreshJobConfig): Promise<RefreshJobResult> {
    ...
    if (config.dependencies && config.dependencies.length > 0) {
      ...
      if (!dependencyCheck.success) {
        logger.error(
          `Job ${config.jobName} failed dependency check: ${dependencyCheck.message}`,
        );
    ...
    try {
      ...
      result = this.createJobResult(
        config,
        startTime,
        true,
        jobResult.recordsProcessed,
        0,
        startMemory,
        [],
        jobResult.metrics,
      ...
      if (config.retryAttempts > 0) {
        logger.info(
          `Retrying job ${config.jobName} (attempts remaining: ${config.retryAttempts})`,
        );
    ...
    logger.info(
      `Job ${config.jobName} completed: ${result.success ? "SUCCESS" : "FAILED"} (${result.duration}ms)`,
    );

    return result;
  }

  private async executeJobLogic(
    config: RefreshJobConfig,
  ): Promise<{ recordsProcessed: number; metrics?: any }> {
  ...
  private async runDataIngestion(): Promise<{
    recordsProcessed: number;
    metrics: any;
  }> {
    ...
    const totalRecords = results.reduce(
      (sum, result) => sum + result.newRecords,
      0,
    );
  ...
  private async runClassification(): Promise<{
    recordsProcessed: number;
    metrics: any;
  }> {
  ...
  private async runGeocoding(): Promise<{
    recordsProcessed: number;
    metrics: any;
  }> {
  ...
  private async runRanking(): Promise<{
    recordsProcessed: number;
    metrics: any;
  }> {
    const ranked = await resourceRanking.rankUnrankedResources(
      250,
  ...
  private async runQualityImprovement(): Promise<{
    recordsProcessed: number;
    metrics: any;
  }> {
  ...
  private async runFullDataRefresh(): Promise<{
    recordsProcessed: number;
    metrics: any;
  }> {
    ...
    return {
      recordsProcessed:
        ingestion.recordsProcessed +
        classification.recordsProcessed +
        geocoding.recordsProcessed +
        ranking.recordsProcessed,
      metrics: {
  ...
  private async runComprehensiveReranking(): Promise<{
    recordsProcessed: number;
    metrics: any;
  }> {
  ...
  private async runDataQualityAnalysis(): Promise<{
    recordsProcessed: number;
    metrics: any;
  }> {
  ...
  private async runArchiveOldData(): Promise<{
    recordsProcessed: number;
    metrics: any;
  }> {
  ...
  private async runDatabaseOptimization(): Promise<{
    recordsProcessed: number;
    metrics: any;
  }> {
  ...
  private async checkDependencies(
    ...
    for (const dependency of dependencies) {
      const dependentJob = recentJobs.find(
        (job) => job.jobName === dependency && job.success,
      );
      if (!dependentJob) {
        return {
          success: false,
  ...
  private async cleanupOrphanedRecords(): Promise<{ cleaned: number }> {
    ...
    const totalCleaned =
      orphanedClassified.count + orphanedGeocoded.count + orphanedRanked.count;
  ...
  private async performHealthCheck(): Promise<void> {
    ...
    if (failedJobs.length > 0) {
      logger.warn(
        `Health check: ${failedJobs.length} failed jobs in last 24 hours`,
      );
  ...
  private async checkPipelineHealth(): Promise<void> {
    ...
    const ratios = {
      ...
      geocodingRate:
        counts.classified > 0 ? counts.geocoded / counts.classified : 0,
    ...
    if (ratios.classificationRate < 0.8) {
      logger.warn(
        `Low classification rate: ${(ratios.classificationRate * 100).toFixed(1)}%`,
      );
    }
    if (ratios.geocodingRate < 0.7) {
      logger.warn(
        `Low geocoding rate: ${(ratios.geocodingRate * 100).toFixed(1)}%`,
      );
    }
    if (ratios.rankingRate < 0.9) {
      logger.warn(
        `Low ranking rate: ${(ratios.rankingRate * 100).toFixed(1)}%`,
      );
    }
  }
  ...
  async getJobStatus(): Promise<{
    ...
    const recentResults = this.jobHistory
      .filter(
        (job) => Date.now() - job.endTime.getTime() < 7 * 24 * 60 * 60 * 1000,
      )
      .slice(-20);
