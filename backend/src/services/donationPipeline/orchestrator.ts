import { PrismaClient, TranscriptionSource, TranscriptionEngine } from '@prisma/client';
import { SessionManager } from '../speechIntelligence/sessionManager';
import { getTranscriptionProvider } from '../../providers/transcription';
import { extractSignals, validateSignalQuality } from '../speechIntelligence/transcriptSignalExtractor';
...
export async function processTicket(
  ...
  try {
    ...
    if (!options?.skipTranscription) {
      ...
      result.steps.transcription = await performTranscription(ticketId, audioFilePath);
      
      if (!result.steps.transcription.success) {
        throw new Error(`Transcription failed: ${result.steps.transcription.error}`);
      }
    ...
    console.log(`[Orchestrator] Extracted signals - Quality: ${signalValidation.qualityScore.toFixed(2)}`);
    console.log(`[Orchestrator] Missing fields: ${signals.missingFields.join(', ') || 'none'}`);
    ...
    if (generatedDraft.missingFields.length > 0) {
      console.log(`[Orchestrator] Draft incomplete - missing: ${generatedDraft.missingFields.join(', ')}`);
    ...
    result.steps.analysis = {
      success: true,
      keyPoints: signals.keyPoints,
      sentiment: signals.urgencyScore > 0.7 ? 'urgent' : signals.urgencyScore > 0.4 ? 'moderate' : 'low',
      language: 'en',
    };
    ...
    try {
      ...
      await handleFailure({
        ...
        context: {
          ...
          currentStage: result.steps.transcription?.success ? 'draft_generation' : 'transcription',
          partialResult: result,
        },
        severity: IncidentSeverity.ERROR,
      });
    } catch (incidentError) {
      console.error(`[Orchestrator] Failed to create incident for error:`, incidentError);
    }
...
async function performTranscription(
  ...
  try {
    ...
    const transcriptionResult = await transcriptionProvider.transcribe(audioFilePath);
    const transcript = transcriptionResult.transcript;

    console.log(`[Orchestrator] ✅ Transcription successful via ${transcriptionResult.source}`);
...
async function performAnalysis(
  ...
  try {
    ...
    const positiveWords = ['hope', 'grateful', 'thank', 'happy', 'better', 'improve'];
    const negativeWords = ['difficult', 'struggle', 'hard', 'challenging', 'problem', 'crisis'];
    ...
    const commonEnglishWords = ['the', 'is', 'are', 'and', 'to', 'of', 'in', 'a', 'for'];
...
async function generateDraft(
  ...
  try {
    console.log('[Orchestrator] Generating donation draft with Knowledge Vault guidance');
    ...
    if (template) {
      console.log(`[Orchestrator] Using Knowledge Vault template: ${template.id}`);
    ...
    if (template?.metadata?.validation) {
      minStoryLength = template.metadata.validation.minStoryLength || minStoryLength;
      requiredFields = template.metadata.validation.requiredFields || requiredFields;
    ...
    if (analysis.keyPoints && analysis.keyPoints.length > 0) {
      ...
      title = firstPoint.length > 60 ? firstPoint.substring(0, 57) + '...' : firstPoint;
    } else {
    ...
    if (story.length < minStoryLength) {
      qualityIssues.push(`story under minimum length (${story.length} < ${minStoryLength})`);
    }
    ...
    if (qualityIssues.length > 0) {
      console.warn(`[Orchestrator] Draft quality issues: ${qualityIssues.join(', ')}`);
