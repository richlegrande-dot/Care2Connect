...
export function getFrontendConfig(): FrontendConfig {
  ...
  if (isNaN(port) || port < 1024 || port > 65535) {
    console.error(`❌ CRITICAL: Invalid frontend port: ${process.env.PORT || '3000'}`);
    console.error('   PORT must be a number between 1024-65535');
    process.exit(1);
  }

  const backendPortNum = parseInt(backendPort, 10);
  if (isNaN(backendPortNum) || backendPortNum < 1024 || backendPortNum > 65535) {
    console.error(`❌ CRITICAL: Invalid backend port: ${backendPort}`);
    console.error('   NEXT_PUBLIC_BACKEND_PORT must be a number between 1024-65535');
    process.exit(1);
  }

  if (port === backendPortNum) {
    console.error(`❌ CRITICAL: Frontend and backend ports cannot be the same: ${port}`);
    console.error('   Set different values for PORT and NEXT_PUBLIC_BACKEND_PORT');
    process.exit(1);
  }
 ...
 */
export async function validateBackendConnectivity(config: FrontendConfig): Promise<ValidationResult> {
  const errors: string[] = [];
  const warnings: string[] = [];

  try {
    console.log(`🔍 [FRONTEND] Testing backend connectivity: ${config.backendUrl}/health/live`);
  ...
  if (!isValid && config.strictMode) {
    ...
    console.error('   • Check backend health: curl http://localhost:3001/health/live');
