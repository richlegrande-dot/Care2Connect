...
function validatePortConsistency(): ConfigIssue[] {
  ...
  try {
      ...
      if (portMatch) {
        ...
        if (currentBackendPort !== ecosystemPort) {
          issues.push({
            ...
            severity: 'ERROR',
            fixAction: 'Update PORT environment variable or ecosystem.prod.config.js'
          });
        }
      }
    }
    ...
    const frontendConfigPath = join(process.cwd(), '..', 'frontend', 'src', 'lib', 'frontendConfig.ts');
    if (existsSync(frontendConfigPath)) {
      const frontendConfig = readFileSync(frontendConfigPath, 'utf-8');
      const backendPortMatch = frontendConfig.match(/backendPort:\s*parseInt.*?(\d+)/);
      if (backendPortMatch) {
        ...
        if (frontendExpectedBackendPort !== currentBackendPort) {
          issues.push({
            ...
            severity: 'CRITICAL',
            fixAction: 'Update frontendConfig.ts to match backend port configuration'
          });
        }
      }
    }
    ...
    for (const varName of requiredProdVars) {
      ...
      if (varName === 'ZERO_OPENAI_MODE' && value !== 'true') {
        issues.push({
          ...
          message: 'ZERO_OPENAI_MODE should be true to prevent OpenAI dependency',
          severity: 'WARNING',
  ...
  try {
    ...
    if (existsSync(frontendConfigPath)) {
      ...
      if (urlMatches) {
        ...
        const unexpectedUrls = urlMatches.filter(url => 
 ...
 */
export async function validateConfigDriftQuick(): Promise<{ valid: boolean, criticalCount: number, errorCount: number }> {
