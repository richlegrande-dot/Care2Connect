...
describe('Phase 2: Performance Benchmarks', () => {
  ...
  describe('Memory Efficiency Tests', () => {
    test('should not leak memory with repeated operations', () => {
      ...
      console.log(`Memory usage increase: ${memoryIncreaseMB.toFixed(2)}MB for 5000 operations`);
    });
  });

  describe('Cache Effectiveness Tests', () => {
    test('should benefit from text normalization cache', () => {
      const sameTranscript = "My name is John Smith and I need $5000 for medical bills";
  ...
  describe('Regex Compilation Benefits', () => {
    test('should demonstrate consistent performance across multiple runs', () => {
      ...
      console.log(`Performance consistency: avg=${average.toFixed(1)}ms, stdDev=${stdDev.toFixed(1)}ms, CV=${coefficientOfVariation.toFixed(3)}`);
    });
  });
});
