  ...
  private async checkAssemblyAI(): Promise<void> {
    ...
    try {
      ...
      if (response.ok || response.status === 400) {
        // 400 is acceptable for GET on this endpoint
        ...
        if (response.status === 401) errorCategory = "Invalid API key";
          ...
          errorCategory = "AssemblyAI server error";
        ...
        await createOrUpdateIncident(
          ...
          response.status === 401
            ? "Check ASSEMBLYAI_API_KEY validity"
            : "Monitor AssemblyAI status",
        );
  ...
  private async checkTunnel(): Promise<void> {
    ...
    try {
      ...
      if (isDnsError) {
        ...
        suggestion =
          "DNS may be propagating - this is normal after DNS changes";
      } else if (isTimeoutError) {
        errorType = "Timeout";
        suggestion =
          "External connectivity issue - local tunnel may still be working";
      }
