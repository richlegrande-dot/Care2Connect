...
describe('Phase 1: Correctness Upgrades - False Positive Prevention', () => {
  ...
  describe('Name Extraction False Positive Prevention', () => {
    test('should reject common false positives', () => {
      ...
      expect(extractName("My name is thirty and I'm struggling")).toBeUndefined();
      
      // Emotion words should be rejected
      expect(extractName("I'm critical condition and need help")).toBeUndefined();
      expect(extractName("My name is desperate and I need money")).toBeUndefined();
    ...
    test('should accept valid names with high confidence', () => {
      const result = extractNameWithConfidence("My name is John Smith and I need help");
      expect(result.value).toBe("John Smith");
      expect(result.confidence).toBeGreaterThan(0.8); // Multi-token + proper case
      
      const result2 = extractNameWithConfidence("I'm Sarah Johnson from Seattle");
  ...
  describe('Goal Amount False Positive Prevention', () => {
    ...
    test('should require context for amount extraction', () => {
      ...
      expect(extractGoalAmount("Looking for $3000 to help with medical bills")).toBe(3000);
    });
    
    test('should extract amounts with high confidence when context is strong', () => {
      const result = extractGoalAmountWithConfidence("I need to raise $5000 for my surgery");
      expect(result.value).toBe(5000);
      expect(result.confidence).toBeGreaterThan(0.8);
      
      const result2 = extractGoalAmountWithConfidence("My goal is $3000 for housing");
    ...
    test('should handle range expressions', () => {
      const result = extractGoalAmountWithConfidence("I need between $2000 and $3000");
  ...
  describe('Beneficiary Relationship Extraction', () => {
    test('should detect family member relationships', () => {
      expect(extractBeneficiaryRelationship("I need help for my son")).toBe('family_member');
      expect(extractBeneficiaryRelationship("This is for my daughter's surgery")).toBe('family_member');
      expect(extractBeneficiaryRelationship("My kids need help")).toBe('family_member');
      expect(extractBeneficiaryRelationship("I'm helping my mom")).toBe('family_member');
      expect(extractBeneficiaryRelationship("For my father's treatment")).toBe('family_member');
      expect(extractBeneficiaryRelationship("Our family needs assistance")).toBe('family_member');
    });
    
    test('should detect self-identification', () => {
      expect(extractBeneficiaryRelationship("I need help for myself")).toBe('myself');
      expect(extractBeneficiaryRelationship("This is for my own medical bills")).toBe('myself');
    });
    
    test('should default to myself when no relationship detected', () => {
      expect(extractBeneficiaryRelationship("I need help with medical bills")).toBe('myself');
      expect(extractBeneficiaryRelationship("Please help me")).toBe('myself');
      expect(extractBeneficiaryRelationship("Need assistance urgently")).toBe('myself');
    });
  });
  ...
  describe('Edge Cases and Robustness', () => {
    ...
    test('should handle multiple potential matches correctly', () => {
      ...
      const transcript = "My name is John Smith, but people call me Johnny. I need $1500 for rent but $500 would help too.";
    ...
    test('should maintain confidence scoring consistency', () => {
      const nameResult = extractNameWithConfidence("My name is Dr. Sarah Elizabeth Johnson");
      const amountResult = extractGoalAmountWithConfidence("I desperately need $5000 for emergency surgery");
  ...
  describe('Real-world Scenario Validation', () => {
    test('should correctly parse complex medical scenario', () => {
      const transcript = "Hi, my name is Maria Rodriguez and I'm calling from Phoenix. I just got out of the hospital after surgery and the bills are overwhelming. I need to raise about $7500 to cover the medical expenses. This is urgent because I have until next week to make a payment plan.";
    ...
    test('should correctly parse family assistance scenario', () => {
      const transcript = "I'm calling to get help for my daughter. She needs surgery and we don't have insurance. We're looking to raise fifteen thousand dollars for her medical treatment. Please help our family.";
    ...
    test('should reject false positive scenario', () => {
      const transcript = "I'm twenty years old and make $15 an hour at my job. I've been working for 3 months and I'm struggling to pay rent which is $1200 per month.";
