...
class UrgencyEnhancements_v3d {
  ...
  deEscalateUrgency(transcript, category, baseResult) {
    ...
    if (category === 'EDUCATION' && (urgency === 'HIGH' || urgency === 'CRITICAL')) {
      const hasDeadline = /deadline|due (tomorrow|today|this week)|losing (my )?spot|enrollment deadline|registration closes/i.test(transcript);
      const isOngoingEducation = /college|tuition|certification program|training/i.test(transcript);
    ...
    if (category === 'OTHER' && (urgency === 'MEDIUM' || urgency === 'HIGH')) {
      const isVague = /personal situation|hard to explain|just (a )?personal|not (medical|housing|employment)/i.test(transcript);
      const hasExplicitUrgency = /urgent|emergency|right now|immediately|critical|today/i.test(transcript);
    ...
    if (category === 'FAMILY' && (urgency === 'MEDIUM' || urgency === 'HIGH')) {
      const isFamilyEvent = /wedding|ceremony|graduation|family event/i.test(transcript);
      const hasGriefContext = /passed away|died|death|funeral/i.test(transcript);
      
      if (isFamilyEvent && hasGriefContext) {
        urgency = 'LOW';
        confidence = 0.72;
        de_escalated = true;
        reasons.push('v3d: FAMILY grief context → LOW (wedding after loss, not urgent)');
      }
    }
    ...
    if (category === 'HOUSING' && urgency === 'CRITICAL') {
      const isEvictionThreat = /(facing|threatening) eviction|eviction notice/i.test(transcript);
      const isImminent = /eviction (tomorrow|today|this week|in \d+ days)|eviction (on|by) (monday|tuesday|wednesday|thursday|friday|saturday|sunday)/i.test(transcript);
      
      if (isEvictionThreat && !isImminent) {
        urgency = 'HIGH';
        confidence = 0.78;
        de_escalated = true;
        reasons.push('v3d: HOUSING eviction de-esscalation (threat not imminent → HIGH)');
      }
    }
    ...
    if (category === 'HEALTHCARE' && urgency === 'CRITICAL') {
      ...
      const isEmergency = /emergency|tomorrow|today|right now|life.?threatening|critical condition/i.test(transcript);
      const isInjuryWorkRelated = /injured at work|can't work|lost (my )?job/i.test(transcript);
      
      if (hasSurgery && !isEmergency && isInjuryWorkRelated) {
        urgency = 'HIGH';
        confidence = 0.76;
        de_escalated = true;
        reasons.push('v3d: HEALTHCARE surgery de-escalation (needed but not emergency → HIGH)');
      }
    }
    ...
    if (category === 'HEALTHCARE' && (urgency === 'HIGH' || urgency === 'CRITICAL')) {
      const isBillingConcern = /bills (are )?piling up|bills (are )?really piling|outstanding bills|can't pay (the )?bills/i.test(transcript);
      const hasUrgentMedical = /emergency|surgery tomorrow|critical condition|life.?threatening|dying/i.test(transcript);
      
      if (isBillingConcern && !hasUrgentMedical) {
        urgency = 'MEDIUM';
        confidence = 0.74;
        de_escalated = true;
        reasons.push('v3d: HEALTHCARE billing de-escalation (bills piling up, not urgent medical → MEDIUM)');
      }
    }
