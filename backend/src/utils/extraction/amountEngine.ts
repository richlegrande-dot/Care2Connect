...
class ExplicitAmountPass {
    ...
    for (const pattern of this.EXPLICIT_PATTERNS) {
      ...
      for (const match of matches) {
        ...
        if (amount && amount >= 50 && amount <= 100000) {
          ...
          const confidence = this.calculateExplicitConfidence(match[0], lowerText);
  ...
  private calculateExplicitConfidence(matchText: string, fullText: string): number {
...
class ContextualAmountPass {
  ...
  private readonly VAGUE_PATTERNS = [
    { pattern: /(?:a )?couple (?:of )?thousand\s*dollars?/gi, value: 2500, confidence: 0.65 },
    { pattern: /(?:a )?few thousand\s*dollars?/gi, value: 4000, confidence: 0.60 },
    { pattern: /several thousand\s*dollars?/gi, value: 5000, confidence: 0.55 },
    { pattern: /(?:a )?couple (?:of )?hundred\s*dollars?/gi, value: 250, confidence: 0.65 },
    { pattern: /(?:a )?few hundred\s*dollars?/gi, value: 400, confidence: 0.60 },
    { pattern: /several hundred\s*dollars?/gi, value: 600, confidence: 0.55 },
    { pattern: /around (?:a )?thousand\s*dollars?/gi, value: 1000, confidence: 0.70 },
    { pattern: /about (?:a )?thousand\s*dollars?/gi, value: 1000, confidence: 0.70 }
  ];
  ...
  private processContextualMatch(match: RegExpMatchArray): AmountCandidate | null {
    ...
    if (amount1Str && amount2Str) {
      ...
      if (amount1 && amount2) {
        ...
        if (/(?:deposit|down payment).*?(?:plus|and|first month)/i.test(matchText)) {
  ...
  private createCandidate(amount: number, context: string, baseConfidence: number): AmountCandidate | null {
...
class VagueAmountPass {
  private readonly VAGUE_EXPRESSIONS: Record<string, { min: number; max: number; confidence: number }> = {
    // Thousands
  ...
  detect(transcript: string): AmountCandidate[] {
    ...
    for (const [expression, range] of Object.entries(this.VAGUE_EXPRESSIONS)) {
      if (lowerText.includes(expression)) {
        ...
        const midPoint = Math.round((modifiedRange.min + modifiedRange.max) / 2);
...
class AmbiguityRejectionPass {
  ...
  filterCandidates(candidates: AmountCandidate[], transcript: string): AmountCandidate[] {
  ...
  private isValidGoalAmount(candidate: AmountCandidate, transcript: string): boolean {
    const value = candidate.value;
    
    // Extremely low amounts are suspicious unless with strong context
    if (value < 100) {
      const hasStrongContext = /(?:goal|need|raise|asking for|campaign).*?\$?${value}/.test(transcript);
      return hasStrongContext;
    }
    
    // Very high amounts need verification
    if (value > 50000) {
      const hasHighAmountContext = /(?:medical|surgery|hospital|house|mortgage|business)/.test(transcript.toLowerCase());
...
class ValidationPass {
  private readonly NEED_VERBS = [
    'need', 'require', 'asking for', 'looking for', 'hoping for',
    'trying to raise', 'goal', 'target', 'aim', 'want', 'seeking'
  ];

  validateAndSelect(candidates: AmountCandidate[], transcript: string): AmountDetection {
  ...
  private validateCandidate(candidate: AmountCandidate, transcript: string): number {
    const lowerText = transcript.toLowerCase();
    let confidence = candidate.confidence;

    // Look for need verbs near the amount
    const hasNeedContext = this.NEED_VERBS.some(verb => {
      const pattern = new RegExp(`${verb}.*?\\$?${candidate.value}|\\$?${candidate.value}.*?${verb}`, 'i');
    ...
    const hasNegation = /(?:don't need|not asking for|not looking for|don't want).*?\$?${candidate.value}/.test(lowerText);
    if (hasNegation) {
      confidence -= 0.4;
    }

    // Category alignment boost
    const categoryAlignment = this.getCategoryAlignment(candidate.value, lowerText);
  ...
  private determineSource(candidate: AmountCandidate, transcript: string): 'explicit' | 'contextual' | 'vague' | 'inferred' {
    ...
    if (/(?:rent.*months|surgery costs|bills totaling)/i.test(candidate.context)) {
      return 'contextual';
    }
  ...
  private generateReasons(candidate: AmountCandidate, transcript: string): string[] {
...
export class AmountDetectionEngine {
   ...
   */
  detectGoalAmount(transcript: string, context?: AmountContext): AmountDetection {
    try {
      ...
      // Pass 4: Ambiguity rejection
      const validCandidates = this.rejectionPass.filterCandidates(allCandidates, transcript);
      
      // Pass 5: Validation and selection
      const result = this.validationPass.validateAndSelect(validCandidates, transcript);
   ...
   */
  private applyContextModifiers(result: AmountDetection, context?: AmountContext): AmountDetection {
