...
export function resolveCreateTransport(): CreateTransportFn {
  ...
  let createTransport = nm.createTransport ?? nm.default?.createTransport ?? nm.__createTransportMock;
  ...
  if (typeof createTransport !== 'function' && typeof nm.__sendMailMock === 'function') {
    createTransport = () => ({ sendMail: nm.__sendMailMock });
  }

  if (typeof createTransport !== 'function') {
    const msg = 'Nodemailer createTransport unavailable (module shape mismatch)';
    // If debugging enabled, write to stderr for visibility
    if (process.env.DEBUG_EMAIL === 'true') {
      try { fs.writeFileSync('/dev/stderr', msg + '\n'); } catch (e) {}
      // fallback console
