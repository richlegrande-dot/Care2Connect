...
async function run() {
  ...
  console.log(
    "Starting `stripe listen --forward-to http://localhost:3001/api/payments/stripe-webhook`",
  );
  const proc = spawn(
    "stripe",
    [
  ...
  proc.stdout.on("data", (chunk) => {
    ...
    const m = s.match(
      /Your webhook signing secret is:\s*(whsec_[0-9a-zA-Z_\-]+)/i,
    );
    if (m && m[1]) {
      console.log("\nCaptured webhook secret:", m[1]);
      (async () => {
        const ok = await promptYesNo(
          "Write STRIPE_WEBHOOK_SECRET to backend/.env now?",
        );
        if (ok) {
          const wrote = writeEnvLocal("STRIPE_WEBHOOK_SECRET", m[1]);
          console.log(
            wrote ? "Updated backend/.env" : "Could not update backend/.env",
          );
          process.exit(0);
        } else {
