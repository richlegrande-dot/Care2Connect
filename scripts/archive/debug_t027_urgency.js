// Debug T027 urgency assessment
const transcript = "My name is Robert Chen and I need help finishing my certification program. I lost my job and need this training to find work again. The program costs twenty-eight hundred dollars and I start next month.";
...
function assessUrgencyFallback(transcript) {
  ...
  console.log(`Testing T027 urgency assessment for: ${lower}`);
  ...
  if (/facing.*eviction|about.*to.*be.*evicted|eviction.*notice/i.test(lower) ||
      ...
      // Job loss - only HIGH if urgent, not basic expenses
    ...
    if (/lost.*my.*job|laid.*off|unemployed|jobless/i.test(lower)) {
      ...
      if (!/basic.*living.*expenses|next.*few.*months|couple.*thousand.*get.*through/i.test(lower)) {
        console.log('  → No basic expenses exclusion found - marking HIGH');
      } else {
        console.log('  → Basic expenses exclusion found - should not be HIGH');
      }
    }
