...
class HealthCheckScheduler {
  ...
  private readonly AUTO_RECOVERY_ENABLED =
    process.env.AUTO_RECOVERY_ENABLED !== "false"; // Enabled by default
  ...
  start(): void {
    ...
    console.log(
      `[HEALTH SCHEDULER] Starting health checks every ${config.HEALTHCHECKS_INTERVAL_SEC} seconds`,
    );
  ...
  async runCheck(): Promise<HealthStatus> {
    try {
      ...
      console.log(
        `[HEALTH SCHEDULER] Check completed: ${healthyCount}/${totalCount} services healthy. Overall: ${status.overall ? "HEALTHY" : "DEGRADED"}`,
      );
      ...
      if (failed.length > 0) {
        const failedNames = failed
          .map((s) => `${s.service}(${s.error || "unknown"})`)
          .join(", ");
  ...
  private async attemptAutoRecovery(failedServices: any[]): Promise<void> {
    ...
    if (this.lastRecoveryAttempt) {
      const timeSinceLastAttempt =
        now.getTime() - this.lastRecoveryAttempt.getTime();
      if (timeSinceLastAttempt < this.RECOVERY_COOLDOWN_MS) {
        const remainingSeconds = Math.ceil(
          (this.RECOVERY_COOLDOWN_MS - timeSinceLastAttempt) / 1000,
        );
        console.log(
          `[AUTO-RECOVERY] Cooldown active. Next attempt in ${remainingSeconds}s`,
        );
        return;
      }
    }
    ...
    const failedCritical = failedServices.filter((s) =>
    ...
    console.log(
      `[AUTO-RECOVERY] 🔄 Degraded status detected (${this.consecutiveDegradedChecks} consecutive). Attempting automatic recovery...`,
    );
    ...
    try {
      ...
      if (result.attempted) {
        ...
        if (successCount === totalCount) {
          console.log(
            `[AUTO-RECOVERY] ✅ All services recovered successfully (${successCount}/${totalCount})`,
          );
          this.consecutiveDegradedChecks = 0; // Reset counter on successful recovery
        } else if (successCount > 0) {
          console.log(
            `[AUTO-RECOVERY] ⚠️  Partial recovery (${successCount}/${totalCount} services recovered)`,
          );
        } else {
          console.warn(
            `[AUTO-RECOVERY] ❌ Recovery failed for all services (${totalCount})`,
          );
        }
        ...
        result.results.forEach((r) => {
          ...
          console.log(
            `[AUTO-RECOVERY]   ${status} ${r.service}${r.error ? `: ${r.error}` : ""}`,
          );
        });
      } else {
      ...
      console.error(
        "[AUTO-RECOVERY] Failed to trigger recovery:",
        error?.message || error,
      );
    }
  }
