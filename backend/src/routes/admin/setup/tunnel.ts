...
async function checkCloudflared(): Promise<{ installed: boolean; version?: string | null; path?: string; error?: string; hasBuildInfo?: boolean }> {
  return new Promise((resolve) => {
    ...
    exec('cloudflared --version', (error, stdout, stderr) => {
      ...
      if (!error && stdout) {
        ...
        const hasBuildInfo = stdout.includes('built') || stdout.includes('commit');
        
        // Try different version patterns
        let versionMatch = stdout.match(/cloudflared version v?(\d+\.\d+\.\d+[^\s]*)/);
        if (versionMatch) {
          version = versionMatch[1];
        } else {
          // Try alternative format "Version: X.X.X"
          const altMatch = stdout.match(/Version:\s*v?(\d+\.\d+\.\d+[\w\-\.]*)/i);
          if (altMatch) {
  ...
  try {
    ...
    if (cloudflaredCheck.installed && cloudflaredCheck.version) {
      ...
      if (cloudflaredCheck.version === 'unknown') {
        ...
        const isVeryOutdated = versionParts[0] < latestParts[0] || 
                               (versionParts[0] === latestParts[0] && versionParts[1] < latestParts[1] - 2);
        
        warnings.push(`Cloudflared version ${cloudflaredCheck.version} is outdated version (recommended: ${latestVersion}+)`);
        
        if (isVeryOutdated) {
          recommendations.push('critical upgrade required - upgrade cloudflared immediately using: winget upgrade cloudflare.cloudflared');
        } else {
          recommendations.push('upgrade cloudflared using: winget upgrade cloudflare.cloudflared');
        }
        
        // Add Windows service recommendation for upgrades
        if (process.platform === 'win32') {
          recommendations.push('Windows service: Restart cloudflared service after upgrade (service restart required)');
        }
      } else {
        ...
        if (process.platform === 'win32' && !cloudflaredCheck.hasBuildInfo) {
          recommendations.push('Windows service: Configure cloudflared to run as a service for production');
          recommendations.push('Setup monitoring for tunnel health and connectivity');
        }
      }
    } else {
      ...
      if (cloudflaredCheck.error) {
        if (cloudflaredCheck.error.toLowerCase().includes('timeout')) {
          warnings.push(`Command execution timeout - cloudflared check timed out`);
        } else if (cloudflaredCheck.error.toLowerCase().includes('permission') || cloudflaredCheck.error.toLowerCase().includes('access')) {
          warnings.push(`permission denied - cannot execute cloudflared command`);
        } else {
      ...
      recommendations.push('Install cloudflared using: winget install cloudflare.cloudflared');
      
      // Add installation guidance
      if (process.platform === 'win32') {
        recommendations.push('Configure cloudflared as a Windows service after installation');
        recommendations.push('See CLOUDFLARED_WINDOWS_PRODUCTION.md for setup instructions');
      }
    }
