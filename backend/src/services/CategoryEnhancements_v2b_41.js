...
class CategoryEnhancements_v2b_41 {
  ...
  enhanceCategoryIntelligence(
    ...
    try {
      ...
      if (confidence < 0.7) {
        ...
        const disambiguationResult = this.performTargetedDisambiguation(
          fullText,
          baseCategory,
          v2aCategory,
        );
        ...
        if (
          disambiguationResult.category &&
          disambiguationResult.confidence > finalConfidence
        ) {
          finalCategory = disambiguationResult.category;
          finalConfidence = disambiguationResult.confidence;
          enhancements.push(
            `targeted_disambiguation(${disambiguationResult.reason})`,
          );
        }

        // EDGE CASE HANDLING: Address specific problematic category boundaries
        const edgeCaseResult = this.handleCategoryEdgeCases(
          fullText,
          finalCategory,
          finalConfidence,
        );
        ...
        if (
          edgeCaseResult.category &&
          edgeCaseResult.confidence > finalConfidence
        ) {
        ...
        const clarificationResult = this.clarifyContextualCategory(
          fullText,
          finalCategory,
          finalConfidence,
        );
        ...
        if (
          clarificationResult.category &&
          clarificationResult.confidence > finalConfidence
        ) {
          finalCategory = clarificationResult.category;
          finalConfidence = clarificationResult.confidence;
          enhancements.push(
            `context_clarification(${clarificationResult.reason})`,
          );
        }
      }
      ...
      if (this.debugMode && enhancements.length > 0) {
        console.log(
          `[V2b_4.1 Category] Story: ${story.id || "unknown"} | Base: ${baseCategory} | V2a: ${v2aCategory}(${confidence}) | Final: ${finalCategory}(${finalConfidence}) | Applied: ${enhancements.join(", ")}`,
        );
      }
      ...
      console.error(
        "[CategoryEnhancements_v2b_41] Error in enhanceCategoryIntelligence:",
        error,
      );
      return {
        originalCategory: baseCategory,
  ...
  performTargetedDisambiguation(text, baseCategory, v2aCategory) {
    ...
    if (
      ...
      v2aCategory === "TRANSPORTATION" ||
      baseCategory === "EMPLOYMENT" ||
      v2aCategory === "EMPLOYMENT"
    ) {
      // Clear employment indicators
      if (
        /job.*interview|work.*start|employment.*offer|position.*available/i.test(
          text,
        )
      ) {
        return {
          category: "EMPLOYMENT",
          confidence: 0.85,
          reason: "job_opportunity_context",
        };
      }

      // Clear transportation indicators
      if (
        /car.*broke|bus.*pass|transportation.*to.*work|ride.*needed/i.test(text)
      ) {
        return {
          category: "TRANSPORTATION",
          confidence: 0.85,
          reason: "transport_need_context",
        };
      }
    }
    ...
    if (
      ...
      if (/domestic.*violence|abuse|threat|danger|unsafe.*home/i.test(text)) {
        return {
          category: "SAFETY",
          confidence: 0.85,
          reason: "safety_threat_context",
        };
      }

      // Clear healthcare indicators
      if (
        /medication|doctor.*appointment|surgery|hospital|medical.*bill/i.test(
          text,
        )
      ) {
        return {
    ...
    if (
      ...
      v2aCategory === "FINANCIAL" ||
      baseCategory === "UTILITIES" ||
      v2aCategory === "UTILITIES"
    ) {
      // Clear utilities indicators
      if (
        /electric.*bill|gas.*shutoff|water.*disconnect|utility.*payment/i.test(
          text,
        )
      ) {
        return {
          category: "UTILITIES",
          confidence: 0.85,
          reason: "utility_specific_context",
        };
      }

      // Clear financial indicators (when not utilities)
      if (
        /debt|loan|credit|bankruptcy|financial.*crisis/i.test(text) &&
        !/utility|electric|gas|water/i.test(text)
      ) {
        return {
          category: "FINANCIAL",
          confidence: 0.8,
          reason: "general_financial_context",
        };
      }
    }
  ...
  handleCategoryEdgeCases(text, currentCategory, currentConfidence) {
    ...
    if (
      ...
      /pet.*food|dog.*food|cat.*food|animal.*feed/i.test(text)
    ) {
      return {
        category: "FOOD",
        confidence: 0.75,
        reason: "pet_food_edge_case",
      };
    }
    ...
    if (
      currentCategory === "OTHER" &&
      /hotel.*stay|motel.*room|temporary.*housing|shelter.*need/i.test(text)
    ) {
      return {
        category: "HOUSING",
        confidence: 0.8,
        reason: "temporary_housing_edge_case",
      };
    }
    ...
    if (
      currentCategory === "OTHER" &&
      /school.*supplies|textbook|educational.*material|study.*guide/i.test(text)
    ) {
      return {
        category: "EDUCATION",
        confidence: 0.75,
        reason: "educational_materials_edge_case",
      };
    }
    ...
    if (
      ...
      /gas.*money|fuel.*cost|gasoline.*expense/i.test(text)
    ) {
      return {
        category: "TRANSPORTATION",
        confidence: 0.75,
        reason: "fuel_transportation_edge_case",
      };
    }
    ...
    if (
      currentCategory === "OTHER" &&
      /therapy|counseling|mental.*health|depression|anxiety.*treatment/i.test(
        text,
      )
    ) {
      return {
        category: "HEALTHCARE",
        confidence: 0.8,
        reason: "mental_health_edge_case",
      };
    }
  ...
  clarifyContextualCategory(text, currentCategory, currentConfidence) {
    ...
    if (/for.*work|job.*related|work.*expense|employment.*need/i.test(text)) {
      ...
      if (
        /car|transport|ride|bus|gas/i.test(text) &&
        currentCategory !== "TRANSPORTATION"
      ) {
        return {
          category: "TRANSPORTATION",
          confidence: 0.8,
          reason: "work_transport_context",
        };
      }

      // Clothing for work
      if (
        /clothes|uniform|professional.*attire/i.test(text) &&
        currentCategory === "OTHER"
      ) {
        return {
          category: "EMPLOYMENT",
          confidence: 0.75,
          reason: "work_clothing_context",
        };
      }
    }
    ...
    if (/children|kids|family|child/i.test(text)) {
      ...
      if (
        /sick|doctor|medicine|hospital/i.test(text) &&
        currentCategory !== "HEALTHCARE"
      ) {
        return {
          category: "HEALTHCARE",
          confidence: 0.8,
          reason: "child_medical_context",
        };
      }

      // Children's educational needs
      if (
        /school|education|supplies/i.test(text) &&
        currentCategory !== "EDUCATION"
      ) {
        return {
          category: "EDUCATION",
          confidence: 0.8,
          reason: "child_education_context",
        };
      }
    }
    ...
    if (/emergency|urgent|crisis|immediate/i.test(text)) {
      // Emergency housing
      if (
        /homeless|evict|shelter/i.test(text) &&
        currentCategory !== "HOUSING"
      ) {
        return {
          category: "HOUSING",
          confidence: 0.85,
          reason: "emergency_housing_context",
        };
      }

      // Emergency utilities
      if (
        /shutoff|disconnect|power|heat/i.test(text) &&
        currentCategory !== "UTILITIES"
      ) {
        return {
          category: "UTILITIES",
          confidence: 0.85,
          reason: "emergency_utility_context",
        };
      }
    }
