...
function validateFrontendConfig(config) {
  ...
  if (
    isNaN(config.frontendPort) ||
    config.frontendPort < 1024 ||
    config.frontendPort > 65535
  ) {
    errors.push(
      `Invalid frontend port: ${process.env.PORT || process.env.FRONTEND_PORT || "3000"}`,
    );
  }

  if (
    isNaN(config.backendPort) ||
    config.backendPort < 1024 ||
  ...
  if (config.apiUrl !== expectedApiUrl) {
    errors.push(
      `API URL mismatch: expected "${expectedApiUrl}", got "${config.apiUrl}"`,
    );
  }
  ...
  if (config.frontendPort === config.backendPort) {
    errors.push(
      `Frontend and backend cannot use the same port: ${config.frontendPort}`,
    );
  }
...
async function validateFrontendStartup() {
  ...
  if (configErrors.length > 0) {
    ...
    console.error("\n📋 ACTIONS REQUIRED:");
    console.error(
      "   • Check environment variables: PORT, BACKEND_PORT, NEXT_PUBLIC_API_URL",
    );
  ...
  if (portOccupant !== null) {
    ...
    console.error(
      `   • Check what's using port: netstat -ano | findstr :${config.frontendPort}`,
    );
    ...
    if (config.strictMode) {
      ...
      console.warn(
        "⚠️  Port conflict detected but Next.js may handle it automatically.",
      );
    }
  } else {
    console.log(
      `✅ [FRONTEND HARDENING] Frontend port ${config.frontendPort} is available`,
    );
  }
