 ...
 */
async function checkDatabaseHealth(): Promise<{ ready: boolean; failureCount: number }> {
  try {
...
async function testStartupGateWithInvalidDB(): Promise<TestResult> {
  ...
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      ...
      const hasConnectionFailed = output.includes('Connection failed') || output.includes('ECONNREFUSED');
...
async function testRuntimeWatchdogDetection(): Promise<TestResult> {
  ...
  try {
    ...
    console.log('   ⚠️ NOTE: Full watchdog test requires simulating DB failure');
    console.log('   ⚠️ This would require stopping the database container/process');
...
async function testHealthDbEndpoint(): Promise<TestResult> {
  ...
  try {
    ...
    const hasRequiredFields = typeof health.ready === 'boolean' && 
...
async function testServiceUnavailableResponse(): Promise<TestResult> {
  ...
  try {
    ...
    if (health.ready) {
      ...
      return {
        ...
        details: {
          ...
          expectedBehavior: 'All non-health endpoints should return 503 when dbReady=false',
        },
      };
    }
    ...
    try {
      await axios.get(`${BACKEND_URL}/api/tickets/search?contact=test@example.com`, {
        timeout: 2000,
      });
