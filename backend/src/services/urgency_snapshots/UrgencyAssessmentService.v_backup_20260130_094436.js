...
try {
  ...
  console.warn(
    "UrgencyAssessmentEngine not found, falling back to basic assessment",
  );
}
...
class UrgencyAssessmentService {
  ...
  assessUrgency(transcript, context = {}) {
    console.log("UrgencyAssessmentService.assessUrgency called with:", {
    ...
    if (this.engine) {
      try {
            ...
            case "FAMILY":
              ...
              if (
                ...
                (transcript.toLowerCase().includes("urgent") ||
                  transcript.toLowerCase().includes("immediately"))
              ) {
                boostedScore = Math.max(boostedScore, 0.6); // Escalate to solid HIGH with explicit urgency
        ...
        if (boostedScore >= 0.7) boostedUrgencyLevel = "CRITICAL";
        else if (boostedScore >= 0.38)
          boostedUrgencyLevel = "HIGH"; // Lowered from 0.42 to reduce under-assessment (65 cases)
        else if (boostedScore >= 0.13)
          boostedUrgencyLevel = "MEDIUM"; // Lowered from 0.15 for better MEDIUM coverage
        else boostedUrgencyLevel = "LOW";

        console.log("UrgencyAssessmentService final result:", {
          boostedUrgencyLevel,
          boostedScore,
          boostedReasons,
        });
        ...
        console.warn(
          "UrgencyAssessmentEngine failed, using fallback:",
          error.message,
        );
      }
    }
  ...
  fallbackAssessment(transcript, context = {}) {
    ...
    const patternChecks = [
      ...
      {
        level: "HIGH",
        patterns: this.fallbackPatterns.high,
        baseWeight: 0.65,
        increment: 0.03,
      },
      {
        level: "MEDIUM",
        patterns: this.fallbackPatterns.medium,
        baseWeight: 0.35,
        increment: 0.02,
      },
      {
        level: "LOW",
        patterns: this.fallbackPatterns.low,
        baseWeight: 0.15,
        increment: 0,
      },
    ];
    ...
    for (const check of patternChecks) {
      ...
      for (const pattern of check.patterns) {
        if (pattern.test(lower)) {
          ...
          reasons.push(
            `${check.level.toLowerCase()}_pattern: ${pattern.toString().slice(1, 30)}...`,
          );
      ...
      if (matchesThisLevel > 0) {
        ...
        const contribution =
          check.baseWeight +
          Math.min(matchesThisLevel - 1, 2) * check.increment;
    ...
    if (/\bsurgery|\boperation|\bsurgical/i.test(lower)) {
      if (/\btomorrow|\btoday|\bnow|\bsoon|\burgent/i.test(lower)) {
        score = Math.max(score, 0.9); // Surgery tomorrow = CRITICAL
        reasons.push("contextual_critical: surgery tomorrow");
      } else {
        score = Math.max(score, 0.7); // General surgery = HIGH
    ...
    if (/\btomorrow|\btoday|\bnow|\bthis.*week|\bnext.*week/i.test(lower)) {
      if (score >= 0.3) {
        // Only boost if already has some urgency
    ...
    let urgencyLevel;
    if (score >= 0.75)
      urgencyLevel = "CRITICAL"; // Lowered from 0.8
    else if (score >= 0.5)
      urgencyLevel = "HIGH"; // Lowered from 0.6
    else if (score >= 0.25)
      urgencyLevel = "MEDIUM"; // Lowered from 0.3
