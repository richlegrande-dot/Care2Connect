    ...
    if (options.aggressiveMode || !options.preserveStoryContent) {
      for (const filler of this.CONVERSATIONAL_FILLERS) {
        ...
        if (matches.length > 0) {
          ...
          if (frequency / wordsCount > 0.05) {
            // More than 5% of words
            removedFillers.push(...matches);
            cleaned = cleaned.replace(pattern, " ");
          }
        }
...
class FragmentReconstructor {
  ...
  reconstructFragments(
    ...
    for (let i = 0; i < sentences.length; i++) {
      ...
      const incompleteIntro = this.handleIncompleteIntroduction(
        processed,
        nextSentence,
      );
      ...
      const hangingConjunction = this.handleHangingConjunction(
        processed,
        nextSentence,
      );
...
class IncompletenessAssessor {
  private readonly INCOMPLETENESS_INDICATORS = [
    ...
    // Excessive fillers
    {
      pattern: /\b(?:uh|um|er|ah){3,}/gi,
      weight: 0.2,
      name: "excessive_fillers",
    },
    ...
    {
      pattern: /\b(?:and|but|so|because)\s*$/gi,
      weight: 0.1,
      name: "hanging_conjunction",
    },
    {
      pattern: /^(?:my name is|i'm|i need)\s*$/gim,
      weight: 0.2,
      name: "incomplete_statement",
    },
  ];
...
export class FragmentProcessor {
  ...
  processTranscript(
    ...
    try {
      ...
      const fillerResult = this.fillerProcessor.processFillers(
        transcript,
        options,
      );
      ...
      const reconstructionResult =
        this.fragmentReconstructor.reconstructFragments(
          fillerResult.cleanedText,
          options,
        );

      // Step 3: Assess incompleteness penalty
      const incompletenessPenalty =
        this.incompletenessAssessor.assessIncompleteness(transcript);
  ...
  assessQuality(transcript: string): {
    ...
    try {
      ...
      if (/\b(?:uh|um|er|ah){3,}/gi.test(transcript))
