...
async function checkPortConfiguration(): Promise<ReadinessCheck> {
  try {
    ...
    if (backendPort < 1024 || backendPort > 65535 || frontendPort < 1024 || frontendPort > 65535) {
      return {
        name: 'ports',
        status: 'down',
...
async function checkEnvironmentConfig(): Promise<ReadinessCheck> {
  try {
    ...
    if (envConfig.AI_PROVIDER !== 'rules' && envConfig.AI_PROVIDER !== 'template') {
      issues.push(`AI_PROVIDER should be rules/template, got: ${envConfig.AI_PROVIDER}`);
    }
    
    // Check if keys are validated in production
    if (process.env.NODE_ENV === 'production' && process.env.KEYS_VALIDATED !== 'true') {
...
router.get('/ready', requireAdminAuth, async (req: Request, res: Response) => {
  try {
    ...
    const summary = {
      ...
      critical_failures: checks.filter(c => c.status === 'down' && c.critical).length
    };
    ...
    const httpStatus = overallStatus === 'ready' ? 200 : 
                       overallStatus === 'degraded' ? 200 : 503;
    
    res.status(httpStatus).json(response);
    
    // Log readiness check results
    console.log(`[READINESS] Check completed in ${checkDuration.toFixed(1)}ms - Status: ${overallStatus.toUpperCase()}`);
    if (summary.critical_failures > 0) {
      console.log(`[READINESS] CRITICAL: ${summary.critical_failures} critical system(s) down`);
    }
    ...
    res.status(503).json({
      ...
      uptime: process.uptime(),
      checks: [],
      summary: { total: 0, ready: 0, degraded: 0, down: 1, critical_failures: 1 },
...
router.get('/health/production', async (req: Request, res: Response) => {
  try {
    ...
    const criticalFailures = criticalChecks.filter(c => c.status === 'down' && c.critical);
    ...
    if (criticalFailures.length > 0) {
      ...
      console.log(`[PRODUCTION-READINESS] CRITICAL FAILURE - Duration: ${checkDuration.toFixed(1)}ms`);
      criticalFailures.forEach(failure => {
        console.log(`[PRODUCTION-READINESS] CRITICAL: ${failure.name} - ${failure.message}`);
      });
    ...
    const nonCriticalIssues = criticalChecks.filter(c => c.status !== 'ready');
    ...
    console.log(`[PRODUCTION-READINESS] Ready - Duration: ${checkDuration.toFixed(1)}ms`);
