...
if (ALLOW_INSECURE) {
  console.warn(
    "⚠️  WARNING: Speech Intelligence admin endpoints are UNSECURED (ALLOW_INSECURE_ADMIN_ENDPOINTS=true)",
  );
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const engines = [
        ...
        TranscriptionEngine.NVT,
        TranscriptionEngine.EVTS,
      ];
      ...
      for (let i = 0; i < count; i++) {
        ...
        await sessionManager.updateSession(session.id, {
          ...
          wordCount,
          confidenceScore: success ? 0.85 + Math.random() * 0.1 : null,
        });
