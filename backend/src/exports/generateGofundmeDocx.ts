...
export class GofundmeDocxExporter {
  ...
  static async generateDocument(
    ...
    try {
        ...
        sections: [
          {
            ...
            children: [
              ...
              // Category  
              this.createDetailRow('Category', draft.category?.value || 'General'),
              
              // Goal Amount
              this.createDetailRow('Goal Amount', 
                draft.goalAmount?.value ? `$${draft.goalAmount.value.toLocaleString()}` : 'Not specified'
              ),
              
              // Location
              this.createDetailRow('Location', this.formatLocation(draft.location?.value)),
              
              // Beneficiary
              this.createDetailRow('Beneficiary', this.formatBeneficiary(draft.beneficiary?.value ?? null)),
      ...
      TelemetryCollector.getInstance().recordDocumentMetrics(telemetrySessionId, {
      ...
      try {
        ...
        console.error('[DOCX_GEN_CRITICAL] Even fallback document generation failed:', {
          error: fallbackError.message,
          timestamp: new Date().toISOString()
        });
        
        throw new Error('Document generation completely failed - unable to create even basic document');
      }
    }
  }
  ...
  private static formatLocation(location: any): string {
    ...
    if (location.country && location.country !== 'United States') parts.push(location.country);
