import { extractGoalAmountWithConfidence, EXTRACTION_PATTERNS } from '../../../src/utils/extraction/rulesEngine';
...
describe('Debug amount extraction', () => {
  test('Debug scientific notation', () => {
    ...
    EXTRACTION_PATTERNS.goalAmount.forEach((pattern, index) => {
      ...
      if (match) {
        console.log(`  Match: [${match[0]}] -> captured: [${match[1] || 'undefined'}], [${match[2] || 'undefined'}]`);
      } else {
