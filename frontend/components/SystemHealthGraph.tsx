...
export default function SystemHealthGraph({ token }: SystemHealthGraphProps) {
  ...
  const fetchHistory = async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/health/history?limit=50`, {
        headers: {
  ...
  const chartData = history.slice().reverse().map((snapshot, index) => ({
    index,
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      <h3 className="text-lg font-semibold text-gray-900 mb-4">Health Status Timeline</h3>
      ...
      <div className="mt-4 grid grid-cols-4 gap-4 text-sm">
        <div className="text-center">
          <div className="text-2xl font-bold text-green-600">{chartData.filter(d => d.ready === 1).length}</div>
          <div className="text-gray-600">Ready</div>
        </div>
        <div className="text-center">
          <div className="text-2xl font-bold text-yellow-600">{chartData.filter(d => d.degraded === 1).length}</div>
          <div className="text-gray-600">Degraded</div>
        </div>
        <div className="text-center">
          <div className="text-2xl font-bold text-blue-600">{chartData.filter(d => d.dbOk === 1).length}</div>
          <div className="text-gray-600">DB OK</div>
        </div>
        <div className="text-center">
          <div className="text-2xl font-bold text-purple-600">{chartData.filter(d => d.storageOk === 1).length}</div>
