import { EnhancedExtractor } from "./enhancedExtractorBase";
...
export class EnhancedAmountExtractor extends EnhancedExtractor {
  private readonly vagueAmountMap: { [key: string]: number } = {
    "couple hundred": 250,
    "few hundred": 400,
    "several hundred": 600,
  ...
  extract(transcript: string): AmountResult {
  ...
  private extractByPatterns(transcript: string): AmountCandidate[] {
    const candidates: AmountCandidate[] = [];
    ...
    const patterns = [
      ...
      {
        regex:
          /(?:help|assistance|goal|raise|about)(?:\s+(?:about|around|approximately))?\s+(?:help\s+with\s+)?\$?([0-9,]+)(?:\s+(?:for|to|with|dollars?|bucks|deposit|supplies|repairs|costs?|bills?|fees?|rent|utilities?|groceries|food|medication|treatment|childcare|school|supplies))/gi,
        confidence: 0.9,
      ...
      {
        regex:
          /(?:court|legal|medical|rent|utility|electric|gas|water|phone|internet|insurance|tax|fee|fine|penalty)\s+(?:cost|fee|bill)s?\s+(?:about\s+|around\s+|approximately\s+)?\$?([0-9,]+(?:\.\d{2})?)/gi,
        confidence: 0.85,
      ...
      {
        regex:
          /\$([0-9,]+(?:\.\d{2})?)\s+(?:dollars?|bucks)\s+(?:to|for|that|needed|required|goal)/gi,
        confidence: 0.75,
      ...
      while ((match = pattern.regex.exec(transcript)) !== null) {
        ...
        if (numericValue && this.isValidAmount(numericValue)) {
          ...
          candidates.push({
            ...
            context: {
              before: transcript.substring(
                Math.max(0, position - 50),
                position,
              ),
              after: transcript.substring(
  ...
  private extractByContext(transcript: string): AmountCandidate[] {
    const candidates: AmountCandidate[] = [];
  ...
  private extractVagueAmounts(transcript: string): AmountCandidate[] {
    const candidates: AmountCandidate[] = [];
  ...
  private extractFromRanges(transcript: string): AmountCandidate[] {
    const candidates: AmountCandidate[] = [];

    // Handle ranges like "between 1500 and 2000"
    const rangeRegex =
      /(?:between|from)\s+\$?([0-9,]+(?:\.\d{2})?)\s+(?:and|to)\s+\$?([0-9,]+(?:\.\d{2})?)/gi;
    let match;
  ...
  private rankCandidates(
    ...
    return candidates
      ...
      .map((candidate) => ({
        ...candidate,
        confidence: this.adjustConfidence(candidate, transcript),
      }))
      .sort((a, b) => {
        ...
        return (
          this.getStrategyPriority(b.strategy) -
          this.getStrategyPriority(a.strategy)
        );
      });
  }
