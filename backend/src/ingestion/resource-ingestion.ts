...
export class ResourceIngestionEngine {
  ...
  async ingestFromSource(source: DataSource): Promise<RawResourceRecord[]> {
    ...
    try {
      ...
      logger.info(
        `Successfully ingested ${records.length} records from ${source.name}`,
      );
      return records;
    } catch (error) {
  ...
  private normalizeAPIResponse(
    ...
    for (const item of items) {
      const record: RawResourceRecord = {
        ...
        city: source.city,
        state: source.state,
        zipCode: this.extractField(
          item,
          "zip_code",
          source.extractionConfig.customMappings,
        ),
        county: this.extractField(
          item,
          "county",
          source.extractionConfig.customMappings,
        ),
  ...
  private async saveRawRecords(
    ...
    for (let i = 0; i < records.length; i += batchSize) {
      ...
      try {
        ...
        logger.debug(
          `Saved batch ${Math.floor(i / batchSize) + 1} of ${Math.ceil(records.length / batchSize)} for ${source.name}`,
        );
      } catch (error) {
