...
describe("Telemetry Privacy Guards", () => {
  ...
  test("PRIVACY GUARD: No full name values in telemetry output", () => {
    const result = extractAllWithTelemetry(
      "My name is Sarah Johnson and I need $2000 for rent",
    );
  ...
  test("PRIVACY GUARD: Fail test if PII patterns are detected", () => {
    ...
    piiPatterns.forEach((pattern, index) => {
      ...
      if (matches) {
        console.error(
          `PII DETECTED in telemetry output! Pattern ${index + 1}: ${pattern}`,
        );
        console.error(`Matching text: ${matches[0]}`);
        console.error(
          "Full telemetry output:",
          allTelemetryOutput.substring(0, 1000),
        );
      }
