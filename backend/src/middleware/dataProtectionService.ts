...
export class DataProtectionService {
  ...
  static sanitizeRequest() {
    return (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (req.query) {
          const sanitizedQuery = DataProtectionService.sanitizeObject(req.query);
  ...
  static sanitizeObject(obj: any): any {
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      const isPII = PII_FIELD_PATTERNS.some(pattern => 
        pattern.test(key)
      );
      ...
      if (isPII) {
        ...
        if (!consentGiven && key !== 'consentToPublish' && key !== 'consentGiven') {
  ...
  static containsPII(obj: any): boolean {
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (typeof value === 'string') {
        ...
        // Address patterns
        const addressRegex = /\b\d+\s+[A-Za-z\s]+(street|st|avenue|ave|road|rd|drive|dr|lane|ln)\b/i;
        // Phone patterns
        const phoneRegex = /\b\(?\d{3}\)?[\s.-]?\d{3}[\s.-]?\d{4}\b/;

        if (ssnRegex.test(value) || bankAcctRegex.test(value) || ccRegex.test(value) ||
            nameRegex.test(value) || addressRegex.test(value) || phoneRegex.test(value)) {
          return true;
        }
      }
  ...
  static sanitizeForStorage(data: any): any {
    ...
    if (sanitized.transcript) {
      ...
      sanitized.transcript = DataProtectionService.sanitizeTranscript(sanitized.transcript);
    }
  ...
  static auditDataAccess() {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      console.log(`DATA_ACCESS: ${timestamp} - IP: ${ip} - Path: ${req.path} - Method: ${req.method}`);
