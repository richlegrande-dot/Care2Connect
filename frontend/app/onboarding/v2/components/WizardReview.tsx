...
export function WizardReview({
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8" role="main" aria-label="Review Your Responses">
      <div className="max-w-2xl mx-auto px-4">
        {/* Header */}
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          <h1 className="text-2xl font-bold text-gray-900 mb-2">Review Your Responses</h1>
          <p className="text-gray-600">
            Please review your answers below. Click <strong>Edit</strong> on any section to make changes,
            ...
            >
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gray-50">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="font-semibold text-gray-900">{moduleLabels[mod.id]}</h2>
                </div>
                <button
                  type="button"
              ...
              <div className="px-6 py-4">
                {!hasData ? (
                  <p className="text-sm text-gray-400 italic">
                    {mod.required ? 'No data entered — please fill in this section.' : 'Skipped (optional)'}
                  </p>
                ) : (
                  <dl className="space-y-3">
                    {Object.entries(data!).map(([fieldKey, fieldValue]) => {
                      const fieldSchema = mod.schema.properties[fieldKey];
                      const label = fieldSchema?.title || fieldKey.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
                      const displayValue = formatFieldValue(fieldValue, fieldSchema);

                      // Don't display undefined/null values
                      if (fieldValue === undefined || fieldValue === null) return null;

                      return (
                        <div key={fieldKey} className="flex flex-col sm:flex-row sm:gap-4">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-sm text-gray-500">
            When you're satisfied with your answers, click Submit to complete your intake.
          </p>
          <button
            type="button"
            onClick={onSubmit}
 ...
 */
function formatFieldValue(value: unknown, schema?: { type?: string; enum?: string[] }): string {
