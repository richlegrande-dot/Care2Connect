...
describe('RecordingInterface Component', () => {
  ...
  beforeEach(() => {
    ...
    (navigator.mediaDevices.getUserMedia as jest.Mock).mockImplementation(() => Promise.resolve({
      getTracks: () => [{ stop: jest.fn(), kind: 'audio', label: 'Mock Audio Track' }]
    } as MediaStream));
  });
  ...
  describe('Stop Recording and Processing', () => {
    it('should stop recording and process audio', async () => {
      ...
      mockFetch.mockResolvedValue({
        ...
        json: () => Promise.resolve({
          success: true,
          transcript: 'Hello, this is my story about needing help with housing.',
    ...
    it('should show transcription after processing', async () => {
      ...
      const mockTranscript = 'I need help with medical expenses for my cancer treatment.';
      mockFetch.mockResolvedValue({
        ok: true,
        json: () => Promise.resolve({
          success: true,
  ...
  describe('Manual Input Mode', () => {
    ...
    it('should handle manual text input', async () => {
      ...
      // Submit the text
      const submitButton = screen.getByRole('button', { name: /analyze story/i });
      await user.click(submitButton);
      
      await waitFor(() => {
        expect(mockFetch).toHaveBeenCalledWith('/api/transcription', expect.objectContaining({
          method: 'POST',
