...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (
      ...
      file.originalname.match(/\.(mp3|wav|webm|ogg|m4a|mp4)$/i)
    ) {
      cb(null, true);
    } else {
    ...
    try {
      ...
      try {
        ...
        transcriptionResult.transcript =
          DataProtectionService.sanitizeTranscript(
            transcriptionResult.transcript,
          );

        // Extract structured data
        const extractionResult = await extractionService.extractGoFundMeData(
          transcriptionResult.transcript,
        );
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const transcriptionResult =
        await transcriptionService.processManualTranscript(transcript);
      // Sanitize transcript content to redact sensitive data
      transcriptionResult.transcript = DataProtectionService.sanitizeTranscript(
        transcriptionResult.transcript,
      );
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const question = followUpService.getCurrentQuestion(
        sessionId,
        parsedDraft,
      );
      ...
      const progress = followUpService.getSessionProgress(
        sessionId,
        parsedDraft.followUpQuestions?.length || 0,
      );
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const updatedDraft = followUpService.mergeAnswersIntoDraft(
        draft,
        answers,
      );
