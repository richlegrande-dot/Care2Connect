...
router.get("/stripe-webhook/health", async (req: Request, res: Response) => {
  try {
    ...
    const lastSuccess =
      webhookState.lastWebhookReceivedAt && !webhookState.lastWebhookError;

    const healthStatus =
      dbHealthy && stripeConfigured && recentErrors === 0
        ? "healthy"
        : "degraded";
  ...
  async (req: Request, res: Response) => {
    ...
    if (!stripe) {
      ...
      return res.status(503).json({
        ...
        message:
          "STRIPE_SECRET_KEY or STRIPE_WEBHOOK_SECRET missing - cannot process webhook",
    ...
    if (process.env.STRIPE_WEBHOOK_SECRET) {
      try {
        ...
        updateWebhookReceived(
          (req.body && req.body.type) || "unknown",
          false,
          err.message,
        );
      ...
      switch (event.type) {
        case "checkout.session.completed":
          ...
          if (session.metadata?.ticketId) {
            console.log(
              `[Phase 6] Processing donation for ticket: ${session.metadata.ticketId}`,
            );
            try {
              await StripeService.handleCheckoutCompleted(session);
            } catch (checkoutError: any) {
              console.error(
                `[Webhook] Phase 6 checkout completion failed for ticket ${session.metadata.ticketId}:`,
                checkoutError,
              );
            ...
            try {
              ...
              console.error(
                `[Webhook] Legacy checkout completion failed for session ${session.id}:`,
                legacyError,
              );
        ...
        case "checkout.session.expired":
          ...
          if (expiredSession.metadata?.ticketId) {
            try {
              ...
              console.error(
                `[Webhook] Checkout expiration handling failed for ticket ${expiredSession.metadata.ticketId}:`,
                expireError,
              );
        ...
        case "payment_intent.succeeded":
          ...
          try {
            ...
            if (attribution) {
              console.log(
                `[Webhook] Updating attribution ${attribution.id} with payment details`,
              );
              ...
              const donorLastName = extractDonorLastName(
                chargeDetails.billing_details?.name,
              );
              const donorCountry =
                chargeDetails.billing_details?.address?.country || null;
              const donorEmailHash = hashEmail(
              ...
              console.log(
                `[Webhook] ✅ Attribution updated: ${donorLastName ? `Donor: ${donorLastName}` : "Anonymous"}`,
              );
            }
          } catch (paymentError: any) {
            console.error(
              `[Webhook] Payment intent processing failed for ${paymentIntent.id}:`,
              paymentError,
            );
        ...
        case "payment_intent.payment_failed":
          ...
          try {
            ...
            console.error(
              `[Webhook] Payment failure processing failed for ${failedPayment.id}:`,
              failError,
            );
        ...
        case "charge.refunded":
          ...
          try {
            ...
            const refundedAttribution =
              await prisma.stripeAttribution.findFirst({
            ...
            if (refundedAttribution) {
              ...
              console.log(
                `[Webhook] ✅ Attribution ${refundedAttribution.id} marked as refunded`,
              );
            }
          } catch (refundError: any) {
            console.error(
              `[Webhook] Refund processing failed for charge ${charge.id}:`,
              refundError,
            );
        ...
        case "charge.dispute.created":
          ...
          try {
            ...
            console.error(
              `[Webhook] Dispute processing failed for charge ${dispute.charge}:`,
              disputeError,
            );
