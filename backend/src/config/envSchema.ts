...
export function validateEnvironment(): ValidationResult {
  ...
  optionalVars.forEach((varName) => {
    if (!process.env[varName]) {
      warnings.push(
        `${varName} not set - service will be marked as unavailable`,
      );
    }
  });
  ...
  const config: Partial<EnvConfig> = {
    ...
    HEALTHCHECKS_INTERVAL_SEC: parseInt(
      process.env.HEALTHCHECKS_INTERVAL_SEC || "300",
      10,
    ),
...
export function logEnvironmentStatus(): void {
  ...
  console.log(
    `  STRIPE_SECRET_KEY: ${maskSecret(result.config.STRIPE_SECRET_KEY)}`,
  );
  console.log(
    `  CLOUDFLARE_API_TOKEN: ${maskSecret(result.config.CLOUDFLARE_API_TOKEN)}`,
  );
