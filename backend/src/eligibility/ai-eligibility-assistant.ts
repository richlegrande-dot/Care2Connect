...
export class AIEligibilityAssistant {
  ...
  async analyzeEligibility(
    ...
    try {
      ...
      const userSituation = this.formatUserSituation(questionnaire, userContext);
      ...
      const programDetails = await this.getProgramDetails(rulesEngineResult.programResults);
      
      // Call OpenAI for analysis
      const analysis = await this.callAnalysisAPI(userSituation, rulesAssessment, programDetails);
      ...
      await this.enhanceWithLocalResources(analysis, questionnaire.state, questionnaire.zipCode);
      
      // Add standardized disclaimer
      analysis.disclaimer = this.generateDisclaimer();
      
      logger.info(`AI eligibility analysis completed: ${analysis.overallAssessment.eligibilityLabel}`);
  ...
  private formatUserSituation(
    ...
    if (questionnaire.hasChildren) {
      situation.push(`Has children (ages: ${questionnaire.childrenAges?.join(', ') || 'not specified'})`);
    }
    ...
    if (questionnaire.incomeSource.length > 0) {
      situation.push(`Income sources: ${questionnaire.incomeSource.join(', ')}`);
    }
    ...
    if (questionnaire.fleeDomesticViolence) situation.push('Fleeing domestic violence');
    
    // Location
    situation.push(`Location: ${questionnaire.state}${questionnaire.county ? ', ' + questionnaire.county + ' County' : ''}`);
    
    // Current benefits
    if (questionnaire.currentlyReceivingBenefits?.length) {
      situation.push(`Currently receiving: ${questionnaire.currentlyReceivingBenefits.join(', ')}`);
    }
  ...
  private formatRulesAssessment(result: EligibilityAssessmentResult): string {
    ...
    assessment.push(`Recommended Programs: ${result.recommendedPrograms.length}`);
    assessment.push('');
  ...
  private async getProgramDetails(programResults: RulesEngineResult[]): Promise<string> {
...
Resources: ${JSON.stringify(program.resourceLinks)}`;
  ...
  private async callAnalysisAPI(
    ...
    const prompt = ELIGIBILITY_ANALYSIS_PROMPT
      .replace('{user_situation}', userSituation)
    ...
    try {
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert benefits counselor. Provide comprehensive, empathetic eligibility guidance in valid JSON format.'
          },
          {
            role: 'user',
            content: prompt
  ...
  private enhanceDocumentSources(
    ...
    commonDocs.forEach(commonDoc => {
      ...
      if (!existing) {
        ...
        // Merge sources
        const newSources = commonDoc.sources.filter(source => 
  ...
  private generateFallbackAnalysis(
    ...
    return {
      overallAssessment: {
        eligibilityLabel: rulesEngineResult.overallResult === 'LIKELY_ELIGIBLE' ? 'likely_eligible' : 'possibly_eligible',
        confidenceLevel: rulesEngineResult.confidenceScore,
        explanation: 'Basic assessment completed. Please consult with a benefits counselor for detailed guidance.',
      ...
      programAnalysis: rulesEngineResult.programResults.map(program => ({
        ...
        aiAssessment: program.eligibilityStatus === 'eligible' ? 'recommended' : 'consider',
  ...
  async enhanceExistingAssessment(
    ...
    try {
      ...
      const questionnaire = assessment.questionnaireData as QuestionnaireResponse;
      const rulesResult = assessment.rulesEngineResult as EligibilityAssessmentResult;

      return this.analyzeEligibility(rulesResult, questionnaire, additionalContext);
  ...
  async generateSimplifiedGuidance(
    ...
    try {
      ...
      return {
        guidance: result.guidance || 'Consult with a benefits counselor for personalized assistance.',
        nextSteps: result.nextSteps || ['Contact local assistance office', 'Gather required documents']
      };

    } catch (error) {
      logger.error('Failed to generate simplified guidance:', error);
      return {
        guidance: 'Please consult with a local benefits counselor for assistance.',
        nextSteps: ['Call 211 for local resources', 'Visit local social services office']
      };
    }
  }
}
