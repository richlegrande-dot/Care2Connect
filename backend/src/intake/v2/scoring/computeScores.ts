...
function scoreHousingStability(data: IntakeData, pack: PolicyPack): DimensionScore {
  ...
  if (situation && pm.situationPoints[situation] !== undefined) {
    raw += addContributor(
      contributors, 'current_living_situation', situation,
  ...
  const isHomelessSituation = situation && pm.homelessSituations.includes(situation);

  // At risk of losing
  if (housing.at_risk_of_losing === true) {
    raw += addContributor(contributors, 'at_risk_of_losing', true, pm.atRiskOfLosing, 'At risk of losing current housing');
  }

  // Eviction notice
  if (housing.eviction_notice === true) {
    raw += addContributor(contributors, 'eviction_notice', true, pm.evictionNotice, 'Has eviction notice');
  }
  ...
  if (isHomelessSituation) {
    ...
    if (duration && pm.durationPoints[duration]) {
      raw += addContributor(
        contributors, 'how_long_current', duration,
  ...
  if (housing.can_return_to_previous === false) {
    raw += addContributor(contributors, 'can_return_to_previous', false, pm.cannotReturnToPrevious, 'Cannot return to previous housing');
  }

  return { score: cap(raw, pack.maxDimensionScore), maxScore: pack.maxDimensionScore, contributors };
}
...
function scoreSafetyCrisis(data: IntakeData, pack: PolicyPack): DimensionScore {
  ...
  if (safety.fleeing_dv === true) {
    raw += addContributor(contributors, 'fleeing_dv', true, pm.fleeingDv, 'Fleeing domestic violence');
  }
  if (safety.fleeing_trafficking === true) {
    raw += addContributor(contributors, 'fleeing_trafficking', true, pm.fleeingTrafficking, 'Fleeing trafficking');
  }
  if (safety.suicidal_ideation_recent === true) {
    raw += addContributor(contributors, 'suicidal_ideation_recent', true, pm.suicidalIdeationRecent, 'Recent suicidal ideation');
  }
  if (safety.experienced_violence_recently === true) {
    raw += addContributor(contributors, 'experienced_violence_recently', true, pm.experiencedViolenceRecently, 'Experienced violence recently');
  }

  const safetyFeeling = safety.feels_safe_current_location as string | undefined;
  if (safetyFeeling && pm.feelsSafe[safetyFeeling] !== undefined) {
    const pts = pm.feelsSafe[safetyFeeling];
    if (pts > 0) {
      raw += addContributor(
        contributors, 'feels_safe_current_location', safetyFeeling, pts,
        safetyFeeling === 'no' ? 'Does not feel safe at current location' : 'Only sometimes feels safe'
      );
    }
  }
  ...
  if (mentalHealth && pm.mentalHealth[mentalHealth] !== undefined) {
    ...
    if (pts > 0) {
      raw += addContributor(
        contributors, 'mental_health_current', mentalHealth, pts,
  ...
  if (substance && pm.substanceUse[substance] !== undefined) {
    ...
    if (pts > 0) {
      raw += addContributor(
        contributors, 'substance_use_current', substance, pts,
        substance === 'regular' ? 'Regular substance use' : 'Seeking substance use treatment'
      );
    }
  }

  if (safety.has_protective_order === true) {
    raw += addContributor(contributors, 'has_protective_order', true, pm.protectiveOrder, 'Has protective order');
  }

  return { score: cap(raw, pack.maxDimensionScore), maxScore: pack.maxDimensionScore, contributors };
}

function scoreVulnerabilityHealth(data: IntakeData, pack: PolicyPack): DimensionScore {
  ...
  if (meaningfulConditions.length >= 3) {
    raw += addContributor(contributors, 'chronic_conditions',
      `${meaningfulConditions.length} conditions`, pm.chronicConditions.threeOrMore,
      '3+ chronic health conditions');
  } else if (meaningfulConditions.length >= 1) {
    raw += addContributor(contributors, 'chronic_conditions',
      `${meaningfulConditions.length} condition(s)`, pm.chronicConditions.oneOrTwo,
      '1–2 chronic health conditions');
  }

  if (health.currently_pregnant === true) {
    raw += addContributor(contributors, 'currently_pregnant', true, pm.currentlyPregnant, 'Currently pregnant');
  }
  if (health.needs_immediate_medical === true) {
    raw += addContributor(contributors, 'needs_immediate_medical', true, pm.needsImmediateMedical, 'Needs immediate medical attention');
  }
  if (health.needs_medication === true && health.has_access_to_medication === false) {
    raw += addContributor(contributors, 'medication_access', 'no_access', pm.medicationNoAccess, 'Needs medication but lacks access');
  }
  ...
  if (selfCare && pm.selfCare[selfCare] !== undefined) {
    ...
    if (pts > 0) {
      ...
      raw += addContributor(contributors, 'self_care_difficulty', selfCare, pts,
        labels[selfCare] ?? `Self-care difficulty: ${selfCare.replace(/_/g, ' ')}`);
    }
  }

  if (health.has_health_insurance === false) {
    raw += addContributor(contributors, 'has_health_insurance', false, pm.noHealthInsurance, 'No health insurance');
  }

  const lastVisit = health.last_medical_visit as string | undefined;
  if (lastVisit === 'over_1_year' || lastVisit === 'never') {
    raw += addContributor(contributors, 'last_medical_visit', lastVisit, pm.noRecentMedicalVisit, 'No recent medical visit');
  }
  ...
  if (dob) {
    ...
    if (age !== null) {
      if (age < pm.youthAge.maxAge) {
        raw += addContributor(contributors, 'age_youth', `${age}`, pm.youthAge.points, `Youth (under ${pm.youthAge.maxAge})`);
      } else if (age >= pm.seniorAge.minAge) {
        raw += addContributor(contributors, 'age_senior', `${age}`, pm.seniorAge.points, `Senior (${pm.seniorAge.minAge}+)`);
      }
    }
  }
  ...
  if (demographics.has_dependents === true) {
    ...
    if (depAges.length > 0 || demographics.has_dependents === true) {
      raw += addContributor(contributors, 'has_dependents', true, pm.hasDependents, 'Has dependent children');
    }
  }
...
function scoreChronicitySystem(data: IntakeData, pack: PolicyPack): DimensionScore {
  ...
  if (history.currently_chronic === true) {
    raw += addContributor(contributors, 'currently_chronic', true, pm.currentlyChronic, 'Chronically homeless (12+ months or 4+ episodes)');
  }

  const episodes = history.total_homeless_episodes as number | undefined;
  if (episodes !== undefined) {
    if (episodes >= 4) {
      raw += addContributor(contributors, 'total_homeless_episodes', `${episodes}`, pm.episodes.fourOrMore, `${episodes} episodes of homelessness`);
    } else if (episodes >= 2) {
      raw += addContributor(contributors, 'total_homeless_episodes', `${episodes}`, pm.episodes.twoOrMore, `${episodes} episodes of homelessness`);
    }
  }
  ...
  if (months !== undefined) {
    if (months >= 24) {
      raw += addContributor(contributors, 'total_homeless_months', `${months}`, pm.months.twentyFourOrMore, `${months} total months homeless`);
    } else if (months >= 12) {
      raw += addContributor(contributors, 'total_homeless_months', `${months}`, pm.months.twelveOrMore, `${months} total months homeless`);
    }
  }
  ...
  if (erUse && pm.emergencyServices[erUse] !== undefined) {
    ...
    if (pts > 0) {
      raw += addContributor(contributors, 'emergency_services_use', erUse, pts,
        erUse === '6_plus_times' ? '6+ emergency service uses in 6 months' : '3–5 emergency service uses in 6 months');
    }
  }

  if (history.incarceration_recent === true) {
    raw += addContributor(contributors, 'incarceration_recent', true, pm.incarcerationRecent, 'Recently released from incarceration');
  }

  const institutions = (history.institutional_history as string[] | undefined) ?? [];
  const meaningfulInst = institutions.filter(i => i !== 'none');
  if (meaningfulInst.length >= 2) {
    raw += addContributor(contributors, 'institutional_history', `${meaningfulInst.length} types`, pm.institutionalHistory.twoOrMore, '2+ types of institutional history');
  } else if (meaningfulInst.length === 1) {
    raw += addContributor(contributors, 'institutional_history', meaningfulInst[0], pm.institutionalHistory.one, 'Institutional history');
  }

  if (income.has_any_income === false) {
    raw += addContributor(contributors, 'has_any_income', false, pm.noIncome, 'No income');
  }
  if (income.has_valid_id === false) {
    raw += addContributor(contributors, 'has_valid_id', false, pm.noValidId, 'No valid ID');
  }
...
function evaluateCondition(
  ...
  switch (cond.operator) {
    case '>=': return score >= cond.threshold;
    case '<=': return score <= cond.threshold;
    case '>':  return score > cond.threshold;
    case '<':  return score < cond.threshold;
...
function determineLevel(
  ...
  for (const rule of pack.waterfallRules) {
    ...
    const anyOfPass = !rule.anyOf || rule.anyOf.length === 0 ||
...
function applyOverrides(
  ...
  for (const rule of pack.overrideRules) {
    ...
    const fieldsMatch = rule.fieldChecks.every(check => {
      const moduleData = (data as Record<string, unknown>)[check.module] as Record<string, unknown> | undefined;
      return moduleData !== undefined && moduleData[check.field] === check.value;
    });
      ...
      if (dob) {
        ...
        if (age !== null) {
          switch (rule.ageCheck.operator) {
            case '<':  ageMatch = age < rule.ageCheck.age; break;
            case '>=': ageMatch = age >= rule.ageCheck.age; break;
            case '<=': ageMatch = age <= rule.ageCheck.age; break;
            case '>':  ageMatch = age > rule.ageCheck.age; break;
          }
...
function determinePriorityTier(totalScore: number, pack: PolicyPack): PriorityTier {
...
export function computeScores(
  ...
  const { level: waterfallLevel, rule: placementRule } = determineLevel(scores, pack);

  // Apply override floors from policy pack
  const { level: finalLevel, overrides: overridesApplied } = applyOverrides(waterfallLevel, data, pack);
  ...
  const tierRank: Record<PriorityTier, number> = { CRITICAL: 3, HIGH: 2, MODERATE: 1, LOWER: 0 };
  ...
  return {
    ...
    priorityTier,
    placementRule: overridesApplied.length > 0
