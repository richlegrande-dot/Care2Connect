...
export class APIClient {
  ...
  constructor(baseUrl: string) {
    ...
    this.profile = {
      ...
      create: async (data: any) => {
        ...
        return this._fetchJson(url, { method: 'POST', body: JSON.stringify(data), headers: { 'Content-Type': 'application/json' } })
      },
      update: async (id: string, data: any, userId?: string) => {
        const url = `${this.baseUrl}/profile/${id}`
        const headers: any = { 'Content-Type': 'application/json' }
        if (userId) headers['x-user-id'] = userId
        return this._fetchJson(url, { method: 'PUT', body: JSON.stringify(data), headers })
      },
  ...
  private async _handleResponse(res: Response) {
    if (!res.ok) {
      ...
      throw new Error(`API Error: ${res.statusText}${body ? ' - ' + body : ''}`)
    }
    return res.json()
  }
}
const base: string = typeof process !== 'undefined' && process.env && (process.env.NEXT_PUBLIC_API_URL || process.env.API_URL)
