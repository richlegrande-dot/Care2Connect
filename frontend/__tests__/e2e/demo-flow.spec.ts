...
test.describe('Complete Demo Flow', () => {
  ...
  test('Complete happy path: Record → Extract → Review → Donate', async ({ page }) => {
    ...
    await page.route('/api/transcription', async route => {
      await route.fulfill({
        json: {
          success: true,
          transcript: 'I am John Smith from California. I need $5000 to help with medical expenses after my cancer diagnosis. The treatment costs are overwhelming and I need community support.',
      ...
      await route.fulfill({
        json: {
          ...
          draft: {
            ...
            location: { 
              value: { country: 'United States', state: 'California', city: '', zip: '' }, 
              confidence: 0.8 
            },
            category: { value: 'Medical', confidence: 0.9 },
            goalAmount: { value: 5000, confidence: 0.85 },
            title: { value: 'Help John Smith with Cancer Treatment', confidence: 0.8 },
            storyBody: { value: 'I am facing overwhelming medical expenses...', confidence: 0.9 },
            shortSummary: { value: 'Support for cancer treatment costs', confidence: 0.8 },
    ...
    await expect(page.getByText('$5,000')).toBeVisible();
    await expect(page.getByText('Help John Smith with Cancer Treatment')).toBeVisible();
    ...
    await expect(page.getByText(/Help John Smith with Cancer Treatment/i)).toBeVisible();
    await expect(page.getByText(/Support for cancer treatment costs/i)).toBeVisible();
  ...
  test('Fallback flow: Manual input when recording fails', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /type manually/i })).toBeVisible();
    ...
    // Type story manually
    await page.getByRole('textbox').fill('I am Sarah from Texas and I need help with housing costs after losing my job.');
  ...
  test('Error handling: Network failures and retries', async ({ page }) => {
    ...
    // Switch to manual input
    await page.getByRole('button', { name: /type story|manual input/i }).click();
  ...
  test('Accessibility: Keyboard navigation and screen reader support', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /start recording|record/i })).toBeFocused();
  ...
  test('Data protection: Sensitive information handling', async ({ page }) => {
    ...
    await page.route('/api/transcription', async route => {
      ...
      if (data.transcript?.includes('123-45-6789')) {
        await route.fulfill({
          json: {
            success: false,
            error: 'Story contains sensitive information (SSN). Please remove personal identifiers and try again.',
    ...
    await page.getByRole('textbox').fill('My SSN is 123-45-6789 and I need help.');
    await page.getByRole('button', { name: /analyze story/i }).click();
