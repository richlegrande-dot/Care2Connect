...
class UrgencyEnhancements_v1c_31 {
  ...
  enhanceUrgency(story, currentUrgency) {
    try {
      ...
      if (currentUrgency >= 0.35 && currentUrgency <= 0.6) {
        ...
        const explicitDeadlineBoost =
          this.detectExplicitDeadlineUrgency(fullText);
        if (explicitDeadlineBoost > 0) {
          adjustedUrgency = Math.min(
            0.8,
            adjustedUrgency + explicitDeadlineBoost,
          );
        ...
        const medicalEmergencyBoost =
          this.detectMedicalEmergencyExplicit(fullText);
        ...
        const utilityCrisisBoost =
          this.detectUtilityCrisisWithChildren(fullText);
      ...
      if (this.debugMode && totalBoost > 0) {
        console.log(
          `[V1c_3.1 Enhancement] Story: ${story.id || "unknown"} | Original: ${currentUrgency} | Enhanced: ${adjustedUrgency} | Total Boost: +${totalBoost} | Applied: ${enhancements.join(", ")}`,
        );
      }
      ...
      console.error(
        "[UrgencyEnhancements_v1c_31] Error in enhanceUrgency:",
        error,
      );
      return {
        originalUrgency: currentUrgency,
  ...
  detectExplicitDeadlineUrgency(text) {
    ...
    const matches = explicitTodayTomorrow.filter((pattern) =>
      pattern.test(text),
    ).length;
    if (matches > 0) {
  ...
  detectChildrenAtRisk(text) {
    ...
    const hasExplicitRisk =
      childrenExplicitRisk.filter((pattern) => pattern.test(text)).length > 0;
  ...
  detectMedicalEmergencyExplicit(text) {
    ...
    const matches = medicalExplicitUrgent.filter((pattern) =>
      pattern.test(text),
    ).length;
    if (matches > 0) {
  ...
  detectUtilityCrisisWithChildren(text) {
    ...
    const hasUtilityShutoff =
      /power.*shut.*off|electricity.*disconnected|gas.*turned.*off|water.*shut.*off|heat.*off|no.*heat|no.*power/i.test(
        text,
      );
    const hasChildren = /children|child|kids|baby|infant/i.test(text);
    const hasImmediate =
      /today|tomorrow|this.*week|need.*now|immediately/i.test(text);
