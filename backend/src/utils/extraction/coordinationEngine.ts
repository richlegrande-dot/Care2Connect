...
class CrossFieldValidator {
   ...
   */
  validateCategoryAmount(category: string | null, amount: number | null): {
    ...
    const categoryRanges: Record<string, { typical: [number, number]; max: number }> = {
    ...
    if (amount < range.typical[0] * 0.5) {
      ...
      if (category === 'HEALTHCARE' && amount < 200) {
        suggestions.push(`amount_low_for_medical: ${amount} seems low for healthcare`);
      } else if (category === 'HOUSING' && amount < 300) {
        suggestions.push(`amount_low_for_housing: ${amount} seems low for housing`);
      }
    }
    ...
    if (amount > range.max) {
      confidence -= 0.4;
      suggestions.push(`amount_high_for_category: ${amount} exceeds typical max for ${category}`);
    }
   ...
   */
  validateUrgencyCategory(urgency: string | null, category: string | null, transcript: string): {
    isConsistent: boolean;
    confidence: number;
    suggestions: string[];
  } {
    ...
    if (category === 'SAFETY') {
      if (urgency === 'LOW' || urgency === 'MEDIUM') {
        ...
        const safetyKeywords = ['abuse', 'violence', 'stalker', 'threat', 'danger', 'unsafe'];
        const hasSafetyContent = safetyKeywords.some(keyword => lowerTranscript.includes(keyword));
        
        if (hasSafetyContent) {
          confidence -= 0.3;
          suggestions.push(`urgency_low_for_safety: Safety issues typically require HIGH/CRITICAL urgency`);
        }
      }
    }
    ...
    if (urgency === 'CRITICAL') {
      ...
      if (!urgentCategories.includes(category)) {
        ...
        const criticalKeywords = ['emergency', 'crisis', 'immediate', 'today', 'tomorrow'];
        const hasCriticalContent = criticalKeywords.some(keyword => lowerTranscript.includes(keyword));
        
        if (!hasCriticalContent) {
          confidence -= 0.2;
          suggestions.push(`urgency_critical_category_mismatch: CRITICAL urgency with ${category} category`);
        }
      }
    }
    ...
    if (urgency === 'LOW' && category === 'EMERGENCY') {
      confidence -= 0.4;
      suggestions.push(`urgency_low_for_emergency: EMERGENCY category with LOW urgency is inconsistent`);
    }
   ...
   */
  validateUrgencyAmount(urgency: string | null, amount: number | null): {
    isConsistent: boolean;
    confidence: number;
    suggestions: string[];
  } {
   ...
   */
  validateNameContext(name: string | null, transcript: string): {
    isConsistent: boolean;
    confidence: number;
    suggestions: string[];
  } {
    ...
    for (const pattern of contradictionPatterns) {
      if (pattern.test(transcript)) {
        confidence -= 0.2;
        suggestions.push(`name_contradiction_detected: Potential name correction in transcript`);
        break;
      }
    }

    // Check if name appears in multiple contexts (boost confidence)
    const nameOccurrences = (lowerTranscript.match(new RegExp(lowerName, 'gi')) || []).length;
    ...
    if (urgencyAsName.includes(lowerName)) {
      confidence -= 0.5;
      suggestions.push(`urgency_word_as_name: "${name}" might be urgency indicator, not name`);
    }
...
class ConfidenceCalibrator {
  ...
  calculateConsistencyScore(validationResults: any): number {
    const results = Object.values(validationResults).filter(result => 
    ...
    const inconsistentCount = results.filter(result => result.confidence < 0.6).length;
    const inconsistencyPenalty = inconsistentCount * 0.1;
    
    return Math.max(0.1, Math.min(1.0, averageConfidence - inconsistencyPenalty));
  }
}
...
class FieldEnhancer {
  enhanceFields(
    ...
    enhanced.category = this.enhanceCategory(fields, validationResults, transcript, adjustments);
    
    // Enhance urgency based on category/amount patterns (ensure never null)
    enhanced.urgencyLevel = this.enhanceUrgency(fields, validationResults, transcript, adjustments) || fields.urgencyLevel;
    
    // Enhance amount based on category validation
    enhanced.goalAmount = this.enhanceAmount(fields, validationResults, transcript, adjustments);
  ...
  private enhanceCategory(
    ...
    const lowerTranscript = transcript.toLowerCase();
    if (lowerTranscript.includes('personal situation') && 
        lowerTranscript.includes('not housing related') && 
        lowerTranscript.includes('not medical')) {
      adjustments.push('category_override: Personal situation with explicit non-housing/medical exclusion -> OTHER');
      return 'OTHER';
    }
    ...
    if (currentCategory === 'OTHER') {
      ...
      if (/(?:surgery|hospital|medical|health|doctor|treatment|medication)/i.test(transcript)) {
        adjustments.push('category_enhanced: OTHER -> HEALTHCARE based on medical keywords');
        return 'HEALTHCARE';
      }
      
      // Check for strong housing indicators  
      if (/(?:rent|eviction|housing|apartment|mortgage|utilities)/i.test(transcript)) {
        adjustments.push('category_enhanced: OTHER -> HOUSING based on housing keywords');
        return 'HOUSING';
      }
      
      // Check for strong safety indicators
      if (/(?:abuse|violence|stalker|threat|danger|domestic)/i.test(transcript)) {
        adjustments.push('category_enhanced: OTHER -> SAFETY based on safety keywords');
        return 'SAFETY';
      }
    }

    // If amount suggests healthcare but category is different
    if (fields.goalAmount && fields.goalAmount > 5000 && currentCategory !== 'HEALTHCARE') {
      if (/(?:medical|health|surgery|hospital)/i.test(transcript)) {
        adjustments.push(`category_amount_alignment: High amount ${fields.goalAmount} + medical keywords suggest HEALTHCARE`);
        return 'HEALTHCARE';
      }
    }
  ...
  private enhanceUrgency(
    ...
    if (fields.category === 'SAFETY' && currentUrgency && ['LOW', 'MEDIUM'].includes(currentUrgency)) {
      if (/(?:abuse|violence|stalker|threat|danger)/i.test(transcript)) {
        adjustments.push('urgency_safety_boost: Safety category boosted to HIGH urgency');
        return 'HIGH';
      }
    }

    // Critical temporal indicators should boost urgency
    if (currentUrgency !== 'CRITICAL' && /(?:today|tomorrow|emergency|crisis|immediate)/i.test(transcript)) {
      adjustments.push('urgency_temporal_boost: Temporal indicators suggest CRITICAL urgency');
      return 'CRITICAL';
    }
  ...
  private enhanceAmount(
    ...
    if (fields.category === 'HEALTHCARE' && currentAmount && currentAmount < 500) {
      if (/(?:surgery|hospital|treatment)/i.test(transcript)) {
        ...
        if (potentialAmount <= 20000) {
          adjustments.push(`amount_healthcare_adjustment: ${currentAmount} -> ${potentialAmount} for healthcare context`);
...
export class MultiFieldCoordinationEngine {
  ...
  coordinateExtraction(
    ...
    try {
      ...
      const consistencyScore = this.calibrator.calculateConsistencyScore(validationResults);
  ...
  private collectWarnings(validationResults: any): string[] {
    ...
    Object.entries(validationResults).forEach(([key, result]: [string, any]) => {
      ...
      if (result && !result.isConsistent) {
        warnings.push(`${key}_inconsistent: Cross-field validation detected inconsistency`);
      }
    });
