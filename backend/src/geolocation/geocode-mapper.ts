...
export class GeolocationMapper {
  ...
  async geocodeUnmappedResources(
    limit: number = 100,
    ...
    logger.info(
      `Geocoding complete: ${geocodedResources.length} resources mapped`,
    );
    return geocodedResources;
  }

  async geocodeResource(
    resource: ClassifiedResource,
  ): Promise<GeocodedResource | null> {
    ...
    try {
      ...
      const geocoded = this.createGeocodedResource(
        resource,
        geolocation,
        fallbackUsed,
      );
  ...
  private async tryNominatimGeocode(
    address: string,
  ): Promise<GeolocationResult | null> {
    try {
      // Rate limit check (1 request per second)
      if (
        this.rateLimits.nominatim.requests >= 3600 &&
        Date.now() < this.rateLimits.nominatim.resetTime
      ) {
  ...
  private async tryGoogleGeocode(
    address: string,
  ): Promise<GeolocationResult | null> {
    ...
    try {
      ...
      if (this.rateLimits.google.requests >= 2500) {
  ...
  private async tryCensusGeocode(
    address: string,
  ): Promise<GeolocationResult | null> {
    try {
      const response = await axios.get(
  ...
  private createGeocodedResource(
    ...
    const serviceRadius = this.calculateServiceRadius(
      resource.category,
  ...
  private calculateServiceRadius(
    category: string,
    accuracy: GeocodeAccuracy,
  ): number {
    const baseRadius =
      CATEGORY_SERVICE_RADIUS[category] || CATEGORY_SERVICE_RADIUS.other;
  ...
  private generateQualityFlags(
    ...
    const normalizedFormatted = this.normalizeAddress(
  ...
  private async saveGeocodedResource(
    ...
    try {
      ...
      logger.debug(
        `Saved geocoded resource: ${resource.name} at (${resource.geolocation.latitude}, ${resource.geolocation.longitude})`,
      );
    } catch (error) {
  ...
  async batchGeocode(batchSize: number = 50): Promise<void> {
    ...
    while (hasMoreResources) {
      ...
      if (geocoded.length === 0) {
        ...
        logger.info(
          `Batch complete: ${geocoded.length} resources geocoded (total: ${totalProcessed})`,
        );
    ...
    logger.info(
      `Batch geocoding complete: ${totalProcessed} total resources processed`,
    );
  }
  ...
  async improveGeocodingQuality(): Promise<void> {
    ...
    logger.info(
      `Found ${poorQualityResources.length} resources with poor geocoding quality`,
    );

    for (const geoResource of poorQualityResources) {
      try {
        // Try different geocoding approach or enhanced address
        const improved = await this.geocodeResource(
          geoResource.classifiedResource,
        );

        if (improved && this.isImprovedGeocoding(geoResource, improved)) {
          await this.saveGeocodedResource(improved);
          logger.info(
            `Improved geocoding for ${geoResource.classifiedResource.name}`,
          );
        }
  ...
  private isImprovedGeocoding(
    current: any,
    improved: GeocodedResource,
  ): boolean {
  ...
  private getQualityScore(quality: GeoQualityScore): number {
    switch (quality) {
      case GeoQualityScore.EXCELLENT:
        return 5;
      case GeoQualityScore.GOOD:
        return 4;
      case GeoQualityScore.ACCEPTABLE:
        return 3;
      case GeoQualityScore.POOR:
        return 2;
      case GeoQualityScore.FAILED:
        return 1;
      default:
        return 0;
    }
