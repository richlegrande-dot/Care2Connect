export function detectStripeKeyType(
  key?: string,
): "secret" | "publishable" | "unknown" | "invalid" {
...
export function validateStripeKey(key?: string): {
  ...
  if (type === "invalid")
    return { valid: false, reason: "Key appears to be a placeholder" };
  if (type === "unknown")
    return {
      valid: false,
      reason:
        "Key format not recognized (expected sk_test_/sk_live_/pk_test_/pk_live_)",
    };
...
export function resolveStripeKeysFromEnv() {
  ...
  if (!envKey) {
    if (process.env.STRIPE_SECRET_KEY)
      result.secret = process.env.STRIPE_SECRET_KEY;
    if (process.env.STRIPE_PUBLISHABLE_KEY)
      result.publishable = process.env.STRIPE_PUBLISHABLE_KEY;
    return result;
  }
