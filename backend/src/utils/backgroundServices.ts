...
class BackgroundServicesManager {
  ...
  async start(prisma?: PrismaClient): Promise<void> {
    ...
    try {
      if (config.HEALTHCHECKS_ENABLED) {
        ...
        console.log(
          "[BACKGROUND SERVICES] Health scheduler disabled by config",
        );
      }
    } catch (error) {
    ...
    if (prisma) {
      try {
        ...
        console.error(
          "[BACKGROUND SERVICES] ❌ Database watchdog failed:",
          error,
        );
      }
    }
  ...
  private async startTunnelPing(): Promise<void> {
    ...
    this.tunnelPingInterval = setInterval(
      async () => {
        try {
  ...
  async stop(): Promise<void> {
    ...
    if (this.state.healthScheduler) {
      try {
        ...
        console.error(
          "[BACKGROUND SERVICES] Error stopping health scheduler:",
          error,
        );
      }
    }
    ...
    if (this.dbWatchdog) {
      try {
        ...
        console.error(
          "[BACKGROUND SERVICES] Error stopping database watchdog:",
          error,
        );
      }
    }
    ...
    if (this.state.healthMonitor) {
      try {
        ...
        console.error(
          "[BACKGROUND SERVICES] Error stopping health monitor:",
          error,
        );
      }
    }
...
// Convenience exports
export const startBackgroundServices = (prisma?: PrismaClient) =>
  backgroundServices.start(prisma);
export const stopBackgroundServices = () => backgroundServices.stop();
export const shouldStartBackgroundServices = () =>
  backgroundServices.shouldStart();
