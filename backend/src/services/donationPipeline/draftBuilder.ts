...
export async function generateDraft(
  ...
  const name =
    userProvidedData?.beneficiaryName || signals.nameCandidate || "Anonymous";
  const goalAmount = userProvidedData?.goalAmount || undefined;
  const location =
    userProvidedData?.location || signals.locationCandidates[0] || undefined;
  const duration =
    userProvidedData?.duration ||
    suggestDuration(
      signals.urgencyScore,
      signals.needsCategories.map((c) => c.category),
    );
  ...
  const editableJson = {
    ...
    suggestedGoalAmounts: suggestGoalAmount(
      signals.needsCategories.map((c) => c.category),
    ),
...
function formatStory(transcript: string, signals: ExtractedSignals): string {
  ...
  // Impact statement
  sections.push(
    "Your support will make a real difference and help get life back on track.",
  );
...
export async function updateDraft(
  ...
  const updatedDraftData: DonationDraftData = {
    ...
    beneficiaryName:
      updates.beneficiaryName || ticket.donationDraft.beneficiary || undefined,
    goalAmount:
      updates.goalAmount ||
      ticket.donationDraft.goalAmount?.toNumber() ||
      undefined,
