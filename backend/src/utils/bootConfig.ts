...
export function getBootConfig(): BootConfig {
  ...
  return {
    ...
    transcriptionProvider:
      ...
      process.env.TRANSCRIPTION_PREFERENCE ||
      "assemblyai",
...
export function printBootBanner(config: BootConfig): void {
  ...
  console.log(
    `║ [BOOT] runMode=${config.runMode.padEnd(4)} v1Stable=${String(config.v1Stable).padEnd(5)} zeroOpenAI=${String(config.zeroOpenAI).padEnd(5)}       ║`,
  );
  console.log(
    `║        aiProvider=${config.aiProvider.padEnd(10)} transcription=${config.transcriptionProvider.padEnd(10)}    ║`,
  );
  console.log(
    `║        dbMode=${config.dbMode.padEnd(7)} port=${String(config.port).padEnd(4)} nodeEnv=${config.nodeEnv.padEnd(11)}  ║`,
  );
  console.log(
    "╚════════════════════════════════════════════════════════════════╝\n",
  );
  ...
  if (!config.v1Stable && config.runMode === "prod") {
    console.warn(
      "⚠️  [BOOT] V1_STABLE=false in production mode - not recommended",
    );
  }

  if (!config.zeroOpenAI && (config.v1Stable || config.runMode === "prod")) {
    console.warn(
      "⚠️  [BOOT] ZERO_OPENAI_MODE not enabled - OpenAI may be called",
    );
  }

  if (config.dbMode === "unknown") {
    console.warn(
      "⚠️  [BOOT] DB_MODE not set - database connectivity may be unpredictable",
    );
  }
}

export function validateBootConfig(config: BootConfig): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  if (config.v1Stable) {
    ...
    if (config.aiProvider !== "rules" && config.aiProvider !== "none") {
      errors.push(
        `V1_STABLE=true requires AI_PROVIDER=rules or none, got: ${config.aiProvider}`,
      );
    }
  }
  ...
  if (config.zeroOpenAI) {
    ...
    if (config.transcriptionProvider === "openai") {
      errors.push(
        "ZERO_OPENAI_MODE=true but TRANSCRIPTION_PROVIDER=openai - conflict",
      );
    }
  }
