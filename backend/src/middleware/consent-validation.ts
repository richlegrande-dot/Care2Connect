...
const CONSENT_REQUIREMENTS: Record<string, ConsentRequirement> = {
  ...
  data_sharing: {
    type: 'data_sharing',
    version: '1.0', 
    description: 'Consent to share data with service providers and partner organizations',
    required: true
  },
  ...
  government_access: {
    ...
    version: '1.0',
    description: 'Consent for government agencies to access your information for benefit verification and service coordination',
    required: true
  },
  ...
  pii_access: {
    type: 'pii_access',
    version: '1.0',
    description: 'Consent to collect and process personally identifiable information',
    required: true
  },
  ...
  health_data: {
    type: 'pii_access',
    version: '1.0', 
    description: 'Consent to collect and process health-related information (HIPAA compliance)',
    required: true
  },
  ...
  location_data: {
    type: 'pii_access',
    version: '1.0',
    description: 'Consent to access and process location information for service matching',
    required: false
  }
};
...
export const validateConsent = (requiredConsents: string[] = []) => {
  return async (req: Request & { user?: any }, res: Response, next: NextFunction) => {
    try {
      ...
      const endpointConsents = determineRequiredConsents(endpoint, requiredConsents);
      ...
      for (const consentType of endpointConsents) {
        ...
        const userConsent = userConsents.find(c => c.consentType === consentType);
...
function determineRequiredConsents(endpoint: string, explicitConsents: string[]): string[] {
...
export const recordConsent = async (req: Request & { user?: any }, res: Response, next: NextFunction) => {
  try {
    ...
    const consentRecord: ConsentRecord = {
      userId,
      consentType,
      consentVersion: version || CONSENT_REQUIREMENTS[consentType]?.version || '1.0',
      granted,
      timestamp: new Date(),
...
async function saveConsentRecord(record: ConsentRecord): Promise<void> {
  ...
  logger.info('Consent record saved', { userId: record.userId, type: record.consentType });
}

// Consent withdrawal middleware
export const withdrawConsent = async (req: Request & { user?: any }, res: Response, next: NextFunction) => {
  try {
...
export const getConsentStatus = async (req: Request & { user?: any }, res: Response) => {
  try {
    const userId = req.user?.userId;
    ...
    const consentStatus = Object.entries(CONSENT_REQUIREMENTS).map(([type, requirement]) => {
