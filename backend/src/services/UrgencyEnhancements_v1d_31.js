...
class UrgencyEnhancements_v1d_31 {
        ...
        try {
            ...
            if (currentUrgency >= 0.4 && currentUrgency <= 0.9) {
                ...
                if (currentUrgency >= 0.47 && currentUrgency <= 0.55) {
                    const mediumCorrectionAdjustment = this.correctToMedium(fullText, category, currentUrgency);
                    if (mediumCorrectionAdjustment < 0) {
                        adjustedUrgency = Math.max(0.3, adjustedUrgency + mediumCorrectionAdjustment);
                        totalAdjustment += mediumCorrectionAdjustment;
                        adjustments.push(`medium_boundary_correction(${mediumCorrectionAdjustment})`);
                    }
                }

                // BOUNDARY CORRECTION: Cases just below HIGH threshold that should be HIGH
                if (currentUrgency >= 0.4 && currentUrgency <= 0.46) {
                    const highCorrectionAdjustment = this.correctToHigh(fullText, category, currentUrgency);
                    if (highCorrectionAdjustment > 0) {
                        adjustedUrgency = Math.min(0.75, adjustedUrgency + highCorrectionAdjustment);
                        totalAdjustment += highCorrectionAdjustment;
                        adjustments.push(`high_boundary_correction(+${highCorrectionAdjustment})`);
                    }
                }

                // BOUNDARY CORRECTION: Cases just above CRITICAL threshold that should be HIGH
                if (currentUrgency >= 0.77 && currentUrgency <= 0.85) {
                    const criticalCorrectionAdjustment = this.correctFromCritical(fullText, category, currentUrgency);
                    if (criticalCorrectionAdjustment < 0) {
                        adjustedUrgency = Math.max(0.6, adjustedUrgency + criticalCorrectionAdjustment);
                        totalAdjustment += criticalCorrectionAdjustment;
                        adjustments.push(`critical_boundary_correction(${criticalCorrectionAdjustment})`);
                    }
                }

                // BOUNDARY CORRECTION: Cases just below CRITICAL threshold that should be CRITICAL
                if (currentUrgency >= 0.7 && currentUrgency <= 0.76) {
                    const criticalPromotionAdjustment = this.correctToCritical(fullText, category, currentUrgency);
                    if (criticalPromotionAdjustment > 0) {
                        adjustedUrgency = Math.min(0.9, adjustedUrgency + criticalPromotionAdjustment);
                        totalAdjustment += criticalPromotionAdjustment;
                        adjustments.push(`critical_promotion(+${criticalPromotionAdjustment})`);
                    }
                }
            }

            if (this.debugMode && Math.abs(totalAdjustment) > 0.01) {
                console.log(`[V1d_3.1 Precision] Story: ${story.id || 'unknown'} | Original: ${currentUrgency} | Tuned: ${adjustedUrgency} | Adjustment: ${totalAdjustment > 0 ? '+' : ''}${totalAdjustment} | Applied: ${adjustments.join(', ')}`);
            }
            ...
            console.error('[UrgencyEnhancements_v1d_31] Error in tuneUrgencyPrecision:', error);
            return {
                originalUrgency: currentUrgency,
    ...
    correctToMedium(text, category, currentUrgency) {
        ...
        const nonHighMatches = nonHighIndicators.filter(pattern => pattern.test(text)).length;
        
        // Category-specific corrections
        if (category === 'OTHER' && /personal.*situation|hard.*to.*explain/i.test(text)) {
            adjustment = 0.15 - currentUrgency; // Force to LOW-MEDIUM boundary
        } else if (category === 'EDUCATION' && /next.*semester|interested.*in/i.test(text)) {
    ...
    correctToHigh(text, category, currentUrgency) {
        ...
        const highMatches = highIndicators.filter(pattern => pattern.test(text)).length;

        // Category-specific promotions
        if (category === 'HOUSING' && /eviction.*notice|pay.*or.*quit/i.test(text)) {
            adjustment = 0.5 - currentUrgency; // Promote to solid HIGH
        } else if (category === 'UTILITIES' && /shutoff.*notice|disconnect.*tomorrow/i.test(text)) {
            adjustment = 0.52 - currentUrgency; // Promote to HIGH
        } else if (category === 'HEALTHCARE' && /surgery.*scheduled|emergency.*procedure/i.test(text)) {
    ...
    correctFromCritical(text, category, currentUrgency) {
        ...
        const nonCriticalMatches = nonCriticalIndicators.filter(pattern => pattern.test(text)).length;

        // Category-specific demotions
        if (category === 'OTHER') {
            adjustment = 0.65 - currentUrgency; // Force OTHER down from CRITICAL
        } else if (category === 'EDUCATION' && !/emergency|crisis|urgent/i.test(text)) {
    ...
    correctToCritical(text, category, currentUrgency) {
        ...
        const criticalMatches = criticalIndicators.filter(pattern => pattern.test(text)).length;
