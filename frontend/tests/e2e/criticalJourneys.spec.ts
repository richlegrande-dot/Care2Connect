...
test.describe('Critical User Journeys', () => {
  ...
  test.describe('1. Home Page and Navigation', () => {
    ...
    test('should navigate to record page', async ({ page }) => {
      ...
      const recordLink = page.getByRole('link', { name: /record|share.*story|tell.*story/i });
    ...
    test('should navigate to find stories page', async ({ page }) => {
      ...
      const findLink = page.getByRole('link', { name: /find|search|browse.*stor/i });
  ...
  test.describe('2. Upload/Record Flow (Simulated)', () => {
    test('should display record interface', async ({ page }) => {
      ...
      const hasRecordButton = await page.getByRole('button', { name: /record|start/i }).count() > 0;
      const hasUploadButton = await page.getByRole('button', { name: /upload/i }).count() > 0;
  ...
  test.describe('4. Generate Donation Tools Page', () => {
    ...
    test('should display editable excerpt field', async ({ page }) => {
      ...
      const excerptField = page.locator('textarea[name="excerpt"], textarea[placeholder*="excerpt" i]');
    ...
    test('should show QR code placeholder or image', async ({ page }) => {
      ...
      const qrCode = page.locator('img[alt*="QR" i], img[alt*="code" i], canvas, svg');
  ...
  test.describe('5. Admin Login and Story Browser', () => {
    ...
    test('should access admin panel with correct credentials', async ({ page }) => {
      await page.goto(`${FRONTEND_URL}/admin`);
      ...
      if (await passwordField.count() > 0 && await submitButton.count() > 0) {
        ...
        await passwordField.fill(process.env.ADMIN_TOKEN || 'careconnect-admin-token-2024');
        await submitButton.first().click();
        ...
        const isInAdmin = page.url().includes('admin') || page.url().includes('dashboard');
    ...
    test('should display story browser with table/list', async ({ page }) => {
      ...
      const storyItems = page.locator('[data-testid*="story"], [class*="story-item"]');
      const hasStoryItems = await storyItems.count() > 0;
      
      expect(hasTableOrList || hasStoryItems || page.locator('text=/no stories|empty/i').count() > 0).toBeTruthy();
    });
  });
