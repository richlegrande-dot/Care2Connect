...
export function getAIProvider(): AIProvider {
  ...
  switch (config.aiProvider) {
    ...
    case 'openai':
      ...
      console.warn('[AI Provider] WARNING: OpenAI provider is deprecated for V1 mode');
      console.warn('[AI Provider] Consider using AI_PROVIDER=rules instead');
      throw new Error('OpenAI provider not available in V1 mode. Use AI_PROVIDER=rules');
      
    default:
      console.warn(`[AI Provider] Unknown provider: ${config.aiProvider}, falling back to rules`);
      cachedProvider = new RulesBasedAIProvider();
  }
  
  console.log(`[AI Provider] Using: ${cachedProvider.name} (type: ${cachedProvider.type})`);
