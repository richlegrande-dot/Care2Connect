  ...
  properties: {
    ...
    race_ethnicity: {
      ...
      items: {
        type: 'string' as const,
        enum: [
          'american_indian', 'asian', 'black', 'hispanic_latino',
          'native_hawaiian', 'white', 'multi_racial', 'prefer_not_to_say',
        ],
      },
    },
    ...
    contact_phone: { type: 'string' as const, pattern: '^[0-9\\-\\+\\(\\) ]{7,20}$' },
...
const housingSchema = {
  ...
  properties: {
    ...
    location_zip: { type: 'string' as const, pattern: '^[0-9]{5}(-[0-9]{4})?$' },
  },
};
  ...
  properties: {
    ...
    chronic_conditions: {
      ...
      items: {
        type: 'string' as const,
        enum: [
          'diabetes', 'heart_disease', 'hiv_aids', 'hepatitis', 'respiratory',
          'mental_health', 'substance_use_disorder', 'physical_disability',
          'developmental_disability', 'tbi', 'other', 'none',
        ],
      },
    },
  ...
  properties: {
    ...
    income_sources: {
      ...
      items: {
        type: 'string' as const,
        enum: [
          'employment_full', 'employment_part', 'ssi', 'ssdi', 'tanf',
          'snap', 'unemployment', 'veterans_benefits', 'child_support',
          'informal', 'none',
        ],
      },
    },
  ...
  properties: {
    top_priorities: {
      ...
      items: {
        type: 'string' as const,
        enum: [
          'housing', 'food', 'employment', 'healthcare', 'mental_health',
          'substance_treatment', 'legal_help', 'childcare', 'education',
    ...
    barriers_to_housing: {
      ...
      items: {
        type: 'string' as const,
        enum: [
          'credit_history', 'criminal_record', 'eviction_history', 'pets',
          'no_id', 'no_income', 'substance_use', 'mental_health',
          'physical_disability', 'large_family',
        ],
      },
    },
    additional_notes: {
 ...
 * @module intake/v2/forms
 */
export function validateModuleData(moduleId: ModuleId, data: Record<string, unknown>): { valid: boolean; errors: string[] } {
  ...
  for (const [key, value] of Object.entries(data)) {
    ...
    if (fieldType === 'string' && typeof value === 'string') {
      ...
      if (propSchema.enum) {
        ...
        if (!allowed.includes(value)) {
          errors.push(`Invalid value for ${key}: "${value}". Expected one of: ${allowed.join(', ')}`);
        }
      }
      // minLength
      if (typeof propSchema.minLength === 'number' && value.length < propSchema.minLength) {
        errors.push(`${key} must be at least ${propSchema.minLength} characters (got ${value.length})`);
      }
      // maxLength
      if (typeof propSchema.maxLength === 'number' && value.length > propSchema.maxLength) {
        errors.push(`${key} must be at most ${propSchema.maxLength} characters (got ${value.length})`);
      }
      // Pattern
    ...
    if (fieldType === 'array' && Array.isArray(value)) {
      if (typeof propSchema.maxItems === 'number' && value.length > propSchema.maxItems) {
        errors.push(`${key} must have at most ${propSchema.maxItems} items (got ${value.length})`);
      }
      ...
      if (items) {
        ...
        for (let i = 0; i < value.length; i++) {
          ...
          if (itemType && !checkType(item, itemType)) {
            errors.push(`${key}[${i}]: expected ${itemType}, got ${typeof item}`);
          } else if (itemEnum && !itemEnum.includes(item)) {
            errors.push(`${key}[${i}]: invalid value "${item}". Expected one of: ${itemEnum.join(', ')}`);
          }
          // Integer array items — range checks
          if ((itemType === 'integer' || itemType === 'number') && typeof item === 'number') {
 ...
 */
function isFieldHidden(propSchema: Record<string, unknown>, data: Record<string, unknown>): boolean {
