...
router.post('/auth', async (req: Request, res: Response) => {
  try {
    ...
    const correctPassword = process.env.SYSTEM_PANEL_PASSWORD || 'blueberry:y22';
    ...
    if (secret) {
      const token = jwt.sign({ type: 'system-admin', ip: req.ip }, secret, { expiresIn: '30m' });
    ...
    console.log(`[SYSTEM AUTH] Successful login (session token) from ${req.ip}`);
    return res.json({ ok: true, token, expiresIn: 1800 });
  } catch (error: any) {
 ...
 */
router.get('/diagnostics/env', systemAuthMiddleware, async (req: Request, res: Response) => {
  try {
    const keys = ['OPENAI_API_KEY', 'DATABASE_URL', 'STRIPE_SECRET_KEY', 'STRIPE_PUBLISHABLE_KEY', 'STRIPE_WEBHOOK_SECRET', 'JWT_SECRET'];
    const proof = envProof.getEnvProof(keys);
    res.json({ ok: true, proof });
  } catch (error: any) {
    console.error('[ENV DIAGNOSTICS] Error:', error);
    res.status(500).json({ ok: false, error: 'Failed to generate env diagnostics' });
  }
});
 ...
 */
router.post('/run-tests', systemAuthMiddleware, async (req: Request, res: Response) => {
  try {
    const startedAt = new Date().toISOString();
    const results: Array<{ name: string; ok: boolean; message?: string; duration?: number }> = [];

    // Test 1: Storage directories writable
    const testStart1 = Date.now();
    try {
      const testDirs = ['data/support-tickets', 'data/user-errors', 'receipts', 'uploads'];
      ...
      results.push({ name: 'storageWritable', ok: true, duration: Date.now() - testStart1 });
    } catch (error: any) {
      results.push({ name: 'storageWritable', ok: false, message: error.message, duration: Date.now() - testStart1 });
    }
    ...
    try {
      ...
      results.push({ name: 'healthReady', ok: false, message: error.message, duration: Date.now() - testStart2 });
    }
    ...
    try {
      ...
      results.push({ name: 'qrGeneration', ok: testQr.startsWith('data:image'), duration: Date.now() - testStart3 });
    } catch (error: any) {
      results.push({ name: 'qrGeneration', ok: false, message: error.message, duration: Date.now() - testStart3 });
    }
    ...
    try {
      ...
      results.push({ name: 'wordExport', ok: true, duration: Date.now() - testStart4 });
    } catch (error: any) {
      results.push({ name: 'wordExport', ok: false, message: error.message, duration: Date.now() - testStart4 });
    }

    // Test 5: Support ticket write path
    const testStart5 = Date.now();
    try {
      const ticketPath = path.join(process.cwd(), 'data/support-tickets/.test');
      await fs.writeFile(ticketPath, JSON.stringify({ test: true }));
      await fs.unlink(ticketPath);
      results.push({ name: 'supportTicketWrite', ok: true, duration: Date.now() - testStart5 });
    } catch (error: any) {
      results.push({ name: 'supportTicketWrite', ok: false, message: error.message, duration: Date.now() - testStart5 });
    }
 ...
 */
router.get('/user-errors', systemAuthMiddleware, async (req: Request, res: Response) => {
  try {
 ...
 */
router.get('/user-errors/:id', systemAuthMiddleware, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const errorsDir = path.join(process.cwd(), 'data/user-errors');
    const files = await fs.readdir(errorsDir);
 ...
 */
router.get('/history', systemAuthMiddleware, async (req: Request, res: Response) => {
  try {
 ...
 */
router.get('/setup/stripe', systemAuthMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ ok: false, error: 'Failed to evaluate stripe configuration' });
  }
});
 ...
 */
router.get('/setup/stripe-webhook', systemAuthMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ ok: false, error: 'Failed to evaluate stripe webhook configuration' });
  }
});
 ...
 */
router.post('/setup/stripe-webhook/preflight', systemAuthMiddleware, async (req: Request, res: Response) => {
  try {
    if (process.env.ALLOW_SYSTEM_COMMANDS !== 'true') {
      return res.json({ cliInstalled: 'unknown', reason: 'ALLOW_SYSTEM_COMMANDS=false' });
    }
    ...
    exec('stripe --version', (err, stdout, stderr) => {
      ...
      return res.json({ cliInstalled: true, version: (stdout || stderr).toString().trim() });
    });
  } catch (error: any) {
 ...
 */
router.get('/setup/tunnel/cloudflare', systemAuthMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    const installUrl = 'https://developers.cloudflare.com/cloudflare-one/connections/connect-networks/downloads/';
    ...
    res.json({
      ...
      computedWebhookEndpoint: process.env.PUBLIC_URL ? `${process.env.PUBLIC_URL}${webhookPath}` : null,
      cliPreflight: {
        allowed: allow,
    ...
    res.status(500).json({ ok: false, error: 'Failed to evaluate cloudflare tunnel guidance' });
  }
});
 ...
 */
router.post('/setup/tunnel/cloudflare/preflight', systemAuthMiddleware, async (req: Request, res: Response) => {
  try {
    if (process.env.ALLOW_SYSTEM_COMMANDS !== 'true') {
    ...
    exec('cloudflared --version', (err, stdout, stderr) => {
      if (err) {
        return res.json({ allowed: true, cloudflaredInstalled: false, reason: String(err.message) });
      }

      return res.json({ allowed: true, cloudflaredInstalled: true, version: (stdout || stderr).toString().trim() });
    });
  } catch (error: any) {
 ...
 */
router.get('/setup/preflight', systemAuthMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    // Stripe presence
    const keys = resolveStripeKeysFromEnv();
    const stripePresent = !!(keys.secret || keys.publishable || process.env.STRIPE_WEBHOOK_SECRET);
    ...
    res.json({
      ...
      preflight: {
        ...
        email: { configured: emailConfigured, archived: true, note: 'SMTP support removed; use support log in admin health' },
        guidance: {
