...
class UrgencyEnhancements_v1d_32 {
  /**
   * Main entry point: Apply corrective precision adjustments to urgency score
   * @param {Object} story - The story object with story text
   * @param {number} currentUrgency - Current urgency score (0-1)
   * @param {string} category - Detected category
   * @returns {Object} - Tuned urgency result {score, urgency, adjustments}
   */
  static tuneUrgencyPrecision(story, currentUrgency, category = "UNKNOWN") {
    const instance = new UrgencyEnhancements_v1d_32();
  ...
  constructor() {
    ...
    this.description =
      "Corrective Precision Enhancement - Evidence-Based Surgical Corrections";
    ...
    this.patternConfidence = {
      ...
      MEDIUM: 0.7, // Medium confidence patterns (eviction+urgent)
  ...
  tuneUrgencyPrecisionInstance(story, currentScore, categoryInfo) {
    ...
    if (this.debugMode && corrections.adjustments.length > 0) {
      console.log(
        `[V1d_3.2] Applied ${corrections.adjustments.length} corrections: ${currentScore} → ${corrections.finalScore}`,
      );
      corrections.adjustments.forEach((adj) => {
        console.log(
          `  - ${adj.type}: ${adj.reason} (${adj.adjustment > 0 ? "+" : ""}${adj.adjustment.toFixed(3)})`,
        );
      });
    }

    // Return result in same format as V1d for compatibility
    return {
  ...
  applyCriticalBoundaryCorrection(story, currentScore, categoryInfo) {
    ...
    if (this.matchesCriticalMedicalEmergency(storyText)) {
      ...
      result.reason =
        "Surgery + accident/emergency + hospital pattern (T002 target)";
  ...
  applyMediumPrecisionCorrection(story, currentScore, categoryInfo) {
    ...
    if (currentScore > 0.55 && this.matchesNonCriticalMedical(storyText)) {
      ...
      result.reason =
        "Dampen non-emergency medical over-assessment (T023 target)";
      result.confidence = this.patternConfidence.MEDIUM;
      return result;
    }

    if (currentScore > 0.5 && this.matchesMotherCareNonCritical(storyText)) {
      const dampen = -0.03;
      result.applied = true;
      result.adjustment = dampen;
      result.newScore = currentScore + dampen;
    ...
    if (currentScore < 0.3 && this.matchesJobLossWithFamilyStress(storyText)) {
      const boost = 0.04;
      result.applied = true;
      result.adjustment = boost;
      result.newScore = currentScore + boost;
    ...
    if (
      currentScore < 0.35 &&
      this.matchesBasicLivingExpensesFamilyNeed(storyText)
    ) {
      const boost = 0.03;
      result.applied = true;
      result.adjustment = boost;
  ...
  matchesCriticalMedicalEmergency(text) {
    ...
    return (
      surgeryTerms.some((term) => text.includes(term)) &&
  ...
  matchesUrgentEviction(text) {
    ...
    return (
      evictionTerms.some((term) => text.includes(term)) &&
  ...
  matchesRentArrears(text) {
    ...
    return (
      rentTerms.some((term) => text.includes(term)) &&
  ...
  matchesMotherCareNonCritical(text) {
    ...
    return (
      motherTerms.some((term) => text.includes(term)) &&
      nonCriticalTerms.some((term) => text.includes(term)) &&
      !criticalExcludes.some((term) => text.includes(term))
    );
  }
  ...
  matchesBasicLivingExpensesFamilyNeed(text) {
    ...
    return (
      basicNeedTerms.some((term) => text.includes(term)) &&
      familyTerms.some((term) => text.includes(term))
    );
  }
