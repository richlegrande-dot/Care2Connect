  ...
  try {
    ...
    if (error) {
      ...
      const errorTerms = error
        ...
        .filter(
          (term) =>
            term.length > 3 && !["error", "failed", "undefined"].includes(term),
        );
    ...
    if (context) {
      ...
      if (context.transcriptLength === 0)
        searchTerms.push("empty", "transcript");
    }
    ...
    for (const match of matches) {
      if (
        ...
        Array.isArray(match.metadata.actions)
      ) {
        for (const action of match.metadata.actions) {
          if (action.type && action.payload) {
            actions.push({
              ...
              description:
                action.description || `Action from ${match.tags.join(", ")}`,
            });
          }
        }
      }
 ...
 */
export async function getNativeLanguageRules(
  language?: string,
): Promise<KnowledgeMatch[]> {
  try {
    const results = await searchKnowledge({
 ...
 */
export async function findTroubleshootingBySymptoms(
  symptoms: string[],
): Promise<KnowledgeMatch[]> {
  try {
    ...
    for (const chunk of chunks) {
      if (chunk.metadata && Array.isArray(chunk.metadata.symptoms)) {
        const chunkSymptoms = chunk.metadata.symptoms.map((s: string) =>
          s.toLowerCase(),
        );
        const matchCount = symptoms.filter((s) =>
          chunkSymptoms.some(
            (cs) =>
              cs.includes(s.toLowerCase()) || s.toLowerCase().includes(cs),
          ),
        ).length;
...
export function logKnowledgeUsage(params: {
  ...
  console.log(
    `[KnowledgeUsage] stage=${stage} chunks=${chunkIds.length} outcome=${outcome}${ticketId ? ` ticket=${ticketId}` : ""}`,
  );
