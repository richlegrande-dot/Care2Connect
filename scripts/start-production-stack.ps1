# DEPRECATED: This script has been replaced by start-stack.ps1
# This wrapper exists for backward compatibility only
# 
# CANONICAL SCRIPT: scripts/start-stack.ps1
# 
# Why the change?
# - Reads ports from config/ports.json (no hardcoded ports)
# - Fully atomic startup with better error handling
# - Integrated with watchdog-stack.ps1 auto-recovery
# 
# Migration: Replace all references to start-production-stack.ps1 with start-stack.ps1
...
Write-Host "========================================" -ForegroundColor Yellow
Write-Host ""
Write-Host "This script (start-production-stack.ps1) is deprecated." -ForegroundColor Yellow
Write-Host "Use scripts/start-stack.ps1 instead." -ForegroundColor Cyan
Write-Host ""
...
$args = @()
if ($SkipDomainGuard) { $args += "-SkipDomainGuard" }
if ($SkipTunnelValidation) { $args += "-SkipTunnelValidation" }

& "$WorkspaceRoot\scripts\start-stack.ps1" @args
exit $LASTEXITCODE
