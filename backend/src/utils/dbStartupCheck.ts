 ...
 */
export async function checkDatabaseStartup(prisma: PrismaClient): Promise<boolean> {
  try {
    // Lightweight ping
    ...
    if (authIndicators.some(i => msg.toLowerCase().includes(i))) {
      console.error('DB auth failed — check DATABASE_URL password vs container POSTGRES_PASSWORD');
    } else {
