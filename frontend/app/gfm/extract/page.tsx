...
export default function ExtractPage() {
  ...
  const checkTranscriptionStatus = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/transcription/status`)
  ...
  const handleManualTranscript = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/transcription/text`, {
        method: 'POST',
        headers: {
  ...
  const startFollowUpSession = async (draft: any) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/transcription/followup/start`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const proceedToStepper = () => {
    if (extractionData?.draft) {
      ...
      localStorage.setItem('gfm_extracted_draft', JSON.stringify(extractionData.draft))
  ...
  const renderFieldPreview = (field: any, label: string) => {
    ...
    const confidenceClass = confidence > 0.7 ? 'text-green-600' : confidence > 0.4 ? 'text-yellow-600' : 'text-red-600'
    const confidenceIcon = confidence > 0.7 ? '✅' : confidence > 0.4 ? '⚠️' : '❓'
              ...
              <div>
                <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  <p className="text-blue-800 text-sm">
                    <strong>Manual Mode:</strong> Please paste your story transcript below. 
                    Our AI will analyze it and help create your GoFundMe campaign.
                  </p>
                </div>
            ...
            {extractionData ? (
              <div>
                ...
                <div className="space-y-3">
                  ...
                  {renderFieldPreview(extractionData.draft.title, 'Campaign Title')}
                  {renderFieldPreview(extractionData.draft.category, 'Category')}
                  {renderFieldPreview(extractionData.draft.goalAmount, 'Goal Amount')}
                  {renderFieldPreview(extractionData.draft.location, 'Location')}
                  {renderFieldPreview(extractionData.draft.shortSummary, 'Summary')}
                </div>

                {extractionData.draft.missingFields?.length > 0 && (
                  <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg">
                    <h4 className="font-medium text-red-800 mb-1">Missing Information:</h4>
                    <p className="text-sm text-red-700">
                      {extractionData.draft.missingFields.join(', ')} - We'll ask follow-up questions for these.
                    </p>
                  </div>
                )}
