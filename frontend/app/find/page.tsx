    ...
    try {
      ...
      if (searchMode === 'id') {
        ...
        if (!validateUUID(searchValue.trim())) {
          throw new Error('Invalid ticket ID format. Please enter a valid UUID.');
        }

        // Direct ticket lookup
        try {
          const ticket = await api.get<RecordingTicket>(`/tickets/${searchValue.trim()}`);
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        >
          <form onSubmit={handleSearch} className="space-y-6">
            ...
            {/* Search Input */}
            <div>
              <label htmlFor="searchValue" className="block text-sm font-semibold text-gray-900 mb-2">
                {searchMode === 'id' ? 'Ticket ID' : searchMode === 'email' ? 'Email Address' : 'Phone Number'}
              </label>
        ...
        {hasSearched && !isSearching && searchResults.length === 0 && !searchError && (
          ...
          >
            ...
            <p className="text-sm text-gray-500">
              Try searching with a different {searchMode === 'id' ? 'ticket ID' : searchMode}
            </p>
          </motion.div>
        )}
        ...
        >
          ...
          <div className="grid md:grid-cols-2 gap-4 text-sm">
            <div className="bg-white rounded-lg p-4 border border-blue-200">
              <h4 className="font-semibold text-gray-900 mb-2">Search by Ticket ID</h4>
              <p className="text-gray-600">
                If you have the exact ticket ID (UUID format), you can search directly for that profile
              </p>
            </div>
            <div className="bg-white rounded-lg p-4 border border-purple-200">
              <h4 className="font-semibold text-gray-900 mb-2">Search by Contact</h4>
              <p className="text-gray-600">
          ...
          <div className="mt-4 text-center">
            <Link href="/support" className="text-indigo-600 hover:underline font-semibold">
