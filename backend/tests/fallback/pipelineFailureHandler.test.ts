...
import {
  ...
  isSystemDegraded,
} from "../../src/services/pipelineFailureHandler";
import { prisma } from "../../src/utils/database";

const RUN = process.env.RUN_LEGACY_INTEGRATION === "true";
(RUN ? describe : describe.skip)("Pipeline Failure Handler", () => {
  beforeEach(async () => {
  ...
  describe("createPipelineFailure", () => {
    it("should create TRANSCRIPTION_FAILED failure with incident", async () => {
      ...
      const result = await createPipelineFailure("TRANSCRIPTION_FAILED", {
    ...
    it("should create PARSING_INCOMPLETE failure with partial data", async () => {
      ...
      const result = await createPipelineFailure("PARSING_INCOMPLETE", {
    ...
    it("should create SYSTEM_DEGRADED failure", async () => {
      ...
      const result = await createPipelineFailure("SYSTEM_DEGRADED", {
        ticketId: testTicketId,
    ...
    it("should handle DRAFT_GENERATION_FAILED", async () => {
      const result = await createPipelineFailure("DRAFT_GENERATION_FAILED", {
    ...
    it("should handle DOCX_FAILED", async () => {
      const result = await createPipelineFailure("DOCX_FAILED", {
    ...
    it("should handle PIPELINE_EXCEPTION", async () => {
      const result = await createPipelineFailure("PIPELINE_EXCEPTION", {
  ...
  describe("Incident Logging", () => {
    it("should create unique debugId for each failure", async () => {
      const result1 = await createPipelineFailure("TRANSCRIPTION_FAILED", {
        ticketId: "test-1",
      });
      const result2 = await createPipelineFailure("TRANSCRIPTION_FAILED", {
        ticketId: "test-2",
      });
    ...
    it("should mark all incidents with WARN severity", async () => {
      ...
      await createPipelineFailure("SYSTEM_DEGRADED", {
        ticketId: "test-sev-2",
      });
      await createPipelineFailure("PIPELINE_EXCEPTION", {
        ticketId: "test-sev-3",
      });

      const incidents = await prisma.systemIncident.findMany({
        where: {
          metadata: { path: ["ticketId"], string_starts_with: "test-sev-" },
        },
      });
