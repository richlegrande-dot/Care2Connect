...
router.get('/status', authenticate, async (req, res) => {
  try {
    ...
    const pipelineStats = {
      ...
      rawRecords: rawCount,
      classificationRate: rawCount > 0 ? (classifiedCount / rawCount * 100).toFixed(1) : '0',
      geocodingRate: classifiedCount > 0 ? (geocodedCount / classifiedCount * 100).toFixed(1) : '0',
      rankingRate: geocodedCount > 0 ? (rankedCount / geocodedCount * 100).toFixed(1) : '0',
      overallCompletionRate: rawCount > 0 ? (rankedCount / rawCount * 100).toFixed(1) : '0',
...
router.post('/refresh', authenticate, authorize(['admin', 'data_manager']), async (req, res) => {
  try {
    ...
    switch (type) {
      case 'full':
        ...
        result = await resourceRefreshScheduler.executeJobManually('full_data_refresh');
        break;
      ...
      case 'ingestion':
        ...
        if (sources.length > 0) {
          ...
          for (const source of sources) {
            const sourceResult = await resourceIngestionEngine.ingestFromSource(source, limit);
            results.push(sourceResult);
          }
          result = { 
            success: true, 
            recordsProcessed: results.reduce((sum, r) => sum + r.newRecords, 0),
            sources: results.length
          };
        } else {
          result = await resourceRefreshScheduler.executeJobManually('data_ingestion');
        }
        break;
      ...
      case 'geocoding':
        ...
        const geocoded = await geolocationMapper.geocodeUnmappedResources(limit);
      ...
      default:
        return res.status(400).json({
          success: false,
          error: 'Invalid refresh type. Use: full, ingestion, classification, geocoding, or ranking'
        });
    }
  ...
  try {
    ...
    res.json({
      ...
      summary: {
        ...
        activeSources: sourceStats.filter(s => s.statistics.status === 'active').length,
        totalRecords: sourceStats.reduce((sum, s) => sum + s.statistics.totalRecords, 0)
      }
    });
...
router.get('/pipeline', authenticate, async (req, res) => {
  try {
    ...
    const classificationStats = await resourceClassifier.getClassificationStats();
...
router.get('/categories', authenticate, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      categories: categories.sort((a, b) => b.totalResources - a.totalResources),
      summary: {
        totalCategories: categories.length,
        totalResources: categoryStats.reduce((sum, stat) => sum + stat._count.id, 0)
      }
    });
...
router.post('/quality-improvement', authenticate, authorize(['admin', 'data_manager']), async (req, res) => {
  try {
