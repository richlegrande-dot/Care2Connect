...
export function getPortConfig(): PortConfig {
  ...
  if (isNaN(backend) || backend < 1024 || backend > 65535) {
    console.error(`❌ CRITICAL: Invalid backend port: ${process.env.PORT || '3001'}`);
    console.error('   PORT must be a number between 1024-65535');
    process.exit(1);
  }

  if (isNaN(frontend) || frontend < 1024 || frontend > 65535) {
    console.error(`❌ CRITICAL: Invalid frontend port: ${process.env.FRONTEND_PORT || '3000'}`);
    console.error('   FRONTEND_PORT must be a number between 1024-65535');
    process.exit(1);
  }

  if (backend === frontend) {
    console.error(`❌ CRITICAL: Backend and frontend ports cannot be the same: ${backend}`);
 ...
 */
async function getPortOccupant(port: number): Promise<{pid?: number, process?: string} | null> {
          ...
          try {
            ...
            if (portLine) {
              ...
              if (!isNaN(pid)) {
                ...
                const tasklist = spawn('tasklist', ['/FI', `PID eq ${pid}`, '/FO', 'CSV'], { shell: true });
                let taskOutput = '';
                ...
                tasklist.on('close', () => {
                  try {
                    ...
                    if (taskLines.length > 1) {
                      ...
                      const processName = processInfo[0]?.replace(/"/g, '') || 'unknown';
 ...
 */
export async function validatePorts(config: PortConfig): Promise<PortValidationResult> {
  const errors: string[] = [];
  ...
  if (!isValid && config.strictMode) {
    ...
    for (const occupied of occupiedPorts) {
      console.error(`   - Port ${occupied.port}: PID ${occupied.pid || 'unknown'}${occupied.process ? ` (${occupied.process})` : ''}`);
    }
    ...
    for (const occupied of occupiedPorts) {
      if (occupied.pid) {
        ...
        console.error(`   • Check what's using port ${occupied.port}: netstat -ano | findstr :${occupied.port}`);
      }
    }
...
export function logPortConfiguration(config: PortConfig): void {
  ...
  console.log(`📋 [PORT CONFIG]   Mode:     ${config.strictMode ? 'STRICT' : 'FLEXIBLE'}`);
