...
import {
  processTicket,
  getTicketStatus,
} from "../../../src/services/donationPipeline/orchestrator";
...
(RUN ? describe : describe.skip)("Pipeline Integration Tests (Mocked)", () => {
  ...
  describe("Status Tracking", () => {
    it("should track processing progress through stages", async () => {
      ...
      expect(["READY", "PUBLISHED", "NEEDS_INFO"]).toContain(
        statusAfter.status,
      );
    }, 30000);
