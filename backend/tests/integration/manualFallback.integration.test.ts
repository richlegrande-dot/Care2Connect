...
import { orchestrateDonationPipeline, processDonationFromRecording } from '../../src/services/donationPipelineOrchestrator';
...
const mockedExtractSignals = extractSignals as jest.Mocked<typeof extractSignals>;
const mockedGetHealthStatus = getHealthStatus as jest.MockedFunction<typeof getHealthStatus>;
...
describe('Manual Fallback Flow - End-to-End Integration', () => {
  ...
  describe('Scenario 1: Transcription Success → Extraction Failure → Manual Save → QR Generation', () => {
    ...
    const transcript = 'Hi, my name is Jane Doe. I lost my job and need help with rent. I need about $2000.';

    it('should trigger fallback when signal extraction fails', async () => {
      // Mock extraction failure
      mockedExtractSignals.extractSignals = jest.fn().mockRejectedValue(
  ...
  describe('Scenario 2: Dry Recording → Immediate Manual Entry', () => {
    ...
    it('should immediately fallback for dry recordings', async () => {
      const result = await orchestrateDonationPipeline(ticketId, dryTranscript);
  ...
  describe('Scenario 3: System Degraded → Automatic Fallback', () => {
    ...
    const transcript = 'My name is Bob and I need help with medical bills totaling $5000.';
  ...
  describe('Scenario 4: Partial Data Extraction → User Completes', () => {
    ...
    it('should fallback when critical fields missing', async () => {
      ...
      expect(result.partialData?.extractedFields?.beneficiaryName).toBe('Sarah');
    });
  ...
  describe('Scenario 6: Load Existing Draft → Continue Editing', () => {
    ...
    it('should return 404 for non-existent draft', async () => {
      const response = await request(app)
        .get('/api/donations/manual-draft/non-existent-ticket');
    ...
    it('should track all fallback types', async () => {
      ...
      for (const testCase of testCases) {
        if (testCase.mockError) {
          mockedExtractSignals.extractSignals = jest.fn().mockRejectedValue(testCase.mockError);
        }

        await orchestrateDonationPipeline(testCase.ticketId, testCase.transcript);
