...
class CategoryEnhancements {
  ...
  static enhanceCategory(transcript, baseCategoryResult, context = {}) {
    ...
    const transportationFix = this.fixTransportationMisclassification(
      cleanText,
      baseCategoryResult,
    );
    ...
    const utilitiesFix = this.fixUtilitiesMisclassification(
      cleanText,
      baseCategoryResult,
    );
    ...
    const healthcareFix = this.fixHealthcareMisclassification(
      cleanText,
      baseCategoryResult,
    );
  ...
  static fixTransportationMisclassification(cleanText, baseResult) {
    ...
    const vehicleTerms = /\b(car|vehicle|truck|auto|automobile)\b/i.test(
      cleanText,
    );
    const repairTerms =
      /\b(repair|repairs|fix|fixed|broke|broken|mechanic|maintenance)\b/i.test(
        cleanText,
      );
    const workAccess =
      /\b(get to work|can't work|work access|commute|transportation)\b/i.test(
        cleanText,
      );

    // HARD_004 specific pattern: "emergency car repairs so I can get to work"
    const emergencyCarRepair =
      /\b(emergency|urgent)\s+(car|vehicle)\s+(repair|fix)/i.test(cleanText);
    const carRepairForWork =
      /(car|vehicle).*(repair|repairs|fix|fixed).*(work|job|commute)/i.test(
        cleanText,
      );

    if (
      (vehicleTerms && repairTerms) ||
      emergencyCarRepair ||
      carRepairForWork
    ) {
  ...
  static fixUtilitiesMisclassification(cleanText, baseResult) {
    ...
    const utilityTerms =
      /\b(electric|power|gas|water|utility|utilities)\b/i.test(cleanText);
    const billTerms = /\b(bill|payment|pay|owe)\b/i.test(cleanText);
    const shutoffTerms = /\b(shut\s*off|disconnect|turn\s*off|service)\b/i.test(
      cleanText,
    );

    // HARD_009 specific pattern: "pay my electric bill before they shut off my power"
    const electricBillShutoff =
      /\b(electric|power)\s+(bill|payment).*(shut\s*off|disconnect)/i.test(
        cleanText,
      );
    const utilityBill = /\b(electric|power|gas|water)\s+(bill|payment)/i.test(
      cleanText,
    );
  ...
  static fixHealthcareMisclassification(cleanText, baseResult) {
    ...
    if (baseResult.category === "TRANSPORTATION") {
      ...
      const transportationContext =
        /\b(transportation|get to work|commute|ride|bus|train)\b/i.test(
          cleanText,
        );
    ...
    if (baseResult.category === "MEDICAL") {
      return {
        ...
        reasons: [
          ...(baseResult.reasons || []),
          "v2a_medical_to_healthcare_mapping",
        ],
        enhanced: true,
      };
    }
    ...
    const medicalTerms =
      /\b(medical|hospital|surgery|doctor|healthcare|treatment|medication)\b/i.test(
        cleanText,
      );
    const healthContext =
      /\b(health|surgery|operation|medical\s+bills|hospital\s+bill)\b/i.test(
        cleanText,
      );

    if (
      (medicalTerms || healthContext) &&
      baseResult.category !== "HEALTHCARE"
    ) {
      return {
        category: "HEALTHCARE",
        confidence: 0.85,
        reasons: [
          ...(baseResult.reasons || []),
          "v2a_healthcare_strong_indicators",
        ],
        enhanced: true,
      };
    }
