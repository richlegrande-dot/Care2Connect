...
if (process.env.NODE_ENV !== "production") {
  ...
  router.post("/db-failure/force-exit", (req: Request, res: Response) => {
    console.log(
      "[DB Failure Test] 🚨 FORCE EXIT requested - shutting down in 2 seconds...",
    );
    ...
    setTimeout(() => {
      console.log(
        "[DB Failure Test] 🚨 Exiting with code 1 (simulated DB failure)",
      );
      process.exit(1);
    }, 2000);
  });
  ...
  console.log(
    "  - POST /api/test/db-failure/force-exit (WARNING: shuts down server)",
  );
}
