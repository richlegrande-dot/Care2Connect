...
export default function ReviewPage() {
  ...
  const getConfidenceIcon = (confidence: number) => {
    if (confidence > 0.7) return <CheckCircleIcon className="w-5 h-5 text-green-500" />
    if (confidence > 0.4) return <ExclamationTriangleIcon className="w-5 h-5 text-yellow-500" />
      ...
      <div className="border rounded-lg p-4 hover:shadow-md transition-shadow">
        <div className="flex justify-between items-start mb-2">
          ...
          <div className="flex items-center space-x-2">
            {fieldData && (
              <div className={`px-2 py-1 rounded text-xs ${getConfidenceColor(fieldData.confidence)}`}>
                {getConfidenceIcon(fieldData.confidence)}
                <span className="ml-1">{Math.round(fieldData.confidence * 100)}%</span>
              </div>
            )}
            {!isCurrentlyEditing && (
              <button
          ...
          <div>
            ...
            {fieldData && fieldData.confidence < 0.7 && (
              <p className="text-sm text-gray-500 mt-1">
                {getConfidenceText(fieldData.confidence)} - Consider reviewing this field
              </p>
            )}
          </div>
        )}
  ...
  const generateQRCode = async () => {
    ...
    try {
      ...
      const publicSlug = draft.name.value.toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-')
      
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/qr/donation-page`, {
        method: 'POST',
        headers: {
  ...
  const downloadWordDoc = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/exports/gofundme-docx`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="space-y-4">
                ...
                {renderField('category', 'Category', draft.category, 'select', categoryOptions)}
                {renderField('goalAmount', 'Goal Amount ($)', draft.goalAmount, 'number')}
                {renderField('name', 'Your Name', draft.name, 'text')}
                {renderField('dateOfBirth', 'Date of Birth (MM/DD/YYYY)', draft.dateOfBirth, 'text')}
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="space-y-4">
                {renderField('storyBody', 'Campaign Story', draft.storyBody, 'textarea')}
                {renderField('shortSummary', 'Short Summary', draft.shortSummary, 'textarea')}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
              ...
              <div className="space-y-4">
                <label className="flex items-start space-x-3 cursor-pointer">
                  ...
                  <span className="text-sm text-gray-700">
                    I consent to publish this campaign information and understand that it will be publicly visible. 
            ...
            {/* QR Code Generation */}
            <div className="bg-white rounded-lg shadow p-6">
              <h2 className="text-xl font-semibold mb-4">QR Code for Donations</h2>
              
              {qrCodeUrl ? (
                <div className="text-center">
                  <img src={qrCodeUrl} alt="Donation QR Code" className="mx-auto mb-4 border rounded" />
            ...
            <div className="bg-white rounded-lg shadow p-6">
              <h2 className="text-xl font-semibold mb-4">GoFundMe Draft Package</h2>
