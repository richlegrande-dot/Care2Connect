...
export class SpeechIntelligenceScheduler {
  ...
  start() {
    ...
    this.scheduleDaily(2, 0, async () => {
      ...
      try {
        ...
        console.log(
          `[SpeechIntel] Cleanup: ${results.deleted} sessions deleted, ${results.errors.length} errors`,
        );
      } catch (error) {
        this.addError(
          `Cleanup failed: ${error instanceof Error ? error.message : "Unknown"}`,
        );
    ...
    this.scheduleDaily(3, 0, async () => {
      ...
      try {
        ...
        console.log(
          `[SpeechIntel] Profiles: ${results.updated} updated, ${results.skipped} skipped, ${results.errors.length} errors`,
        );
      } catch (error) {
        this.addError(
          `Profile compute failed: ${error instanceof Error ? error.message : "Unknown"}`,
        );
    ...
    const smokeTestHours = parseInt(
      process.env.SPEECH_SMOKE_INTERVAL_HOURS || "6",
    );

    // Mutex to prevent overlapping smoke test runs
    let smokeTestRunning = false;

    this.smokeTestInterval = setInterval(
      async () => {
        if (smokeTestRunning) {
          console.warn(
            "[SpeechIntel] Smoke test already running, skipping this interval",
          );
          return;
        }
        ...
        try {
          ...
          console.log(
            `[SpeechIntel] Smoke test: ${this.lastSmokeTestResult.success ? "PASSED" : "FAILED"}`,
            {
  ...
  async runSmokeTestsNow(): Promise<any> {
    ...
    try {
      this.lastSmokeTestResult = await this.smokeTestRunner.runTests();
      console.log(
        `[SpeechIntel] Manual smoke test: ${this.lastSmokeTestResult.success ? "PASSED" : "FAILED"}`,
      );
      return this.lastSmokeTestResult;
    } catch (error) {
      ...
      this.addError(
        `Smoke test failed: ${error instanceof Error ? error.message : "Unknown"}`,
      );
      throw error;
    }
  }
