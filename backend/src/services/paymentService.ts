...
export const PaymentService = {
  createCheckoutSession: async ({ clientSlug, amountCents, successUrl, cancelUrl }: any) => {
    ...
    const session = await stripeClient.checkout.sessions.create({
      ...
      line_items: [{ price_data: { currency: 'usd', product_data: { name: `Donation for ${clientSlug}` }, unit_amount: amountCents }, quantity: 1 }],
      mode: 'payment',
    ...
    FileStore.saveDonation({ sessionId: session.id, clientSlug, amountCents, status: 'pending', createdAt: new Date().toISOString() });
