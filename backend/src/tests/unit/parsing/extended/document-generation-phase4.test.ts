...
describe('Phase 4: Document Generation Hardening Tests', () => {
  ...
  describe('Input Validation and Sanitization', () => {
    test('should handle null draft gracefully', async () => {
      mockPacker.toBuffer.mockResolvedValueOnce(Buffer.from('fallback document'));
    ...
    test('should handle undefined draft gracefully', async () => {
      ...
      const result = await GofundmeDocxExporter.generateDocument(undefined as any);
    ...
    test('should sanitize missing required fields with safe defaults', async () => {
      ...
      const result = await GofundmeDocxExporter.generateDocument(incompleteDraft);
    ...
    test('should handle malformed draft data without crashing', async () => {
      ...
      const result = await GofundmeDocxExporter.generateDocument(malformedDraft);
    ...
    test('should handle extremely long content without memory issues', async () => {
      ...
      mockPacker.toBuffer.mockResolvedValueOnce(Buffer.from('valid large document'));
  ...
  describe('Document Structure Integrity', () => {
    ...
    test('should warn about suspiciously small documents', async () => {
      ...
      const result = await GofundmeDocxExporter.generateDocument({ title: { value: 'Test' } });
      
      expect(result).toBeInstanceOf(Buffer);
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('[DOCX_GEN] Generated document is unusually small'),
        expect.any(Object)
      );
  ...
  describe('Error Handling and Fallback Mechanisms', () => {
    ...
    test('should throw only when both main and fallback fail', async () => {
      ...
      await expect(GofundmeDocxExporter.generateDocument({
        title: { value: 'Test' }
  ...
  describe('Document Content Validation', () => {
    test('should include all essential campaign elements', async () => {
      const completeDraft = {
        ...
        storyBody: { value: 'John needs help with medical bills after surgery.' },
      ...
      mockPacker.toBuffer.mockResolvedValueOnce(Buffer.from('complete document'));
    ...
    test('should handle optional content gracefully', async () => {
      ...
      mockPacker.toBuffer.mockResolvedValueOnce(Buffer.from('minimal document'));
    ...
    test('should sanitize special characters in content', async () => {
      ...
      mockPacker.toBuffer.mockResolvedValueOnce(Buffer.from('sanitized document'));

      const result = await GofundmeDocxExporter.generateDocument(specialContentDraft);
  ...
  describe('QR Code Integration Hardening', () => {
    test('should handle missing QR data gracefully', async () => {
      ...
      mockPacker.toBuffer.mockResolvedValueOnce(Buffer.from('document without QR'));

      const result = await GofundmeDocxExporter.generateDocument(draftWithoutQR);
    ...
    test('should validate QR code data format if provided', async () => {
      ...
      mockPacker.toBuffer.mockResolvedValueOnce(Buffer.from('document with fallback QR'));

      const result = await GofundmeDocxExporter.generateDocument(draftWithInvalidQR);
  ...
  describe('Configuration and Options Handling', () => {
    test('should respect document generation options', async () => {
      ...
      mockPacker.toBuffer.mockResolvedValueOnce(Buffer.from('configured document'));
    ...
    test('should use safe defaults for missing options', async () => {
      ...
      mockPacker.toBuffer.mockResolvedValueOnce(Buffer.from('default document'));
