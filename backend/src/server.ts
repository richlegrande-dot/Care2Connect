...
import { validatePorts, logPortConfiguration, getPortConfig } from './config/runtimePorts';

// PRODUCTION HARDENING: Configuration validation and startup logging
import { validateConfiguration, logStartupConfigSummary } from './config/configValidation';
...
if (!bootValidation.valid) {
  ...
  if (bootConfig.runMode === 'prod' || bootConfig.v1Stable) {
    console.error('🚨 [BOOT] Cannot start in prod/V1 mode with invalid configuration');
    process.exit(1);
  } else {
...
if (!envValidation.isValid) {
  console.error('❌ [STARTUP] Environment validation FAILED:', envValidation.errors);
  ...
  if (isProduction) {
    ...
    console.error('🔧 [STARTUP] IMMEDIATE ACTION: Fix environment configuration before restart');
    process.exit(1);
  } else {
    console.warn('⚠️  [STARTUP] Development mode: Continuing despite environment validation errors');
  }
}
...
*/
console.log('⚠️  [STARTUP] Config drift validation temporarily disabled for startup fix');
...
if (!envValidation.isValid) {
  ...
  if (isProduction) {
    console.error('🚨 [STARTUP] Cannot start in production mode with invalid environment');
    console.error('   Fix environment configuration and restart');
    process.exit(1);
  } else {
    console.warn('⚠️  [STARTUP] Proceeding in dev mode despite validation errors');
  }
}
...
import { backgroundServices, startBackgroundServices, shouldStartBackgroundServices } from './utils/backgroundServices';
import { preflightPortCheck, printPortConfig, validatePortConsistency } from './utils/portConfig';

// Import AI provider abstraction (V1 Zero-OpenAI Mode)
import { getAIProvider, isAIProviderAvailable, getProviderConfig } from './providers/ai';
import { getTranscriptionProvider, isTranscriptionProviderAvailable, getTranscriptionProviderConfig } from './providers/transcription';
...
if (process.env.NODE_ENV !== 'test') {
  ...
  if (KEYS_MANUAL_CONFIRM === 'true') {
    ...
    const presentRealKeys = candidateKeys.filter(k => k.value && !suspiciousPattern.test(k.value));
    if (presentRealKeys.length > 0 && process.env.KEYS_VALIDATED !== 'true') {
      console.warn('[KEYS VALIDATION NOTICE] Detected API keys present that appear real: ' + presentRealKeys.map(k => k.name).join(', '));
      console.warn('Server will still start. To remove this notice set KEYS_VALIDATED=true after reviewing backend/.env.');
    }
  }
}
...
if (process.env.NODE_ENV !== 'test') {
  (async () => {
    ...
    try {
      ...
      console.log(`[AI_MODE] Story Analysis Mode: ${aiConfig.storyAnalysisMode}`);
      console.log(`[AI_MODE] Transcription Provider: ${transcriptionConfig.provider}`);
      console.log(`[AI_MODE] Stress Test Mode: ${aiConfig.enableStressTestMode ? 'ENABLED' : 'DISABLED'}`);
      
      // Validate OpenAI not used in V1 mode
      if (aiConfig.aiProvider === 'none' || aiConfig.aiProvider === 'rules') {
        if (process.env.OPENAI_API_KEY && !process.env.OPENAI_API_KEY.includes('placeholder')) {
          console.warn('[AI_MODE] ⚠️  OPENAI_API_KEY is set but AI_PROVIDER=' + aiConfig.aiProvider);
          console.warn('[AI_MODE] ⚠️  OpenAI key will be IGNORED in V1 mode');
        }
        console.log('[AI_MODE] ✅ OpenAI disabled - V1 rules/template mode active');
      }
      ...
      console.log(`[AI_MODE] ✅ Transcription Provider initialized: ${transcriptionProvider.name}`);
      console.log('[AI_MODE] ========================================');
    } catch (error) {
      console.error('[AI_MODE] ❌ Error initializing providers:', error);
      console.error('[AI_MODE] Server will continue but AI features may not work');
    }
    ...
    try {
      ...
      if (!result.passed) {
        console.warn('Integrity check returned issues; server will continue in degraded mode');
      }
    } catch (e) {
      const error = e as Error;
      console.warn('Integrity validation failed to run:', error?.message || String(e));
    }
    ...
    try {
      ...
      if (startup.shouldExit) {
        console.warn('Startup behavior requests exit in strict mode:', startup.message);
        console.warn('Proceeding without exit to maintain non-blocking startup in this environment.');
      }
    } catch (e) {
      const error = e as Error;
      console.warn('IntegrityManager startup behavior check failed:', error?.message || String(e));
    }
  })();
}
...
app.get('/ops/health', async (req, res) => {
  const checks: {
    ...
    caddy: { status: string; tested: boolean; httpStatus?: number; error?: string };
    frontend: { status: string; tested: boolean; mimeType?: string | null; httpStatus?: number };
  ...
  try {
    ...
    const staticProbe = await fetch(`http://localhost:${proxyPort}${staticAssetPath}`, {
      headers: { 'Host': 'care2connects.org' },
      signal: AbortSignal.timeout(3000),
    });

    checks.frontend.tested = true;
    checks.frontend.status = staticProbe.ok && 
    ...
    checks.caddy.status = apiProbe.ok && 
                         apiProbe.headers.get('content-type')?.includes('json') 
                         ? 'ok' : 'degraded';
...
app.use('/admin/speech-intelligence', require('./routes/admin/speechIntelligence').default); // Speech Intelligence admin
...
console.log('[Server] Knowledge Vault Admin routes mounted at /admin/knowledge, /admin/knowledge/audit, /admin/db');
...
if (process.env.NODE_ENV !== 'test') {
  (async () => {
    ...
    if (!configValidation.valid) {
      ...
      if (process.env.NODE_ENV === 'production' || process.env.STRICT_PORT_MODE === 'true') {
        console.error('🚨 [HARDENING] Cannot start in production/strict mode with invalid configuration');
        process.exit(1);
      } else {
        console.warn('⚠️  [HARDENING] Proceeding in flexible mode despite configuration errors');
      }
    } else if (configValidation.warnings.length > 0) {
      console.warn('⚠️  [HARDENING] Configuration warnings detected - review startup summary above');
    } else {
    ...
    if (!portValidation.isValid) {
      ...
      if (portConfig.strictMode) {
        console.error('🚨 [HARDENING] STRICT MODE: Cannot start with port conflicts');
        process.exit(1);
      } else {
        console.warn('⚠️  [HARDENING] Continuing in flexible mode (not recommended)');
      }
    } else {
    ...
    const strictPortMode = portConfig.strictMode || process.env.NODE_ENV === 'production';
    const maxAttempts = strictPortMode ? 0 : Number(process.env.PORT_FAILOVER_RANGE || 5);

    console.log(`🎯 [HARDENING] Backend will bind to port ${requestedPort} (strict=${strictPortMode})`);

    const server = http.createServer(app);

    let boundPort: number | null = null;
    for (let attempt = 0; attempt <= (strictPortMode ? 0 : maxAttempts); attempt++) {
      ...
      try {
        ...
        if (attempt > 0) {
          console.warn(`⚠️  WARNING: Bound to port ${tryPort} instead of requested ${requestedPort}`);
          console.warn(`   Set STRICT_PORT_MODE=true to prevent port switching`);
        }
        break;
      } catch (err: any) {
        if (err && (err.code === 'EADDRINUSE' || err.code === 'EACCES')) {
          if (strictPortMode) {
            ...
            const occupied = portValidation.occupiedPorts.find(p => p.port === tryPort);
            if (occupied && occupied.pid) {
              console.error(`   OCCUPIED BY: PID ${occupied.pid}${occupied.process ? ` (${occupied.process})` : ''}`);
              console.error(`   KILL COMMAND: taskkill /PID ${occupied.pid} /F`);
            } else {
              console.error(`   CHECK USAGE: netstat -ano | findstr :${tryPort}`);
            }
    ...
    console.log(`🚀 HTTP Server successfully bound and listening on http://localhost:${boundPort}`);
    console.log(`✅ Process ID: ${process.pid}`);
    
    // Initialize health monitoring system
    if (envValidation.config.HEALTHCHECKS_ENABLED) {
      console.log('🏥 Starting health check scheduler...');
      const interval = (envValidation.config.HEALTHCHECKS_INTERVAL_SEC ?? 60) * 1000;
      
      // Run initial health check
      healthCheckRunner.runAllChecks().then(() => {
      ...
      console.log(`✅ Health checks scheduled every ${envValidation.config.HEALTHCHECKS_INTERVAL_SEC}s`);
    } else {
    ...
    setInterval(() => {
      ...
      console.log(`💗 Server alive on port ${boundPort} (PID: ${process.pid}) - uptime: ${uptime}s`);
    ...
    (async () => {
      try {
        ...
        if (!dbOk && (process.env.FEATURE_INTEGRITY_MODE || 'demo').toLowerCase() === 'strict') {
          console.warn('Database auth failed but server continues in degraded mode');
        }
        ...
        const error = e as Error;
        console.warn('Startup checks failed but server continues:', error?.message || String(e));
      ...
      if (process.env.ALERT_MODE && process.env.ALERT_MODE !== 'none') {
        console.log(`[ALERT] Alert system active: mode=${process.env.ALERT_MODE}`);
      }
    })();
