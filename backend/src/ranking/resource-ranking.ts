...
export class ResourceRanking {
  ...
  async rankUnrankedResources(
    ...
    logger.info(
      `Starting ranking of up to ${limit} resources with ${weightingProfile} profile`,
    );
    ...
    for (const resource of unrankedResources) {
      try {
        const ranked = await this.rankResource(
          resource as any,
          weightingProfile,
        );
        if (ranked) {
  ...
  private async calculateAccessibilityScore(
    ...
    const requirements = (resource.accessRequirements || [])
      .join(" ")
      .toLowerCase();
    ...
    if (
      description.includes("public transportation") ||
      description.includes("bus line") ||
    ...
    if (
      ...
      description.includes("free parking")
    ) {
      score += 10;
    }
    if (
      description.includes("multilingual") ||
      description.includes("spanish") ||
    ...
    if (
      requirements.includes("id required") ||
      requirements.includes("documentation")
    ) {
      score -= 10;
    }
    if (
      requirements.includes("appointment only") ||
      requirements.includes("referral")
    ) {
      score -= 15;
    }
    if (
      requirements.includes("income verification") ||
      requirements.includes("proof")
    ) {
      score -= 5;
    }
  ...
  private async calculateCapacityScore(
    ...
    if (category === ResourceCategory.SHELTER_HOUSING) {
      ...
      score = 40;
      if (
        description.includes("beds available") ||
        description.includes("vacancy")
      ) {
        score += 30;
      }
      if (
        description.includes("waiting list") ||
      ...
      score = 70;
      if (
        description.includes("while supplies last") ||
      ...
      score = 50;
      if (
        description.includes("walk-in") ||
        description.includes("no appointment")
      ) {
        score += 25;
      }
    }
    ...
    if (
      description.includes("first come") ||
      description.includes("walk-in welcome")
    ) {
      score += 20;
    }
    if (
      description.includes("large capacity") ||
      description.includes("serves many")
    ) {
      score += 15;
    }
    if (
      description.includes("small program") ||
      description.includes("limited slots")
    ) {
      score -= 15;
    }
  ...
  private async calculateQualityScore(
    ...
    // Quality indicators
    if (
      description.includes("accredited") ||
      description.includes("licensed") ||
    ...
    if (
      ...
      description.includes("recognized") ||
    ...
    if (
      ...
      description.includes("full service")
    ) {
      score += 15;
    }
  ...
  private calculateUrgencyScore(resource: GeocodedResource): number {
    ...
    if (
      services.includes("crisis") ||
      services.includes("emergency") ||
      services.includes("hotline") ||
      services.includes("24/7")
    ) {
      score += 30;
    }
    if (
      services.includes("immediate") ||
      services.includes("urgent") ||
  ...
  private calculatePopulationScore(resource: GeocodedResource): number {
    ...
    if (
      targetGroups.includes(TargetGroup.CHRONICALLY_HOMELESS) ||
      targetGroups.includes(TargetGroup.RECENTLY_HOMELESS)
    ) {
      score += 20;
    }
    if (
      targetGroups.includes(TargetGroup.DISABLED) ||
    ...
    if (
      targetGroups.includes(TargetGroup.VETERANS) ||
    ...
    if (
      targetGroups.includes(TargetGroup.LGBTQ) ||
    ...
    if (
      ...
      !targetGroups.includes(TargetGroup.GENERAL_PUBLIC)
    ) {
  ...
  private determinePriorityLevel(
    ...
    if (
      category === ResourceCategory.CRISIS_EMERGENCY ||
      category === ResourceCategory.DOMESTIC_VIOLENCE ||
      category === ResourceCategory.HUMAN_TRAFFICKING
    ) {
    ...
    if (
      category === ResourceCategory.SHELTER_HOUSING ||
  ...
  private generateQualityFlags(
    ...
    if (!resource.phone && !resource.website)
      flags.push("contact_verification_needed");
    if (this.calculateDataFreshness(resource) > 90)
      flags.push("data_verification_needed");
  ...
  private async saveRankedResource(resource: RankedResource): Promise<void> {
    try {
      ...
      logger.debug(
        `Saved ranked resource: ${resource.name} (score: ${resource.overallScore})`,
      );
    } catch (error) {
  ...
  async batchRanking(
    ...
    while (hasMoreResources) {
      const ranked = await this.rankUnrankedResources(
        batchSize,
        weightingProfile,
      );
      ...
      if (ranked.length === 0) {
        ...
        logger.info(
          `Batch complete: ${ranked.length} resources ranked (total: ${totalProcessed})`,
        );
      }
    }

    logger.info(
      `Batch ranking complete: ${totalProcessed} total resources processed`,
    );
  }
  ...
  async reRankResources(
    ...
    logger.info(
      `Found ${lowScoredResources.length} low-scored resources to re-rank`,
    );
    ...
    for (const rankedResource of lowScoredResources) {
      try {
        ...
        if (reRanked && reRanked.overallScore > rankedResource.overallScore) {
          ...
          logger.info(
            `Improved ranking for ${reRanked.name}: ${rankedResource.overallScore} -> ${reRanked.overallScore}`,
          );
        }
      } catch (error) {
