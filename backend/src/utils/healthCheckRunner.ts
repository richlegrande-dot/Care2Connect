...
async function checkOpenAI(): Promise<HealthCheckResult> {
  ...
  try {
    ...
    return {
      ...
      lastCheck: new Date(),
      error: error.name === 'TimeoutError' ? 'Request timeout' : 'Network error',
...
async function checkPrismaDB(): Promise<HealthCheckResult> {
  ...
  try {
    ...
    return {
      ...
      latency: Date.now() - start,
      lastCheck: new Date(),
      error: error.message?.includes('authentication') ? 'Authentication failed' : 'Connection failed',
...
async function checkStripe(): Promise<HealthCheckResult> {
  ...
  if (!config.STRIPE_SECRET_KEY.startsWith('sk_test_') && 
    ...
    return {
      ...
      latency: 0,
      lastCheck: new Date(),
      error: 'Invalid STRIPE_SECRET_KEY format (must start with sk_test_ or sk_live_)',
...
async function checkCloudflare(): Promise<HealthCheckResult> {
  ...
  try {
    ...
    if (config.CLOUDFLARE_ZONE_ID) {
      const zoneResponse = await fetch(`https://api.cloudflare.com/client/v4/zones/${config.CLOUDFLARE_ZONE_ID}`, {
...
async function checkTunnel(): Promise<HealthCheckResult> {
  ...
  try {
    ...
    // Test main domain
    try {
      const frontendResponse = await fetch(`https://${config.CLOUDFLARE_DOMAIN}`, {
        method: 'HEAD',
        signal: AbortSignal.timeout(10000)
      });
      checks.push({ domain: config.CLOUDFLARE_DOMAIN, status: frontendResponse.status, healthy: frontendResponse.ok });
    } catch (error: any) {
      checks.push({ domain: config.CLOUDFLARE_DOMAIN, status: 0, healthy: false, error: error.message });
    }

    // Test API domain
    try {
      const apiResponse = await fetch(`https://api.${config.CLOUDFLARE_DOMAIN}/health/live`, {
        method: 'GET',
        signal: AbortSignal.timeout(10000)
      });
      checks.push({ domain: `api.${config.CLOUDFLARE_DOMAIN}`, status: apiResponse.status, healthy: apiResponse.ok });
    } catch (error: any) {
      checks.push({ domain: `api.${config.CLOUDFLARE_DOMAIN}`, status: 0, healthy: false, error: error.message });
    }
...
export async function runHealthChecks(): Promise<HealthStatus> {
  ...
  const services = [openaiResult, prismaResult, stripeResult, cloudflareResult, tunnelResult, speechResult];
  const overall = services.every(service => service.healthy);

  console.log(`[HEALTH] Health check completed in ${Date.now() - startTime}ms. Overall: ${overall ? 'HEALTHY' : 'DEGRADED'}`);
  ...
  for (const result of services) {
    if (!result.healthy && result.error) {
      await incidentManager.reportIncident({
        ...
        recommendation: getRecommendationForService(result.service, result.error)
      });
...
function getSeverityForService(service: ServiceName, error: string): Severity {
  ...
  if (service === 'cloudflare' && error.includes('not configured')) return 'warn';
  
  // Authentication/configuration errors are warnings
  if (error.includes('not configured') || error.includes('Authentication failed')) return 'warn';
  
  // Network errors are info (temporary)
  if (error.includes('timeout') || error.includes('Network error')) return 'info';
 ...
 */
function getRecommendationForService(service: ServiceName, error: string): string {
