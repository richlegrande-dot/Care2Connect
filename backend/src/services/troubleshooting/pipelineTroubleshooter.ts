...
export async function handleFailure(
  ...
  try {
    ...
    console.log(
      `[Troubleshooter] Created incident ${incident.id} for stage ${stage}${ticketId ? ` (ticket ${ticketId})` : ""}`,
    );
    console.log(
      `[Troubleshooter] Found ${recommendations.matchedChunks.length} knowledge matches, ${recommendations.suggestedActions.length} suggested actions`,
    );
...
export async function investigateIncident(
  ...
  switch (incident.stage) {
    case PipelineStage.TRANSCRIPTION:
      ...
      if (
        incident.recording_tickets?.audioFileId ||
        incident.recording_tickets?.audioUrl
      ) {
  ...
  const matchedChunks =
    symptoms.length > 0 ? await findTroubleshootingBySymptoms(symptoms) : [];
  ...
  console.log(
    `[Troubleshooter] Investigation complete: ${diagnostics.checks.length} checks, ${matchedChunks.length} new matches`,
  );
...
export async function attemptSelfHeal(
  ...
  console.log(
    `[Troubleshooter] Attempting self-heal for incident ${incidentId} (stage: ${incident.stage})...`,
  );
  ...
  try {
    ...
    if (success) {
      ...
      console.log(
        `[Troubleshooter] Self-heal successful for incident ${incidentId}`,
      );
    }
    ...
    return {
      ...
      message: success
        ...
        : "No safe self-heal actions available",
      details: healActions,
    };
  } catch (error) {
...
export async function getIncidentStats(params?: {
  ...
  return {
    ...
    byStage: byStage.reduce(
      (acc: any, item: any) => ({ ...acc, [item.stage]: item._count }),
      {},
    ),
    bySeverity: bySeverity.reduce(
      (acc: any, item: any) => ({ ...acc, [item.severity]: item._count }),
      {},
    ),
    byStatus: byStatus.reduce(
      (acc: any, item: any) => ({ ...acc, [item.status]: item._count }),
      {},
    ),
  };
}
