...
export interface RulesEngineResult {
  programId: string;
  programName: string;
  eligibilityStatus: 'eligible' | 'possibly_eligible' | 'ineligible' | 'needs_review';
...
export class EligibilityRulesEngine {
  ...
  private async initializePrograms(): Promise<void> {
    try {
      ...
      logger.info(`Loaded ${this.programs.length} aid programs for eligibility assessment`);
    } catch (error) {
  ...
  async assessEligibility(
    ...
    for (const program of this.programs) {
      const result = await this.assessProgramEligibility(program, profile, questionnaire);
      programResults.push(result);
    }

    // Calculate overall results
    const eligiblePrograms = programResults.filter(r => r.eligibilityStatus === 'eligible');
    const possiblyEligiblePrograms = programResults.filter(r => r.eligibilityStatus === 'possibly_eligible');
    ...
    } else if (programResults.some(r => r.eligibilityStatus === 'needs_review')) {
    ...
    overallConfidence = programResults.reduce((sum, r) => sum + r.confidenceScore, 0) / programResults.length;

    // Generate recommendations and next steps
    const recommendedPrograms = this.generateRecommendations(programResults, questionnaire);
    ...
    logger.info(`Eligibility assessment complete: ${overallResult} (${eligiblePrograms.length} eligible programs)`);
  ...
  private async assessProgramEligibility(
    ...
    try {
      ...
      if (criteria.incomeLimit) {
        const result = this.assessIncome(program, profile, questionnaire, criteria);
        if (result.passed) {
      ...
      if (criteria.householdSizeMin || criteria.householdSizeMax) {
        const result = this.assessHouseholdSize(program, questionnaire, criteria);
        if (result.passed) {
          matchedCriteria.push(result.criterion);
        } else {
          failedCriteria.push(result.criterion);
      ...
      if (criteria.housingRequirements) {
        const result = this.assessHousingStatus(program, questionnaire, criteria);
        if (result.passed) {
          matchedCriteria.push(result.criterion);
        } else if (result.failed) {
          failedCriteria.push(result.criterion);
      ...
      const specialPopResult = this.assessSpecialPopulations(program, questionnaire, criteria);
      ...
      let eligibilityStatus: 'eligible' | 'possibly_eligible' | 'ineligible' | 'needs_review';
      ...
      logger.error(`Error assessing eligibility for program ${program.name}:`, error);
  ...
  private assessHouseholdSize(
    ...
    if (criteria.householdSizeMin && householdSize < criteria.householdSizeMin) {
      passed = false;
      criterion = `Household size minimum of ${criteria.householdSizeMin} not met`;
    } else if (criteria.householdSizeMax && householdSize > criteria.householdSizeMax) {
      passed = false;
  ...
  private assessSpecialPopulations(
    ...
    if (questionnaire.isVeteran && (criteria.veteranPreference || program.category === ProgramCategory.HOUSING)) {
    ...
    if (questionnaire.fleeDomesticViolence && criteria.domesticViolencePriority) {
  ...
  private assessGeographic(
    ...
    // County requirements
    if (criteria.counties && criteria.counties.length > 0 && questionnaire.county) {
  ...
  private generateRecommendations(
    ...
    if (questionnaire.currentHousingStatus === HousingStatus.UNSHELTERED) {
      const housingPrograms = eligiblePrograms.filter(p => p.programName.toLowerCase().includes('housing'));
    ...
    possiblePrograms.slice(0, 3).forEach(p => {
      if (!recommendations.includes(p.programId) && recommendations.length < 8) {
        recommendations.push(p.programId);
      }
    });
  ...
  private generateNextSteps(
    ...
    const eligiblePrograms = programResults.filter(r => r.eligibilityStatus === 'eligible');
    const possiblePrograms = programResults.filter(r => r.eligibilityStatus === 'possibly_eligible');
    ...
    if (possiblePrograms.length > 0) {
      steps.push('Provide additional information to clarify eligibility for some programs');
      steps.push('Speak with a benefits counselor for personalized assistance');
    }

    // Missing information steps
    const needsInfo = programResults.some(r => r.missingInformation.length > 0);
    if (needsInfo) {
    ...
    if (questionnaire.currentHousingStatus === HousingStatus.UNSHELTERED) {
      steps.unshift('Seek immediate shelter assistance - call 211 or local crisis line');
    }
  ...
  private aggregateRequiredDocuments(programResults: RulesEngineResult[]): string[] {
    
    const documents = new Set<string>();
    
    // Standard documents for most programs
    documents.add('Photo identification (driver\'s license, state ID, passport)');
    documents.add('Social Security card or verification letter');
    documents.add('Proof of income (pay stubs, benefit letters, bank statements)');
    documents.add('Proof of address or residency');
    
    // Program-specific documents
    const eligiblePrograms = programResults.filter(r => 
      r.eligibilityStatus === 'eligible' || r.eligibilityStatus === 'possibly_eligible'
    );

    eligiblePrograms.forEach(program => {
      if (program.programName.toLowerCase().includes('snap') || program.programName.toLowerCase().includes('food')) {
