...
export function validateConfiguration(): ConfigValidationResult {
  ...
  try {
    ...
    if (portConfig.backend === portConfig.frontend) {
      errors.push(
        `Port conflict: backend and frontend both use port ${portConfig.backend}`,
      );
    }
    ...
    if (actualApiUrl !== expectedApiUrl) {
      errors.push(
        `Frontend API URL mismatch: expected "${expectedApiUrl}", got "${actualApiUrl}"`,
      );
    }
    ...
    if (!envConfig.V1_STABLE) {
      warnings.push(
        "V1_STABLE not set - system may not be in stable configuration",
      );
    }

    if (!envConfig.ZERO_OPENAI_MODE) {
      warnings.push(
        "ZERO_OPENAI_MODE not enabled - may have OpenAI dependencies",
      );
    }
    ...
    if (process.env.NODE_ENV === "production") {
      ...
      if (!portConfig.strictMode) {
        warnings.push(
          "STRICT_PORT_MODE not enabled in production - ports may drift",
        );
      }
    }
    ...
    const summary: ConfigSummary = {
      ...
      payments: {
        ...
        stripe_mode: process.env.STRIPE_SECRET_KEY?.startsWith("sk_test_")
          ? "test"
...
export function logStartupConfigSummary(): void {
  try {
    ...
    console.log("🔧 [CONFIG] ======================================");
    console.log(
      `🔧 [CONFIG] Backend Port:     ${validation.summary.ports?.backend || "unknown"}`,
    );
    console.log(
      `🔧 [CONFIG] Frontend Port:    ${validation.summary.ports?.frontend || "unknown"}`,
    );
    console.log(
      `🔧 [CONFIG] Port Mode:        ${validation.summary.ports?.strict_mode ? "STRICT" : "FLEXIBLE"}`,
    );
    console.log(
      `🔧 [CONFIG] Database Mode:    ${validation.summary.database?.mode || "unknown"}`,
    );
    console.log(
      `🔧 [CONFIG] AI Provider:      ${validation.summary.ai?.provider || "unknown"}`,
    );
    console.log(
      `🔧 [CONFIG] V1 Stable:        ${validation.summary.ai?.v1_stable ? "YES" : "NO"}`,
    );
    console.log(
      `🔧 [CONFIG] Zero OpenAI:      ${validation.summary.ai?.zero_openai ? "YES" : "NO"}`,
    );
    console.log(
      `🔧 [CONFIG] Transcription:    ${validation.summary.transcription?.provider || "unknown"}`,
    );
    console.log(
      `🔧 [CONFIG] Payments:         ${validation.summary.payments?.stripe_mode || "none"}`,
    );
    console.log(
      `🔧 [CONFIG] Environment:      ${validation.summary.environment?.node_env || "unknown"}`,
    );
    console.log(
      `🔧 [CONFIG] Keys Validated:   ${validation.summary.environment?.keys_validated ? "YES" : "NO"}`,
    );
    ...
    console.error(
      "🔧 [CONFIG] Failed to log startup configuration:",
      error.message,
    );
  }
}
