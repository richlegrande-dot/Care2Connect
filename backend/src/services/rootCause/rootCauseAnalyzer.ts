...
export function analyzeRootCause(error: ErrorEntry): RootCauseAnalysis {
  ...
  if (
    ...
    combined.includes("not defined")
  ) {
    if (
      combined.includes("env") ||
      combined.includes("config") ||
      /[A-Z0-9_]+\s+is not defined/i.test(error.message || "")
    ) {
      return {
        suspectedCause: "Missing or invalid environment variable",
        recommendedFix:
          "Check .env file, verify all required variables are set, restart server",
        confidence: "high",
        category: "Config",
      };
    }
  ...
  if (
    ...
    return {
      suspectedCause:
        "network connection or API failure (timeout, Backend server unavailable)",
      recommendedFix:
        "Check internet connectivity, verify API endpoints are reachable, check firewall and proxy settings",
      confidence: "high",
      category: "Network",
    };
  }
  ...
  if (
    ...
    (combined.includes("not configured") ||
      combined.includes("no_keys_mode") ||
      combined.includes("stripe is not configured"))
  ) {
    return {
      suspectedCause: "Stripe integration not configured",
      recommendedFix:
        "Set STRIPE_SECRET_KEY in .env or enable NO_KEYS_MODE=true for demo",
      confidence: "high",
      category: "Stripe",
    };
  }
  ...
  if (
    ...
    (combined.includes("not found") ||
      ...
      combined.includes("model file"))
  ) {
    return {
      suspectedCause: "AI model files not installed or path incorrect",
      recommendedFix:
        "Download required models (Vosk/Whisper) to backend/models/ directory",
      confidence: "high",
      category: "Models",
    };
  }
  ...
  if (
    ...
    if (combined.includes("eacces") || combined.includes("permission")) {
      return {
        ...
        recommendedFix:
          "Check file permissions on backend/exports/ directory, verify disk write access",
        confidence: "high",
        category: "Export",
      };
    }
    return {
      ...
      recommendedFix:
        "Check storage configuration, verify SUPABASE_URL and credentials, check logs for details",
      confidence: "medium",
      category: "Export",
    };
  }
  ...
  if (
    ...
    (combined.includes("generate") || combined.includes("encode"))
  ) {
    return {
      suspectedCause: "QR code generation or encoding failed",
      recommendedFix:
        "Check qrcode library installation, verify data format, check logs for encoding errors",
      confidence: "medium",
      category: "QR",
    };
  }
  ...
  if (
    ...
    return {
      ...
      recommendedFix:
        "Enable DEMO_SAFE_MODE=true for auto port selection, or kill process: netstat -ano | findstr :3001",
      confidence: "high",
      category: "Port",
    };
  }
  ...
  if (
    ...
    return {
      ...
      recommendedFix:
        "Check DATABASE_URL, verify Supabase is operational, check network connectivity",
      confidence: "high",
      category: "Database",
    };
  }
  ...
  if (combined.includes("storage") || combined.includes("supabase")) {
    return {
      ...
      recommendedFix:
        "Check SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY, verify bucket exists, check storage policies",
      confidence: "high",
      category: "Storage",
    };
  }
  ...
  if (
    ...
    combined.includes("nodemailer")
  ) {
    return {
      suspectedCause: "Email/SMS subsystem archived",
      recommendedFix:
        "SMTP support has been removed; check support tickets in admin health logs",
      confidence: "medium",
      category: "Support",
    };
  }
  ...
  if (
    ...
    if (combined.includes("microphone") || combined.includes("mediadevices")) {
      return {
        ...
        recommendedFix:
          "Grant microphone permissions in browser, check device availability",
        confidence: "high",
        category: "Media",
      };
    }
    return {
      ...
      recommendedFix:
        "Check browser compatibility, verify microphone permissions, check API connectivity",
      confidence: "medium",
      category: "Recording",
    };
  }
  ...
  if (
    ...
    return {
      ...
      recommendedFix:
        "Check directory permissions (chmod 755), verify user has write access, check disk space",
      confidence: "high",
  ...
  if (
    ...
    combined.includes("ts-node") ||
    /\bts\d+\b/.test(combined) ||
    /\bts\b/.test(combined)
  ) {
    if (
      ...
      return {
        ...
        recommendedFix:
          "Run npm run typecheck, fix type errors, rebuild with npm run build",
        confidence: "medium",
        category: "TypeScript",
      };
    }
  ...
  return {
    ...
    recommendedFix:
      "Check server logs, review error stack trace, verify system health endpoints",
    confidence: "low",
    category: "Unknown",
  };
}
...
export function analyzeErrors(
  errors: ErrorEntry[],
): Array<ErrorEntry & { rootCause: RootCauseAnalysis; category?: string }> {
