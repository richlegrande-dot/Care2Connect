...
export function getPortConfig(): PortConfig {
  ...
  const maxFailoverAttempts = strictMode
    ? 0
    : parseInt(process.env.PORT_FAILOVER_RANGE || "5", 10);
...
export function getPortOwner(port: number): number | null {
  try {
    ...
    const lines = output.trim().split("\n");
    const listeningLine = lines.find(
      (line) => line.includes("LISTENING") && line.includes(`:${port}`),
    );
...
export function preflightPortCheck(config: PortConfig): void {
  if (isPortInUse(config.port)) {
    ...
    if (config.strictMode) {
      console.error(
        "🚨 STRICT_PORT_MODE is enabled - cannot start with occupied port",
      );
      console.error("   Options:");
      console.error("     1. Kill the conflicting process");
      console.error("     2. Change BACKEND_PORT in .env");
      console.error(
        "     3. Set STRICT_PORT_MODE=false (not recommended for production)",
      );
      console.error("");
      process.exit(1);
    } else {
...
export function validatePortConsistency(): void {
  ...
  if (
    apiUrl &&
    !apiUrl.includes(backendPort) &&
    !apiUrl.includes("care2connect")
  ) {
