...
class UrgencyEnhancements_v1d {
  ...
  tuneUrgencyPrecision(story, currentUrgency, category = "UNKNOWN") {
    try {
      ...
      if (currentUrgency > 0.5) {
        ...
        const falseUrgencyReduction = this.detectFalseUrgencyPatterns(
          fullText,
          category,
        );
        if (falseUrgencyReduction > 0) {
          adjustedUrgency = Math.max(
            0.3,
            adjustedUrgency - falseUrgencyReduction,
          ); // Lower bound
          totalAdjustment -= falseUrgencyReduction;
          adjustments.push(
            `false_urgency_reduction(-${falseUrgencyReduction})`,
          );
        }

        // 2. CATEGORY-SPECIFIC THRESHOLD OPTIMIZATION
        const categoryThresholdAdjustment = this.applyCategoryThresholds(
          fullText,
          category,
          currentUrgency,
        );
        if (categoryThresholdAdjustment !== 0) {
          adjustedUrgency = Math.max(
            0.3,
            Math.min(0.9, adjustedUrgency + categoryThresholdAdjustment),
          );
          totalAdjustment += categoryThresholdAdjustment;
          const sign = categoryThresholdAdjustment > 0 ? "+" : "";
          adjustments.push(
            `category_threshold_adj(${sign}${categoryThresholdAdjustment})`,
          );
        }

        // 3. CONTEXT-AWARE BOOST LIMITING
        const boostLimitReduction = this.applyContextAwareBoostLimiting(
          fullText,
          currentUrgency,
        );
        if (boostLimitReduction > 0) {
          adjustedUrgency = Math.max(
            0.3,
            adjustedUrgency - boostLimitReduction,
          ); // Lower bound
        ...
        const confidenceCapAdjustment = this.applyConfidenceBasedCapping(
          fullText,
          adjustedUrgency,
        );
        if (confidenceCapAdjustment < 0) {
          adjustedUrgency = Math.max(
            0.3,
            adjustedUrgency + confidenceCapAdjustment,
          ); // confidenceCapAdjustment is negative
      ...
      if (this.debugMode && Math.abs(totalAdjustment) > 0.01) {
        console.log(
          `[V1d Precision] Story: ${story.id || "unknown"} | Original: ${currentUrgency} | Tuned: ${adjustedUrgency} | Adjustment: ${totalAdjustment > 0 ? "+" : ""}${totalAdjustment} | Applied: ${adjustments.join(", ")}`,
        );
      }
      ...
      console.error(
        "[UrgencyEnhancements_v1d] Error in tuneUrgencyPrecision:",
        error,
      );
      return {
        originalUrgency: currentUrgency,
  ...
  detectFalseUrgencyPatterns(text, category) {
    ...
    const falseTimeMatches = falseTimePatterns.filter((pattern) =>
      pattern.test(text),
    ).length;
    ...
    const conditionalMatches = conditionalPatterns.filter((pattern) =>
      pattern.test(text),
    ).length;
    ...
    if (category === "EDUCATION") {
      ...
      const eduMatches = educationNonUrgent.filter((pattern) =>
        pattern.test(text),
      ).length;
      if (eduMatches > 0) {
    ...
    switch (category) {
      case "HEALTHCARE":
        ...
        if (/appointment|check.*up|routine|physical|annual/i.test(text)) {
          ...
          /prescription|medication/i.test(text) &&
          !/emergency|urgent|critical/i.test(text)
        ) {
      ...
      case "TRANSPORTATION":
        ...
        if (/oil.*change|tune.*up|maintenance|inspection/i.test(text)) {
          ...
          /broke.*down|repair/i.test(text) &&
          /hoping|might|could/i.test(text)
        ) {
      ...
      case "EMPLOYMENT":
        ...
        if (
          /looking.*for.*work|job.*hunting|resume|interview.*prep|interview.*clothes/i.test(
            text,
          )
        ) {
      ...
      case "EDUCATION":
        ...
        if (
          /want.*to.*learn|interested.*in|considering|exploring|program.*costs|certification.*program/i.test(
            text,
          )
        ) {
  ...
  applyContextAwareBoostLimiting(text, currentUrgency) {
    ...
    const multiPriorityCount = multiPriorityIndicators.filter((pattern) =>
      pattern.test(text),
    ).length;
    ...
    const explanationCount = detailedExplanationIndicators.filter((pattern) =>
      pattern.test(text),
    ).length;
    ...
    const politenessCount = politeLanguage.filter((pattern) =>
      pattern.test(text),
    ).length;
  ...
  applyConfidenceBasedCapping(text, currentUrgency) {
    ...
    if (currentUrgency > 0.7) {
      ...
      const evidenceCount = strongUrgencyEvidence.filter((pattern) =>
        pattern.test(text),
      ).length;
    ...
    if (currentUrgency > 0.8) {
      ...
      const criticalEvidenceCount = criticalUrgencyEvidence.filter((pattern) =>
        pattern.test(text),
      ).length;
    ...
    if (
      /personal.*situation|hard.*to.*explain|something.*personal/i.test(text) &&
      currentUrgency > 0.3
    ) {
