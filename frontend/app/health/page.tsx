...
function SupportTicketPanel() {
  ...
  const updateTicketStatus = async (ticketId: string, newStatus: string) => {
    try {
      const backendUrl = process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:3001'
      const response = await fetch(`${backendUrl}/admin/support/tickets/${ticketId}`, {
        method: 'PATCH',
        headers: {
  ...
  return (
    <div>
      ...
      {tickets.length === 0 ? (
        <div className="text-gray-500 italic">No {filter !== 'all' ? filter : ''} tickets found</div>
      ) : (
        <div className="space-y-3">
          {tickets.map((ticket) => (
            <div key={ticket.id} className="border border-gray-300 rounded p-3 hover:bg-gray-50">
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  ...
                  <div className="text-xs text-gray-400 mt-1">
                    {new Date(ticket.createdAt).toLocaleString()} • Source: {ticket.source}
                  </div>
                </div>
                <div className="ml-4">
                  <select
...
function HealthGraphs({ history }: { history: any[] }) {
  if (!history || history.length === 0) {
    return <div className="text-gray-500 italic">No historical data available yet</div>
  }
  ...
  return (
    <div className="space-y-6">
      {/* Status Distribution */}
      <div>
        <h4 className="font-semibold text-gray-700 mb-2">System Status History</h4>
        <div className="bg-gray-50 p-4 rounded">
          <div className="flex items-end gap-1 h-32">
            {statusData.slice(-20).map((d, i) => (
              <div key={i} className="flex-1 flex flex-col justify-end gap-1">
                {d.healthy > 0 && <div className="bg-green-500" style={{ height: '100%' }} />}
                {d.degraded > 0 && <div className="bg-yellow-500" style={{ height: '100%' }} />}
                {d.unhealthy > 0 && <div className="bg-red-500" style={{ height: '100%' }} />}
              </div>
            ))}
          </div>
      ...
      <div>
        <h4 className="font-semibold text-gray-700 mb-2">API Dependencies (Latency ms)</h4>
        <div className="bg-gray-50 p-4 rounded">
          <div className="flex items-end gap-1 h-32">
...
export default function HealthPage() {
  ...
  const runConnectivityTests = async () => {
    ...
    try {
      ...
      setConnectivityTests(prev => ({
        ...prev,
        backend: { ok: false, latency: Date.now() - backendStart, error: err instanceof Error ? err.message : 'Failed' }
      }))
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        const dbMessage = data.services?.db?.message || data.services?.database?.message || ''
        const detail = dbMessage.includes('FileStore') ? 'FileStore mode (no database)' : 'Database connected'
        setConnectivityTests(prev => ({
          ...prev,
        ...
        setConnectivityTests(prev => ({
          ...prev,
          database: { ok: false, latency: dbLatency, error: 'Health check failed' }
        }))
      }
    } catch (err) {
      setConnectivityTests(prev => ({
        ...prev,
        database: { ok: false, latency: Date.now() - dbStart, error: err instanceof Error ? err.message : 'Failed' }
      }))
    }
  ...
  const triggerSelfHealing = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setHealingLog(prev => [...prev, `Self-heal API failed: ${errorData.error || response.statusText}`])
      }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mt-8 bg-white p-6 rounded-lg border-2 border-purple-200">
          ...
          <div className="grid md:grid-cols-4 gap-4">
            ...
            <div className={`p-4 rounded-lg border-2 ${connectivityTests.database?.ok ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
              <div className="font-semibold text-gray-900 mb-2">Database</div>
              {connectivityTests.database ? (
                <>
                  <div className={`text-sm ${connectivityTests.database.ok ? 'text-green-700' : 'text-red-700'}`}>
                    {connectivityTests.database.ok ? 'Connected' : 'Failed'}
                  </div>
                  <div className="text-xs text-gray-600 mt-1">
                    {connectivityTests.database.latency}ms latency
                  </div>
                  {connectivityTests.database.detail && (
                    <div className="text-xs text-gray-600 mt-1">{connectivityTests.database.detail}</div>
                  )}
                  {connectivityTests.database.error && (
                    <div className="text-xs text-red-600 mt-1">{connectivityTests.database.error}</div>
                  )}
                </>
              ) : (
                <div className="text-sm text-gray-500">Testing...</div>
        ...
        {healthHistory.length > 0 && (
          <div className="mt-8 bg-white p-6 rounded-lg border-2 border-gray-200">
            ...
            {health.metrics && (
              <>
                <h2 className="text-2xl font-bold text-gray-900 mb-6 mt-12">Performance Metrics</h2>
