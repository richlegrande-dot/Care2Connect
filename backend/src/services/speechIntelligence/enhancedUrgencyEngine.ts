 ...
 */
class TemporalLayer {
  assess(text: string): { score: number; reason?: string } {
    ...
    if (
      /\b(eventually|someday|when possible|whenever|maybe)\b/.test(textLower)
    ) {
...
class CrisisPatternLayer {
  assess(text: string): { multiplier: number; reasons: string[] } {
    const textLower = text.toLowerCase();
    let multiplier = 0.0;
    ...
    if (
      /\b(eviction|evicted|homeless|starving|dying|life-threatening)\b/.test(
        textLower,
      )
    ) {
      multiplier += 0.6;
    ...
    if (
      /\b(dangerous|unsafe|threat|violence|violent|domestic|abuse)\b/.test(
        textLower,
      )
    ) {
      multiplier += 0.4;
    ...
    if (
      /\b(emergency|critical|hospital|surgery|urgent care)\b/.test(textLower)
    ) {
      multiplier += 0.5;
...
class CategoryContextLayer {
  assess(text: string, category: string): { boost: number; reason?: string } {
    const textLower = text.toLowerCase();

    switch (category) {
      case "SAFETY":
        // Safety always gets maximum boost (0.30 - ITERATION 1: was 0.25)
        return { boost: 0.3, reason: "category_safety_max" };

      case "LEGAL":
        // Check for court dates, deadlines (0.24 - ITERATION 1: was 0.20)
        if (
          /\b(court|hearing|trial|deadline|legal deadline)\b/.test(textLower)
        ) {
      ...
      case "MEDICAL":
        ...
        if (
          /\b(pain|emergency|critical|surgery|life-threatening)\b/.test(
            textLower,
          )
        ) {
      ...
      case "HOUSING":
        ...
        if (
          /\b(evict|eviction|kicked out|losing apartment|losing home)\b/.test(
            textLower,
          )
        ) {
          return { boost: 0.3, reason: "category_housing_eviction_risk" };
        }
      ...
      case "EMPLOYMENT":
        ...
        if (
          /\b(work|job|employment|can't get to work|need for work)\b/.test(
            textLower,
          )
        ) {
...
class SignalCombinationLayer {
  assess(text: string): { multiplier: number; matchedSignals: string[] } {
    const textLower = text.toLowerCase();
    ...
    const signals = [
      ...
      {
        pattern: /\b(don't know|can't|unable|impossible)\b/,
        name: "inability",
      },
      {
        pattern: /\b(worried|scared|afraid|anxious|terrified)\b/,
        name: "fear",
      },
      {
        pattern: /\b(last resort|out of options|nowhere to turn|no choice)\b/,
        name: "desperation",
      },
      { pattern: /\b(urgent|emergency|critical|serious)\b/, name: "severity" },
      {
        pattern: /\b(children|kids|family|elderly|disabled)\b/,
        name: "vulnerable_dependents",
      },
    ];
    ...
    const multiplier =
      signalCount <= 1 ? 1.0 : 1.0 + Math.min(signalCount - 1, 3) * 0.1;
...
class EmotionalDistressLayer {
  assess(text: string): { boost: number; reasons: string[] } {
    const textLower = text.toLowerCase();
    let boost = 0.0;
    const reasons: string[] = [];
 ...
 */
export class EnhancedUrgencyEngine {
  private temporalLayer: TemporalLayer;
  private crisisPatternLayer: CrisisPatternLayer;
  private categoryContextLayer: CategoryContextLayer;
  private signalCombinationLayer: SignalCombinationLayer;
  private emotionalDistressLayer: EmotionalDistressLayer;
  ...
  assess(text: string, category: string): UrgencyResult {
    ...
    if (temporal.reason) debug.reasons.push(temporal.reason);
    ...
    if (categoryBoost.reason) debug.reasons.push(categoryBoost.reason);
  ...
  private applyConfidenceThresholds(
    ...
    if (score >= 0.7 - thresholdAdjustment) {
      return "CRITICAL";
    }
