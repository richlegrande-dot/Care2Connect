...
const mockProfile = {
  ...
  donationPitch:
    "Help John get back on his feet with stable housing and work opportunities.",
  cashtag: "$johndoe123",
...
describe("ProfileCard", () => {
  it("renders profile information correctly", () => {
    ...
    expect(
      screen.getByText(
        "Experienced construction worker looking for stable employment opportunities.",
      ),
    ).toBeInTheDocument();
  ...
  it("shows QR code when available", () => {
    ...
    expect(qrCodeImage).toHaveAttribute(
      "src",
      "/qr-codes/cashapp-johndoe123.png",
    );
  });
  ...
  it("handles long bio text with proper truncation", () => {
    const longBioProfile = {
      ...mockProfile,
      bio: "This is a very long bio that should be truncated or handled properly in the UI. ".repeat(
        10,
      ),
    };
    ...
    const bioElement = screen.getByText(
      /This is a very long bio that should be truncated or handled properly in the UI\./,
    );
