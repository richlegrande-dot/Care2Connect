...
class UrgencyEnhancements_v1c {
  ...
  enhanceUrgency(story, currentUrgency) {
    try {
      ...
      if (currentUrgency < 0.5) {
        ...
        if (medicalEmergencyBoost > 0) {
          adjustedUrgency = Math.min(
            0.85,
            adjustedUrgency + medicalEmergencyBoost,
          ); // Lower cap
          totalBoost += medicalEmergencyBoost;
          enhancements.push(
            `medical_emergency_boost(${medicalEmergencyBoost})`,
          );
        }
        ...
        if (housingCrisisBoost > 0) {
          adjustedUrgency = Math.min(
            0.85,
            adjustedUrgency + housingCrisisBoost,
          ); // Lower cap
        ...
        if (employmentCrisisBoost > 0) {
          adjustedUrgency = Math.min(
            0.85,
            adjustedUrgency + employmentCrisisBoost,
          ); // Lower cap
          totalBoost += employmentCrisisBoost;
          enhancements.push(
            `employment_crisis_boost(${employmentCrisisBoost})`,
          );
        }
      ...
      if (this.debugMode && totalBoost > 0) {
        console.log(
          `[V1c Enhancement] Story: ${story.id || "unknown"} | Original: ${currentUrgency} | Enhanced: ${adjustedUrgency} | Boost: +${totalBoost} | Applied: ${enhancements.join(", ")}`,
        );
      }
      ...
      console.error(
        "[UrgencyEnhancements_v1c] Error in enhanceUrgency:",
        error,
      );
      return {
        originalUrgency: currentUrgency,
  ...
  detectTimeCriticalScenarios(text) {
    ...
    const deadlineMatches = immediateDeadlines.filter((pattern) =>
      pattern.test(text),
    ).length;
    ...
    const legalMatches = legalDeadlines.filter((pattern) =>
      pattern.test(text),
    ).length;
    if (legalMatches > 0) {
  ...
  detectFamilyCrisis(text) {
    ...
    const childRiskMatches = childrenAtRisk.filter((pattern) =>
      pattern.test(text),
    ).length;
    ...
    const singleParentMatches = singleParentCrisis.filter((pattern) =>
      pattern.test(text),
    ).length;
    ...
    const separationMatches = familySeparation.filter((pattern) =>
      pattern.test(text),
    ).length;
  ...
  detectMedicalEmergency(text) {
    ...
    const lifeThreateningMatches = lifeThreatening.filter((pattern) =>
      pattern.test(text),
    ).length;
    ...
    const surgicalMatches = surgicalUrgency.filter((pattern) =>
      pattern.test(text),
    ).length;
    ...
    const pregnancyMatches = pregnancyEmergency.filter((pattern) =>
      pattern.test(text),
    ).length;
    ...
    const mentalHealthMatches = mentalHealthCrisis.filter((pattern) =>
      pattern.test(text),
    ).length;
  ...
  detectHousingCrisis(text) {
    ...
    const evictionMatches = imminentEviction.filter((pattern) =>
      pattern.test(text),
    ).length;
    ...
    const homelessMatches = homelessness.filter((pattern) =>
      pattern.test(text),
    ).length;
    ...
    const utilityMatches = utilityShutoffs.filter((pattern) =>
      pattern.test(text),
    ).length;
    if (utilityMatches > 0) {
    ...
    const unsafeMatches = unsafeHousing.filter((pattern) =>
      pattern.test(text),
    ).length;
    if (unsafeMatches > 0) {
  ...
  detectEmploymentCrisis(text) {
    ...
    const jobLossMatches = suddenJobLoss.filter((pattern) =>
      pattern.test(text),
    ).length;
    if (jobLossMatches > 0) {
    ...
    const necessityMatches = workNecessity.filter((pattern) =>
      pattern.test(text),
    ).length;
    ...
    const transportMatches = workTransportation.filter((pattern) =>
      pattern.test(text),
    ).length;
    ...
    const supportMatches = workSupport.filter((pattern) =>
      pattern.test(text),
    ).length;
    if (supportMatches > 0) {
