...
function Test-ConfigurationConsistency {
    ...
    $criticalVars = @('V1_STABLE', 'ZERO_OPENAI_MODE', 'DB_MODE', 'NODE_ENV')
    $missingVars = @()
    
    foreach ($var in $criticalVars) {
        $envVar = Get-Item "Env:$var" -ErrorAction SilentlyContinue
        if (-not $envVar -or -not $envVar.Value) {
            $missingVars += $var
        }
    }
...
function Show-TestSummary {
    ...
    if ($failedTests -eq 0 -and $successRate -ge 90) {
        ...
        Write-Host ""
        Write-Host "SYSTEM TEST PASSED WITH WARNINGS"
        return 0
    } else {
        Write-Host ""
