...
describe("Telemetry Performance Guards", () => {
  ...
  test("PERFORMANCE GUARD: Memory usage stays bounded", async () => {
    ...
    expect(results.every((r) => r.sessionId.startsWith("extraction_"))).toBe(
      true,
    );
  });
  ...
  test("PERFORMANCE GUARD: Telemetry buffer management", () => {
    ...
    for (let i = 0; i < bufferTestSize; i++) {
      extractAllWithTelemetry(
        `Test transcript ${i} with different content each time`,
      );
    }
    ...
    console.log(
      `- Average: ${averageTimePerExtraction.toFixed(2)}ms per extraction`,
    );
  ...
  test("PERFORMANCE GUARD: Prometheus metrics export speed", () => {
    ...
    const averageTime = times.reduce((a, b) => a + b) / times.length;
    const averageSize =
      exportSizes.reduce((a, b) => a + b) / exportSizes.length;
  ...
  test("PERFORMANCE GUARD: Concurrent extraction performance", async () => {
    ...
    const promises = Array(concurrentExtractions)
      .fill(0)
      .map((_, i) => {
        return Promise.resolve(
          extractAllWithTelemetry(
            `${PERFORMANCE_TRANSCRIPT} - Concurrent test ${i}`,
          ),
        );
      });
    ...
    console.log(
      `- Average per extraction: ${(totalTime / concurrentExtractions).toFixed(2)}ms`,
    );
