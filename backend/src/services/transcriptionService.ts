...
export class TranscriptionService {
  ...
  async transcribeAudio(audioFilePath: string): Promise<TranscriptionResult> {
    try {
      ...
      if (!provider.isAvailable()) {
        throw new Error(
          "Transcription provider not available. Please use manual transcript mode.",
        );
      }
      ...
      if (result.confidence < 0.5) {
        warnings.push(
          "Low transcription confidence. Consider re-recording with clearer audio.",
        );
      }
      if (!result.transcript || result.transcript.length < 20) {
        warnings.push(
          "Transcript seems very short. Please check if full story was recorded.",
        );
      }

      console.log(
        `[Transcription] Completed successfully (${result.wordCount} words)`,
      );
  ...
  async processManualTranscript(
    ...
    if (cleanTranscript.length < 20) {
      warnings.push(
        "Transcript seems very short. Consider adding more details about your situation.",
      );
    }

    if (cleanTranscript.length > 5000) {
      warnings.push(
        "Transcript is quite long. Consider focusing on the most important points.",
      );
    }
    ...
    if (!/[.!?]/.test(cleanTranscript)) {
      warnings.push(
        "Consider adding proper punctuation to improve story clarity.",
      );
    }
  ...
  async extractProfileData(transcript: string): Promise<ExtractedProfileData> {
    try {
      ...
      return {
        ...
        donation_pitch:
          "Every contribution helps provide stability and opportunity.",
        tags: [],
  ...
  private validateExtractedData(data: any): ExtractedProfileData {
    ...
    return {
      ...
      long_term_goals: Array.isArray(data.long_term_goals)
