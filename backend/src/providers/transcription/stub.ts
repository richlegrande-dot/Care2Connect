...
export class StubTranscriptionProvider implements TranscriptionProvider {
  ...
  private fixtures: Array<string | {transcript: string; wordCount?: number; detectedLanguage?: string}> = [];
      ...
      try {
        ...
        try {
          ...
          if (parsed.transcript) {
            ...
            console.log(`[StubProvider] Loaded single fixture from JSON string`);
            return;
          } else if (Array.isArray(parsed)) {
            this.fixtures = parsed;
            console.log(`[StubProvider] Loaded ${this.fixtures.length} fixture(s) from JSON array`);
            return;
          } else if (typeof parsed === 'object' && parsed.transcripts) {
            this.fixtures = parsed.transcripts;
            console.log(`[StubProvider] Loaded ${this.fixtures.length} fixture(s) from JSON object`);
            return;
          }
        } catch (jsonError) {
        ...
        if (fs.existsSync(fixturePathOrJson)) {
          console.log(`[StubProvider] Loading fixtures from file: ${fixturePathOrJson}`);
          ...
          console.log(`[StubProvider] Loaded ${this.fixtures.length} fixture(s) from file`);
          return;
        }
      } catch (error) {
        console.warn('[StubProvider] Error loading fixtures, using defaults:', error);
      }
    }
  ...
  async transcribe(
    ...
    console.log(`[StubProvider] Simulating transcription for: ${audioFilePath}`);
    ...
    switch (stubMode) {
      ...
      case StubMode.NEEDS_INFO:
        ...
        console.log(`[StubProvider] NEEDS_INFO mode - returning minimal transcript`);
      ...
      default:
        ...
        const configuredDelay = parseInt(process.env.STUB_TRANSCRIPTION_DELAY_MS || '0');
        const latency = configuredDelay > 0 ? configuredDelay : 150 + Math.random() * 150;
        return this.generateResponse(latency);
    }
  }

  private async generateResponse(delay: number): Promise<TranscriptionResult> {
    console.log(`[StubProvider] Simulating ${Math.round(delay)}ms processing time`);
    await new Promise(resolve => setTimeout(resolve, delay));
    
    // Select random fixture
    const fixtureItem = this.fixtures[Math.floor(Math.random() * this.fixtures.length)];
    ...
    console.log(`[StubProvider] Returning fixture transcript (${wordCount} words, ${Math.round(delay)}ms)`);
