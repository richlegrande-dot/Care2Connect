...
describe('QR Code Pipeline Tests', () => {
  ...
  describe('QR Code Generation (Library Tests)', () => {
    ...
    test('should handle special characters in URLs', async () => {
      const urlWithParams = 'https://example.com/donate?name=John%20Doe&amount=100';
    ...
    test('should support different error correction levels', async () => {
      ...
      const qrLow = await QRCode.toDataURL(testUrl, { errorCorrectionLevel: 'L' });
      const qrMedium = await QRCode.toDataURL(testUrl, { errorCorrectionLevel: 'M' });
      const qrHigh = await QRCode.toDataURL(testUrl, { errorCorrectionLevel: 'H' });
  ...
  describe('Performance Tests', () => {
    test('should generate 5 QR codes sequentially within budget', async () => {
      ...
      const elapsed = Date.now() - start;
      expect(elapsed).toBeLessThan(3000); // 3 seconds for 5 QR codes
    });

    test('should handle concurrent QR generation', async () => {
      const urls = Array.from({ length: 5 }, (_, i) => `https://example.com/test${i}`);
      ...
      expect(qrCodes.every(qr => qr.startsWith('data:image/png;base64,'))).toBe(true);
      expect(elapsed).toBeLessThan(3000); // Generous 3 second budget for concurrent generation
    });
  });
});
