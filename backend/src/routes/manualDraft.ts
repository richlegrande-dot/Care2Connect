...
import { ManualDraftRequest, ManualDraftResponse, DonationDraftData } from '../types/fallback';
...
router.post('/manual-draft', async (req: Request, res: Response) => {
  try {
    const { ticketId, title, story, goalAmount, currency = 'USD' }: ManualDraftRequest = req.body;
